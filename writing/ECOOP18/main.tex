...
\noindent
The program start with an instance of \C, cast it to \D, then cast it back
to \C. The reason we generate pass-through methods, for instance, the 
wrapper that enforce type \D has a method \xt{b}, is that if we did not
generate them, some methods would be ``lost''. In this example, when
we cast the object back to \C, it is necessary to be able to invoke
method \xt{b}.
...
encountered, the \xt{bcast} metafunction is used to generate a wrapper
class, which is then instantiated, producing a new class table, return
value, and heap. We must then show that the new class table is well formed,
that the new heap is also well formed, and that the new wrapper is a subtype
of the given type $\C$.  Proving these properties is relatively
straightforward. Class table well- formedness follows by construction of the
wrapper class and by well-formedness of the old class table. Heap
well-formedness follows by well-formedness of the class table, construction
of the new heap, and well-formedness of the old heap.  Lastly, proving that
the type of the wrapper is a subtype of the required type proceeds by
structural induction over the required type.
