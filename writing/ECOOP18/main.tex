...
systems. The litmus test is shown in \figref{litmus} and its constituent
...
\kafka in
Coq\footnote{Link to source removed for blind review}. %https://github.com/BenChung/GradualComparison/tree/master/impl
...
The type system for the source language, shown in \figref{fig:slts}, is a
straightforward static semantics for gradual typing. To differentiate this
type system from the \kafka type system, we use the $\EnvTypeS \Env\K\e\t$
judgment form, indicating a well-typed expression in the surface type system.
This surface static semantics allows implicit mixing of static and dynamically
typed terms and is shared between all of the gradual type systems. As a
gradually typed language, the source language type system allows the implicit
conversion of typed and untyped terms via the convertibility operator, written
$\ConvertE\K{s}{\t}{\tp}$.

This $\ConvertE\K{s}{\t}{\tp}$ relation indicates that type $\t$ is
convertible to one of type $\tp$, and is used both for traditional up-casting
and for conversions of \any to non-\any types. Our definition of the type
convertibility operator is shown in \figref{fig:tyconvert}. The operator
enforces type discipline where possible (via \RuleRef{STGC-SUB}) while
allowing implicit conversion to and from the dynamic type where not (via
\RuleRef{STGC-TOANY} and \RuleRef{STGC-ANYCONC}). To avoid collapsing the type
...
untyped code, and must be protected from misuse. Likewise, if \RuleRef{STGC-ANYCONC} 
is used, then an untyped value is being passed into typed code, and type invariants 
must be checked. 

\subsection{Translations}

The translations between the source language and \kafka are the cornerstone of
our approach. These translations encode the soundness guarantee of the gradual
type system into \kafka types and protect this guarantee via cast insertion,
the details of which depend on the type system in question.
...
checks are done. Transient ensures that all specified methods exist
on the value, but not what the argument or return types of those methods are.

Mutable state in transient is handled by checking at dereference time. Any
value can be assigned to a field, but must conform to the statically expected
type when fetched. We can express this in \kafka by converting all field types
to \any.

Thanks to \kafka's use of structural subtyping, encoding the transient
soundness guarantee is simple. To allow methods to take or return any value,
we first erase all of the types on arguments and return types, eliminating all
type information. By erasing all types we then convert the subtype cast into a
simple check that one type has all the method names as the other, exactly the
check semantics  we need. We then use the subtype cast to check the argument
type prior to (translated) method body entry, matching the transient soundness
guarantee.
...
of soundness of the source language into \kafka, a sound intermediate
...
We present four expression translation systems, one for each of the gradual
type systems in question. Each takes a well-typed source language term and
translates it into a well-typed \kafka term, in the presence of a suitably
translated class table. These expression translation systems come in one of
two forms - a straightforward typeless translation used for the optional
semantics, and a bidirectional type-driven translation used for the other
three gradual type systems.

\begin{itemize}
  \item $\TR[\OTS]\e$: Used for the optional semantics, this form translates the source level term $\e$ into a \kafka expression. Using this form, the translation of \e is completely independent of its statically known type.
  \item Our system has two type-driven translation forms, whose design arises from prior work on bidirectional type checking~\cite{pierce:1998:local}. 

  In order to determine where casts must be added, the type-driven
  translations need to find the places where a dynamically-typed term was used
  in place of a statically-typed one, or vice versa. While the existing type
  system can find out the type of some sub-term, it cannot determine what type
  the surrounding expression expected. To acquire this information during translation,
  we use a second translation form which takes the expected type of the expression being
  translated.

  \begin{itemize}
    \item $\TRG[\SOMS]\e\Env$: For the system $\SOMS$, and against the typing environment $\Env$, translate the source level term $\e$ to \kafka. 

    This form, analogous to the \emph{synthetic} case of Pierce and Turner, is used
    when no type is demanded of the resulting \kafka term.
    \item $\TAG[\SOMS]\e\Env\t$: For the system $\SOMS$, and against the typing environment $\Env$, translate the source term $\e$ to \kafka, 
    guaranteeing that the \kafka term has type \t.

    This form, which we call assert translation, is equivalent to Pierce and
    Turner's \emph{analytic} case, when some type is required of the expression
    \e. 
  \end{itemize}
\end{itemize}

We begin with the assert translations, as they enforce the soundness guarantee
encoded by class translation, and illustrate how each gradual type system
approaches the dynamic enforcement of soundness.
...
Convertibility and assert translation, shown in \figref{fig:trtype}, have
a close correspondence. Every type-driven translation has two rules. The first
handles the case of \RuleRef{STGC-SUB}, when the required type happens to be a
supertype of the expression's actual type, and no further action is required
of the translation. The second case shows the behavior when 
\RuleRef{STGC-TOANY} or \RuleRef {STGC-ANYCONC} were needed to conclude the source level
typing judgment. In these cases, we are converting to or from \any, and
therefore dynamic behavior that depends on the soundness guarantee is
required.

In this second case, the encoding of surface level types into \kafka becomes
paramount. While the concrete and behavioral translations are predictable -
checking the runtime type of a value with subtype cast and behavioral casts,
respectively - the transient assert translation is unusual. Despite offering a
much weaker soundness guarantee than the concrete semantics, the transient
assert translation is identical.

This similarity is caused by the different class translations used by the
concrete and transient semantics. The encoding of surface level types in
\kafka according to the transient semantics,  shown in
\figref{fig:traclass}, calls for all types to be erased from methods while
the concrete semantics retains them. As a result, the concrete subtype cast
will check method types where the transient subtype cast will not.
...
Under the transient semantics, we have to treat both variable references
and field access differently. Variables are translated to contain a cast
to their statically known type, as while the method checks this upon entry
(maintaining error semantics), this information cannot be bound in \kafka,
as no binding forms exist outside of functions. For simplicity, we therefore
insert a redundant cast (which will always succeed if reached) at each variable
reference. Moreover, under the transient semantics, the type of field values
is not guaranteed, so dereference from fields must be checked.

This last property of the transient semantics creates unusual behavior.
In particular, if an ill-typed value is inserted by untyped code into a 
typed data structure, then the error will occur at dereference, rather than
assignment. Even in the presence of a blame system, this error could be
problematic, as the assigner will only ever be blamed if the reference is
ever accessed and therefore ill-typed values can exist in the heap without
error.

The weakest type system under investigation is the optional type system, 
which provides no soundness guarantee whatsoever. Here, we have the unusual
problem that direct translation would have \emph{too many} known types, as the
\this reference's type is always known. To allow the use of dynamic invocation
on the \this reference, we therefore cast \this to \any.
...
Field assignment is much more typical than variable and field access. Shown in
\figref{fig:trassn}, the only stand-out case is the transient semantics, which
uses the assert translation mechanism to ensure that the assigned value is of
type \any, not the statically asserted type of the field. This is due to the
weak guarantees provided by the transient semantics, and again indicating that
no assignment to a typed field in transient will ever fail.
...
In the object oriented context, a type system being sound means that no
...
then shown through the result of translation, as the \kafka type encapsulates
the source language gradual typing guarantee.

At invocation sites, to avoid ``method not understood'' errors, the available
methods and their types are the key property. If a method is known to exist,
and have the correct argument and return types as in the concrete and
behavioral semantics, it follows that the method can be called safely. However,
if no methods are guaranteed, then the untyped invocation (with no soundness
guarantee) must be used.

The detailed translations for invocation are shown in \figref{fig:trafuninv}. 
Each translation illustrates the soundness guarantee provided by the respective
gradual type system through its choice of invocation. The optional semantics
provides no guarantee whatsoever, so it has to always use dynamic invocation.
The transient semantics is sometimes able to guarantee the existence of the
method being called, but not what types it has, so it has to call it under 
type \any and deal with a return type of \any with a cast. Finally, the
optional and concrete systems both guarantee the existence and types of 
methods on typed values, so can make statically-checked, fully typed 
invocations.

\subsection{Litmus Test Translation}

So far, we have compared approaches to gradual typing through two mechanisms,
a litmus test that errors in different ways when executed in different semantics
and a formal translation that shows how the gradual type systems differ formally.
We can then use our translation to explain the behavior of the litmus test, showing
where and why one of the litmus test programs fails.
...
\hrule\vspace{4mm}
 ...
 \caption{Class translation for litmus test 3.} \label{fig:l3trans}
\end{figure}

In this example, we will focus on litmus test 3, as its execution displays the
most varied behavior between each of the gradual typing semantics. The
operational principle of litmus test 3 is that it creates a new object (an
instance of \C), then uses an untyped shim to represent it as type \xt{E}
with an incorrect signature for one of its methods but does not use it as an
\xt{E}. This test serves to identify the behavioral semantics which applies
a wrapper forcing the instance of \C to act like an \xt{E}, despite only
ever being used under type \C.

The key to making litmus test 3 pass the \kafka type checker is the translation
of \xt{F} into \kafka. Inside \xt{F}, method \xt{n} is responsible for calling
method \m with an ill-typed argument using the dynamic type \any as a way to
get it past the type checker. These coercions between \any and \xt{E} must then
be guarded by each of the gradual type systems according to their own semantics.

Under the optional semantics, this guard is nonexistent. The optional semantics
provides no type guarantees whatsoever, so there is no special need to protect
\m from \n. As a result, the optional semantics simply calls \m from \n using the
dynamic invocation form.
