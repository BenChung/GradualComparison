...
The classic definition of soundness in an object oriented setting is that no
"message not understood" errors will be produced, or that every well-typed
method call will find a receiver. \kafka's type system is sound in this
traditional sense, but the source language type systems are not.

Under each of the source language gradual type systems, a different
soundness guarantee is provided for invocations, indicating which calls
are guaranteed to succeed and which are not. Moreover, each source language
type system provides different guarantees about the arguments passed to 
each method.

These differences can be clearly seen in the source-to-\kafka translations,
seen in \figref{fig:trafuninv}. Each translation shows the behaviour taken by
each gradual typing semantics at the call site in a fully explicit way, as
otherwise the result of translation would not pass the \kafka type system.

All three (sound) gradual type systems produce the same results when
given an untyped invocation, but translation of the typed invocation varies,
depending on the desired soundness guarantee. In particular, the transient
semantics differs from the translation of the behavioral and concrete systems.

The transient semantics's translation results from its guarantee (or lack
thereof) of argument and return types. The transient semantics guarantees the
existence of the required methods on a type, but leaves it up to the method
and the caller, respectively, to check the argument and return types. Therefore,
as detailed in class translation, we erase all method types under the transient
semantics and make statically-checked calls to the relevant methods under type
\any, then cast the return value.

In contrast, the behavioral and concrete semantics do guarantee the argument
and return types of methods. As a result, under these semantics, we take the
straightforward translation of checking the argument values using assert
translation, then letting the well-typed return value through directly.
