...
programmers to add types to untyped code incrementally. These approaches aim
to improve the readability and usability of their associated language, and
devote substantial effort to supporting their language's idioms. However, 
by focusing on one language, the fundamental guarantees the gradual type
system provides can be lost, hidden under an ocean of language details.

Nowhere is this problem more apparent than in gradual type systems for object-
oriented languages. The first effort, by Siek and Taha~\cite{SiekTaha07}
presented a gradual type system for a variant of Abadi and Cardelli's  object-
based calculus~\cite{cardelli:1996:theory-of-objects}, adding a notion of
subtype consistency on top of their earlier work for functional programming
languages~\cite{SiekTaha06}. However, this approach devotes little attention
to mutability and aliasing, while disregarding performance entirely, spawning
a decade of research on this complex topic.

However, despite this effort, the practical realization of Siek and Taha's
elegant idea remains elusive. These features that Siek and Taha elided
have turned out to be some of the most complex and challenging to deal
with, and no one approach has proven to be the most successful.

Numerous languages include approaches to gradually typing objects, no
two exactly the same, with some highlights including:
...
programs. However, their fundamental guarantees differ in key ways, and
formally comparing these approaches is extremely difficult, induced by
different syntax, different formalizations, and different types. As a result,
foundational differences in what it means to be gradually typed are easy to
overlook among dozens of other language-specific differences. To compare
gradual type systems, a common language that supports multiple approaches is
needed, enabling direct and easy comparison.
...
To validate \kafka, we have both proven the \kafka type system sound and
implemented the calculus on top of the CLR.
...
calculi. At the top level, classes are notated as \Class\C{\fd[1]..}{\md[1]..},
...
the type dynamically. More details of how this cast works are discussed in
the section on the behavioral semantics.

The minimal calculus would only have one cast operation, with the natural most
general choice being $\SubCast\t\e$. However, generating the code for the
wrappers created by $\BehCast\t\e$ is extremely complex and must happen at
runtime (WHY?), in turn effectively mandating the use of a special cast form.
