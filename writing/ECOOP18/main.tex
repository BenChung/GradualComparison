...
\begin{document}
\title{A Combined Framework for Object-Oriented Gradual Typing}
...
will check method types where the transient subtype cast will no
...
operations are translated verbatim. The grey background is used to identify
translated \kafka terms. 

Even for these simple terms, the behavior of some gradual type systems is
unusual. While the behavioral and concrete systems translate according to the
identity function, the optional and transient approaches are more complex. The
optional semantics erases all types; every operation under the optional
semantics is dynamic. In order for this to be well-typed according to \kafka,
every term produced by the behavioral semantics must be of type \any, and one
term, \this, is always typed. We solve this unusual problem by casting every
self reference to \any. Under the transient semantics, meanwhile, we face the
problem that we erase all types on methods and fields - an artifact of the
guarantee, as mentioned previously. To recover these types, we type check the
expression in question, then cast to the statically-known type.
...
\figref{fig:trafuninv} depicts the translation of function invocation
