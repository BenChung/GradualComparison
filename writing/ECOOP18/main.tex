...
\end{abstract}
% THANKS: Francesco Zappa Nardelli, Leif Andersen
\section{Introduction}

%\epigraph{\small\it ``Because half the problem is seeing the problem''}
%\vspace{-5mm} 

\noindent There has never been a single approach to gradual typing. In 2006, the
field was created by two simultaneously published, independently developed papers. The
first, by Siek and Taha, allows the incremental typing of individual lambda calculus
terms through a consistency relation, and inserts casts based on a simple type
inference system \cite{SiekTaha06}. The second, by Tobin-Hochtstat and
Felleisen, adds types to entire Scheme modules, using constraint solving
to find where casts must be added in untyped code~\cite{tf-dls06}.

These two very different solutions, in very different languages, set the trend
for the next ten years of gradual typing research. Gradual type systems today
support an enormous variety of languages, enforcement mechanisms, and
soundness guarantees. However, this linguistic diversity is not without
consequence.

Necessarily, the type system, syntax, and semantics of a programming language
are tightly coupled, and each has to deal with the language's complexity. As a
result, if we were to implement the same gradual typing semantics for two
different  programming languages, the resulting semantics seem dissimilar,
despite both being semantically identical. Comparing two different gradual
type systems, implemented for two different languages, is correspondingly
more difficult.

The challenge in comparing the fundamental semantics of gradual type systems
is particularly apparent for object-oriented languages. Siek and Taha's first
effort~\cite{SiekTaha07} presents a gradual type system for a variant of Abadi
and Cardelli's object- based calculus~\cite{cardelli:1996:theory-of-objects},
relating objects by generalizing their earlier notion of
consistency~\cite{SiekTaha06} over structural subtyping. This early work has
several major holes, most notably in handling mutable state, aliasing, and
subtyping, vital components of imperative object-oriented languages.

Underlying each of the subsequent gradual type systems for object-oriented
languages is some solution to these problems, leveraging key differences in
soundness property and enforcement, but these fundamental details are obscured
by language features. Gradual type systems enforce their invariants via translation
to a stricter target language, but these targets are just as diverse as the
source languages, ruling out easy comparison. Moreover, gradual typing imposes
unique demands for dynamic typing and for casts on the target language, ruling
out standard target calculi.

Even with the same target, different gradual type systems for different
languages will produce dissimilar code, again frustrating detailed comparison.
In addition to standardizing the target language, the source language must 
be equally held constant, reducing the point of comparison to the translation
alone.

In this paper, we present a new framework for comparing gradual type systems,
standardizing the source and target language. We propose a new common target
language, called \kafka, providing the minimum set of functionality needed
for gradual typing while still offering a sound static type system, and use it
along with a common source language to implement four different gradual type
systems.
