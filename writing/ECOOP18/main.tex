...
Nowhere is this problem more apparent than in gradual type systems for object-oriented
 languages. The first effort, by Siek and Taha~\cite{SiekTaha07}
presents a gradual type system for a variant of Abadi and Cardelli's  object-based calculus~\cite{cardelli:1996:theory-of-objects}, adding a notion of
...
allowing programmers to add types progressively to previously untyped
...
different syntax, different formalizations, and different types. 
The differences between the gradual typing approaches of these languages are easy to
overlook amongst the many other language-specific differences between them.
In order to comprehensively compare these gradual type systems
a common language that supports all of the gradual approaches is needed. 

\noindent In this paper, we make two main contributions:
...
language design decisions. To validate \kafka, we have both proven the \kafka 
type system sound and implemented the calculus on top of the CLR.
...
\noindent We validate the source language design and translation semantics through
