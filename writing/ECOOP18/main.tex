...
\begin{tabbing}
\small\hspace{1cm} \class \C \=\{\\
 \>\HS\HS\Mdef\m\x\Int\Int{\HS\x+2\HS}\\
 \>\HS\HS\=\mp(\x:~\any)~:~\any\{~\SubCast\any{\KCall\this\m{\SubCast\Int\x}\Int\Int} \HS\}\\
     \> \}
\end{tabbing}

\noindent
and assume that that the class table \K has a type definition for \xt{Int}
furthermore we allow ourselves to use addition and integers (these could be
encoded).


The dynamic invocation form, \DynCall\e\m\ep 

in the semantics, $\DynCall\a\m\ap$, requires
...
$\xt{bcast}$'s semantics are shown in \figref{behavetext}.
