...
The presence of the dynamic type is sufficient to allow writing gradually
typed code. A program with no occurrence of \any is statically typed, in
which case, the usual soundness guarantee holds, namely the program will not
get stuck on a method invocation. A program where all variables are
annotated to be \any is fully dynamic and any invocation may get stuck.

Gradual typing is used when an expression of dynamic type occurs as an
argument to a call of a method that expects some other type \T. The static
type system of the surface language allows such cases of implicit coercion,
but the gradual enforcement mechanism has to introduce run-time checks to
catch potential type mismatches. The semantics of the various gradual type
systems will be given in terms of a translation from the surface language to
a common calculus. In this section, we appeal to the reader's intuition.

A more detailed explanation of the surface language will helpful to
understand the translations presented in the following sections.  Subtyping
in the calculus is structural, \StrSub\M\K\C\D holds if class \C has all the
methods of class \D and the arguments and return types are related by
subtyping in the usual contra- and co-variant way.  \K is the class table
that holds definitions of all classes in the program.
