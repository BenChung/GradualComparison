...
individual Scheme terms through a consistency relation, and inserts
casts based on a simple type inference system \cite{SiekTaha06}. The second,
by Tobin-Hochtstat and Felleisen, adds types to entire modules, using
constraint solving to find where casts must be added in untyped
code~\cite{tf-dls06}. These two quite different solutions, solving the same
program in the same language, set the trend for the next ten years of
...
diversity is not without consequence.
...
both starting with the same ideas. Comparing different gradual type systems,
targeting implemented for different languages, is correspondingly more
difficult.  The difficulty is particularly apparent for object-oriented
languages. Siek and Taha's first effort~\cite{SiekTaha07} presents a gradual
type system for a variant of Abadi and Cardelli's object- based
calculus~\cite{cardelli:1996:theory-of-objects}, relating objects by
generalizing their notion of consistency~\cite{SiekTaha06} over structural
subtyping. This early work has several drawbacks, most notably in handling
mutable state, aliasing, and subtyping, vital components of imperative
...
Since \kafka only supports consumption, if \RuleRef{STGC-TOANY} or \RuleRef
...
In our approach, we reduce each gradual type system into translations,
rewriting uses of \RuleRef{STGC-ANYCONC} and \RuleRef{STGC-TOANY} with casts,
in conjunction with type rewriting. Type rewriting is essential to our
approach, as it enables the expression of the source level soundness
guarantee in terms of \kafka types; these types are then checked dynamically
by the casts inserted by expression translation.
