...
type casts but does so only for the top-level of data structures, and 
adds casts precisely where types are asserted.

Each of these approaches conceals a fundamental difference of opinion on what
it means for a type system to be sound. Concrete systems guarantee all static
types, behavioural systems ensure that values will behave as their types
specify, up to errors, and the transient semantics ensures that calls will go
through but not their value, all while the optional semantics provides no
guarantees whatsoever. This key variation, however, is hidden under a thick
layer of interesting syntax, different types, and complex semantics.
