...
\section{Gradual Source Language and Litmus test}\label{litmustest}
...
is that they have different notions of what is an erroneous program. By
observing the errors that each gradual type system produces, one can
determine what kind of type system is being used. We propose a litmus test
that can distinguish the four gradual type system approaches of optional,
concrete, behavioral, and transient.  The litmus test consists of a set of
programs whose execution depends on which gradual type system is in
use. Each program should run without error when executed without gradual
types, and should be statically well-typed in all the type systems. However,
with gradual types, the programs should induce cast failures depending on
what semantics are in play.

\subsection{A Gradually Typed Language}
...
\IRule{TOANY}{~ }{ \ConvertE\K{s}\t\any}
    
\IRule{ANYCONC}{~}{ \ConvertE\K{s}\any\t }
\end{mathpar}

\vspace{-5mm}  

\hrulefill
\caption{Source language syntax and type system (extract).}\label{slts}
\end{figure}

We present a common source language that will be used with different gradual
type systems in \figref{slts}. The source language is a simple object
calculus without inheritance, method overloading or cast
operations. Notably, the language is statically typed, subtyping is
structural -- namely a class is subtype of another if it has all methods of
the other and the type of the argument and the return values are subtypes.

WIP WIP WIP

We give a source-level type system for each semantics (notated
$\vdash_{\!s}$). To simplify the presentation, we elide unsurprising rules.

The type system is a standard static semantics for gradual typing. To
differentiate this type system from the \kafka type system, we use the
$\EnvTypeS \Env\K\e\t$ judgment form, indicating a well-typed expression in
the surface type system. This surface static semantics allows mixing of
static and dynamically typed terms and is shared between all gradual type
systems. The source language type system allows the implicit conversion of
typed and untyped terms via the convertibility operator, written
$\ConvertE\K{s}{\t}{\tp}$.

This $\ConvertE\K{s}{\t}{\tp}$ relation indicates that type $\t$ is
convertible to type $\tp$, and is used both for traditional up-casting and
for conversions of \any to non-\any types. The type convertibility operator
enforces type discipline where possible (via \RuleRef{SUB}) while allowing
implicit conversion to and from the dynamic type where not (via
\RuleRef{TOANY} and \RuleRef{ANYCONC}). To avoid collapsing the type
hierarchy, the type convertibility operator is not transitive.

Since \kafka only supports subsumption, if \RuleRef{TOANY} or
\RuleRef{ANYCONC} were used, then some dynamic check must be inserted. If
\RuleRef{TOANY} is used, then some typed object is being passed into untyped
code, and must be protected from misuse. Likewise, if \RuleRef{ANYCONC} is
used, then an untyped value is being passed into typed code, and type
...
\RuleRef{TOANY} or \RuleRef {ANYCONC} were needed to conclude the
