...
\noindent Since their inception by Siek and  Taha~\cite{SiekTaha06}, gradual
type systems for a vast array of languages have been developed, enabling
programmers to add types to untyped code gradually. Each of these approaches devotes
great care and attention to getting typing for that language correct,
providing tools to type the constructs of the balky untyped source language.
However, in this process, the fundamental guarantees of gradual typing itself
can be lost.

Nowhere is this problem more apparent than in gradual type systems for 
object-oriented languages. The first effort, by Siek and Taha~\cite{SiekTaha07}
presented a gradual type system for a variant of Abadi and Cardelli's 
object-based calculus~\cite{cardelli:1996:theory-of-objects}, adding a notion of
subtype consistency on top of their earlier work for functional programming
languages~\cite{SiekTaha06}. However, this type system does not support
mutable state, which has became the key problem facing gradually-typed
orbject

Despite a decade of continued research, the practical realization of Siek and Taha's
...
\begin{itemize}
\item TypeScript, annotating Javascript with types.
\item Dart, a gradually typed language for web apps.
\item Hack, adding types to PHP.
\item Thorn, a new language with both typed and untyped code.
\item Nom, another new language supporting both dynamic and static typing.
\item Typed Racket, a rich gradual type system for Racket.
\item Reticulated Python, a family of three different gradual type systems for Python.
\end{itemize}

These languages, spanning the design space, all purport to be gradually typed,
allowing programmers to add just a few types at a time to previously untyped
programs. Formally comparing these approaches is extremely difficult, however,
as each is described with a different formalism, or no formalism at all, a
situation gradual typing only makes more complex. As a result, foundational 
differences in what it means to be gradually typed are easy to overlook among
dozens of other language-specific differences. To compare gradual type systems,
a common language that supports multiple approaches is needed, enabling direct
and easy comparison.
...
\begin{itemize}  
 ...
 \item The \kafka object-oriented calculus for gradual typing, providing a
sound and representative translation target for a gradually typed source
language, and discuss the differences between gradual type systems that drive
language design decisions.
      To validate \kafka, we have both proven the \kafka type system sound
      and implemented the calculus on top of the CLR.

\item A set of four translations, from a common Java-like source language
  to \kafka, each implementing one type of gradual type system, illustrating
  the key differences between them. 

  We validate the source language design and translation semantics through
  several litmus tests, demonstrating equivalent behavior between real programs 
  on the actual language and programs written in our source calculi.
\end{itemize}
...
\kafka is designed as a core calculus to which a common gradually typed source
language can be compiled using different gradual typing semantics.
Fundamentally, \kafka is class-based, object-oriented, and statically typed,
in the vein of Featherweight Java, but is carefully tuned to support multiple
gradual type systems in the same environment.

The desire to support multiple gradual type systems on top of the same
calculus drove many of the design decisions behind \kafka, and illustrates
many of the key differences that underlie each of the different approaches
to gradual typing. At a basic level, \kafka is very similar to .NET Common
Intermediate Language or Java Bytecode, augmented with the dynamic type and
two types of cast, but with several odd aspects that were directly driven
by its use as a target for a gradually typed language.
...
The key requirement behind \kafka's design is that it needs to allow the
optional, behavioral, transient, and concrete gradual typing semantics to all
rely on the type soundness guarantee it provides, or to be able to  express
the types of the gradually typed source language in \kafka's type system.
Moreover, \kafka must support the casts needed by the source syntax.

\kafka's syntax is given in \figref{syn}, which is mostly typical for
class-based calculi, with four notable exceptions:

\begin{itemize}
    \item $\KCall\e\m\ep{\t}{\tp}$, denoting a typed call to a method $\m$ with declared type $\t\rightarrow\tp$, and with argument \ep.
    This form is used to indicate a statically-typed call, for which we provide a soundness guarantee and (if well-typed) is provably
    going to succeed.

    The main oddity that caused our inclusion of the types of the receiver method on the function call was the concrete semantics, which
    needs to expose methods from typed objects to untyped code. In order to support this, a check method needs to be called by the untyped
    code, guarding the typed function's input from potentially ill-typed values. In order to make this call, we needed to do one of two things:
    \begin{itemize}
      \item Define a consistent name translation system, that would take a name of a typed function and convert it into the (free) name of its 
      paired untyped function. 
      \item Allow simple statically-resolved overloading, and determine what function implementation is called statically.
    \end{itemize}

    To avoid the complexity of defining a consistent translation from names to free names, we have taken the latter approach, which
    also lets us provide the correct soundness guarantee for transient calls (e.g. a typed function call to a method from \any
    to \any).
    \item $\DynCall\e\m\ep$, indicating a dynamic call to method $\m$. This is the counterpart to the typed call, calling an untyped
    method and providing no soundness guarantee.

    We use this invocation for untyped code, where no gradual type system provides any static guarantees. Notably, the optional semantics
    converts all method calls to dynamic calls, as it provides no soundness guarantee whatsoever.

    This syntax is somewhat unusual for gradually typed languages - other approaches (such as Reticulated Python~\cite{siek14}) represent
    an unsafe call as a combination of a cast and a safe call. In \kafka notation, if we assume that class $\xt{M}$ has the untyped method
    $\m$, this would be written as $\KCall{((\SubCast{\xt M}\e)}\m\ep\any\any$. However, this demands that for every call site we generate
    the class $\xt{M}$, and obscures the soundness guarantee (or lack thereof) given to untyped invocations, leading to our representation 
    of it as a special case.
    \item $\SubCast\t\e$, which is a normal nominal cast operation.

    The standard cast is common to two of the four calculi, as both the concrete and transient semantics use it. In particular, the use of
    structural subtyping allows us to capture the structural check applied by Transient by simply erasing the types from methods on an object -
    which would already be done to match the soundness guarantee - and casting where needed.
    \item $\BehCast\e\t$, indicating the special cast operation used in the behavioral semantics.

    Originally, we had three different casts, one for each system, but now only the behavioral cast is left. The behavioural cast is far more
    complex than the nominal cast operation, as it needs to wrap objects that pass through it to ensure their compliance to the type specification.
    In particular, since the behavioural semantics does not erase types, the implicit structural cast (similar to the Transient semantics) 
    is difficult to represent using the nominal cast, making the presentation simpler with the behavioral cast as a separate syntactic construct.
\end{itemize}
