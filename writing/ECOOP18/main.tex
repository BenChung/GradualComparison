...
our paper is more concerned with the semantic differences between
...
our desire to represent behavioral and transient approaches that require
...
\item $\K~\e~\s \Red \Kp~\ep~\sp$, where $\WFp\sp\Kp$, $\EnvType\emptyset\sp\Kp\ep\t$, $\sp$ has all of the values of $\s$, and $\Kp$ has all of the classes of \K.
...
follows: %https://github.com/BenChung/GradualComparison/tree/master/impl
...
translated \kafka terms. For the optional semantics, we have eliminated the
type of \this by casting it to \any. The optional semantics make no other
alterations to these terms, as it has no guarantee to ensure.  Transient does
not guarantee the values passed to functions or field assignment respect the
declared type of those expressions. Class translation inserts checks for
invalid variables upon method entry, so while the transient translation has to
insert casts to appease the \kafka type system, these casts will never fail.
However, fields are not protected and requires a guard after dereferencing, as
values can be passed through them to violate their types.
