...
several major holes, most notably in handling mutable state and aliasing,
vital components of imperative object-oriented languages.
...
by language features. The logical solution to this problem is to implement
several gradual type systems for the same language, but no existing language
is suitable for multiple gradual type systems.

This idea of using a single source language then induces a new problem, one
of translation. Most gradual type systems are described by translation to
a bespoke target calculus with the requisite primitives. Even with the same
source, a direct implementation of several gradual type systems will then
induce an equally large range of target languages and semantics.

In this paper, we present a new framework for comparing gradual type systems,
standardizing the source and target language. Using the \kafka core calculus,
designed to provide the minimal set of features needed for gradual typing, we
reduce the comparison of gradual type systems to the comparison of two
different translations between the same source and target. We then use this
approach to model four different gradually typed programming languages,
