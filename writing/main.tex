...
A decade ago Siek and Taha~\cite{SeikTaha07} presented a gradual type system
for a variant of Abadi and Cardelli's object-based
calculus~\cite{cardelli:1996:theory-of-objects}. Their key insight was to
marry the structural subtyping relation used in some object-based languages
with the consistency relation that relates objects that only differ in the
addition of type annotations. The resulting calculus was sound in the sense
that any type error would be caught and cause computation to get stuck.
Soundness was enforced adding casts to objects each time they crossed a
type/untyped boundary.

Obtaining a practical implementation of this calculus has proven elusive.


Short of programming with a proof
...
and, in return, these languages ensure that many errors are ruled out
entirely.
