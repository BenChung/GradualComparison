...
avoid degrading performance.\footnote{Dart, Typescript and Hack use unchecked
modes for production, all type errors will not be reported.}
...
concrete types -- the latter have the same semantics as types in traditional
...
teaching programming to high-school students and undergraduates.  For that
reason, being able to explain the source of errors was viewed as an
important design constraint, another constraint was to prevent surprises --
a variable annotated as a \C should behave as a \C. If it didn't, then that
fact must be reported at the first discrepancy. The Typed Racket approach to
gradual typing is thus quite different from optional types. In Typed Racket,
whenever a value crosses a boundary between typed and untyped code, the
value will be wrapped in a contract that monitors it's behavior. This allows
to ensure that the type of mutable values remains consistent with their
declared type and that functions respect their declared interface. When a
value misbehaves, blame can be assigned to module where the value
originated. In Typed Racket, the granularity of typing is the module, thus
each module is either entirely typed or entirely untyped. This design choice
means that a compilation unit never has to deal with a mixture of typed and
untyped code. Also if closely coupled functions are in the same package,
they will not incur boundary crossing costs.
...
calculus with references and a proof of soundness that stated that all type
errors would be caught. They defined the type consistency relation $\t \sim
\tp$ which states that a type that agree on non-\any position are
compatible.  In their follow up work~\cite{SiekTaha07} they extended their
...
challenging. Reticulated Python~\cite{siek14} attempts to find a reasonable
compromise between soundness and efficiency.  The language has three modes:
in \emph{guarded} mode, Python behaves as Typed Racket with contracts
applied to values, \emph{transient} mode performs first order checks before
each call, and the \emph{monotonic} mode is similar to the guarded mode
except in that there is at most one wrapper per object and casts can only
remove occurrences of \any from an object's type.
...
occur within concretely typed code.  StrongScript takes Thorn and
re-implements it within the context of TypeScript.  The interesting design
choices there are that while TypeScript originally relied on structural
subtyping, StrongScript changed the language to follow a nominal
discipline. It is interesting to note that this change did not affect the
semantics of benchmark applications. Moreover, to deal with higher-order
functions which TypeScript inherits from JavaScript, the language does use
...
\noindent
\figref{over} gives an overview of type systems with publicly available
...
needed~\cite{ecoop15}.  The consistency relation used in Reticulated Python
is fundamentally structural.  For Racket, the heavy use of first-class
...
default execution mode for Dart, Hack and TypeScript.  Dart has a checked
mode that can be turned on to catch some type errors. The Transient variant
of Reticulated Python is, in some senses, optionally typed, any value can
flow into a variable regardless of its type annotation, providing its
ability to provide an ``open world'' soundness guarantee~\cite{siek14}.
C\#, StrongScript and Thorn support both optional and concrete types.  In
Thorn and C\#, when primitives are concretely typed they can be unboxed and
...
evidence from Typed Racket users suggests that context provided by blame
when a run-time type error occurs helps developers pinpoint the provenance
of the ill-typed value or function that caused the error. A fitting analogy
are the stack traces printed by Java when a program terminates
abruptly. Developers working with languages like C++ must run their program
in a debugger to obtain the same information. In Java, stack traces have
little run-time cost because they piggyback on another feature of the
language, namely precise exceptions. Of course, precise exceptions have a
price as they inhibit some compiler optimizations. It is likely that
recording blame is costly as some information must be associated with values
at every cast but there is no data on how much implementations pay for it.
...
to catch simple errors, such as misspelt method names, and assisting IDEs. 
A typical example is shown in the program below. The first call is
...
The Transiennt variant of Reticulated Python~\cite{siek14} aims for
soundness with a predictable cost model. The declared types of arguments is
defensively checked in every method. Consider method \m, it expects an
instance of class \C, but it can be called with a value having type \any.
...
\noindent
Transient Python will deem both call to \code m well-typed. Yet, the second
is going to end up with a type error as a \code D is passed to method
expecting a \code C.  When traslating to \kafka, all types are erased
and casts are inserted.
...
\noindent The translated class \code A has check that the argument to \code
m has all the methods defined by \code C. For this we rely on \kafka's
\emph{shallow structural cast}, written {\ShaStart \tt C \ShaEnd x}, these
cast only check for the presence of method names not for the conformance of
types.
...
{\tt ({\BehStart \any \BehEnd}new B())@n(x)}.  The
...
casts introduces new wrappers. A shallow structural cast is also used to
catch obvious mismatches early.  The key properties of this translation are
...
A drawback of the Typed Racket approach is the impact of performance on fully-typed 
code, especially at heap access sites. The monotonic semantics for Reticulated Python 
aims to prevent this cost in exchange for a new dynamically-enforced guarantee.

To see why there is an impact, consider figure~\ref{fig:mutup_b}. We present two
incompatible types \xt{A} and \xt{B}, alongside a typed and an untyped
memory cell (\D and \C, respectively). If we create a \C, cast it to a \D
(which, initially, is correct), then pass the original \C to the \xt{evil}
function, the reference to \f under type \xt{B} from \xt{d} will be 
invalidated internally, causing the field access to \f on \xt{d} to be
ill-typed without any visible cause.

\begin{figure}[h!]
\begin{tabular}{@{}c@{}c@{}}
\begin{lstlisting}[linewidth=0.45\textwidth]
class A { m( x : A ):A { this }}
class B { n( x : B ):B { this }}
class C { f:* }
class D { f:B }
class Evil {
  evil(x:C):C {x.f(new A())}}
\end{lstlisting}
&
\hspace{0.6cm}
...
\end{lstlisting}
\end{tabular}
\caption{Mutable updates and consistency can break type guarantees}
\label{fig:mutup_b}
\end{figure}
...
This is true for many systems we have discussed. Thorn
statically rejects this program, as the cast does not make sense in its type
sysem. Typed Racket will produce an error when \xt{f} is retrieved, as will
Transient. Both accomplish this with expensive checks at every dereference,
which may impact performance. Moreover, this may seem to be the wrong place to
produce an error, as it was \xt{evil} that mistreated the code, after all.
Since, subjectively, there seem to be fewer sets than gets and it would be
better to error on a set than a get, it would be nice to have the gradual type
system produce an error when \xt{evil} mistreated the reference, rather than 
when the reference is later used.

Realizing this idea is the monotonic semantics, described by Siek et
al~\cite{Siek2015}. Monotonic maintains the property that \emph{every reference
is well typed}, catching the error in figure~\ref{fig:mutup_b} in \xt{evil}, as
the assignment of an \xt{A} into \f is invalidating the earlier reference
(\xt{c}) that asserted that \f would only ever contain \xt{B} references.

The monotonic semantics ensures the type pointing to a
value is always going to be correct up to \any types, as a result, we can now
dereference without any additional checks, allowing (fully statically typed)
dereferences to be as fast as a statically typed language. Moreover, 
the monotonic semantics can execute fully typed
programs as fast as a fully typed language can, unlike the transient approach.

The main design issue is that the monotonic
semantics has a very ``brittle'' approach to typing, any type that touches an
object will forever brand that object to be consistent with that type.
...
\end{lstlisting}
\end{tabular}
\caption{Hidden type assertions can break seemingly-correct code}
\label{fig:invisbreak}
\end{figure}

If the roles of the user code and the \xt{evil} method are switched, as seen in
figure~\ref{fig:invisbreak}, then the brittleness of monotonic is clearly illustrated. From the
perspective of the user, the code that they are writing is perfectly type safe,
as \C can have either \xt{A} or \xt{B} for \f, and \xt{evil} takes a \C.
Therefore, naively, the user would expect this code to execute normally.
However, the same mechanism that caused the earlier version of \xt{evil} to
break will cause the user code to break here, because now it is the user
breaking \xt{evil}'s type assertion, despite the cast reference being unusused.
As a result, adding types to distantly connected parts of a program or, slight
changes in library code, can cause substantial knock-on effects
under the monotonic semantics.
...
The basis of our formal approach is the \kafka core calculus. At heart, \kafka
is a class-based, object-oriented, statically typed, language with dynamic
dispatch, modelled off of common intermediate languages like the JVM's Java
Bytecode or the .NET CLR's Common Intermediate Language, and taking inspiration
from C\#'s approach to gradual typing~\cite{Bierman10}.

\kafka has two types: the any type any, and the class type C (also referred to
with \D and \E). In \kafka, class types \C refer to classes of the same name,
which can contain fields (\HT\f\t, or \fd) and methods, which can be normal
methods (\Mdef\m\x\t\t\e), getters (\Mdefz\f\t\e), or setters (\Mdef\f\x\t\t\e),
and are referred to as \md.

At the expression level, \kafka is largely typical, offering variables (which
are bound through method arguments), new expressions of the form \New\C{\b\e},
and two different types of call, the statically-resolved call \Call\e\m\e and
the dynamic call \DynCall\e\m\e, alongside field dereference \Get\e\f and field
assignment \Set\e\f\e. Additionally, we reserve the \this and \that keywords,
the first for use as a self-reference, and the latter as part of our support for
wrappers.
...
\kafka uses a typical small step operational semantics, with special support for
features to ease the translation of gradual type systems to it. While kafka
itself is fully statically typed, it provides several key features that allow it
to act as a common core for gradual typing translation, including special
support for wrappers and limited function overriding.

Evaluation in \kafka is based around \emph{configurations}, which combine a
class table $\K$, an introductory expression $\e$, and a heap \s, and the
evaluation rule for \kafka's single introduction form, \xt{new}, is typical,
allocating and instantiating a new object of the specified type with the given
values.

\kafka's design as a translation target for gradually typed programming languages
shows the most clearly in its call semantics, which provides support for both
field access and method invocation. In \kafka, direct field accesses and assignments
are indistinguishable from getter and setter invocations, similar to properties 
in C\#. However, this design also means that \emph{all field operations must
be correctly typed}, as otherwise, the type invariants of an underlying field 
would be violated.
...
%the bottom.
%
%The auxiliary function \xt{names} returns the names of methods, and the
%auxiliary function \untyped{} returns the methods whose types are \any. The
%auxiliary function \xt{read} return the location \ap pointing to the field \f of
%the object at location \a. The auxiliary function \xt{write} return the heap \sp
...
The other major feature that \kafka offers relates to the final two calling
forms, \Call\e\m\e, which is a static invocation, and \DynCall\e\m\e, which is a
dynamic invocation. In \kafka, methods can be either typed (e.g. have arguments
and return type which are all not \any), or untyped (both argument and return
type are \any). Typed methods can be invoked with the static invocation
\Call\e\m\e, and untyped methods are invoked with the untyped invocation
\DynCall\e\m\e, providing the ability to define two methods of the same name but
where one is typed and the other is untyped.
...
it asserts that $\e$ has the same \emph{names} as $\t$. The transient
...
order to be well formed in \kafka.  Notably the rule \RuleRef{MTT} in figure~\ref{fig:tratrans_cla}, 
...
The last notable rule in the transient translation is \RuleRef{GRAASC2}. In gradual type systems with consistency, 
