...
and subtyping present serious challenges for the implementation of gradual
type systems for object-oriented languages.  This papers presents an
overview of the state of the art in gradual types for objects. We introduce
\kafka, a class-based object calculus with a static type system, dynamic
method dispatch, transparent wrappers and dynamic class generation. We model
key features of several type systems for dynamic languages by translation to
\kafka and discuss the implication of the respective designs.
\end{abstract} 
...
calculus~\cite{cardelli:1996:theory-of-objects}. Their key insight was to
marry the structural subtyping relation used in that calculus with the
consistency relation that relates program constructs that only differ in the
addition of type annotations. The resulting calculus was sound in the sense
that any type error would be caught, and computation would get stuck at a
cast. As in their previous work~\cite{SiekTaha06} soundness was enforced by
systematically applying casts to objects as they crossed type boundaries.
