...
\IRule{CMS1}{
  ...
  \enfortype{\Cp}{\K} = \Cpp\,\Kp \\
  \enfortype{\D}{\Kp} = \Dp\,\Kpp \\
...
\All m \Mtype\m\any\any\in\bmt &\wedge& \Mtype\m\t\tp \not\in\bmtp
\\[1mm]\hline
\SPP \Mdef\m\x\t\tp{ \MonCast\tp{\Call{\Get\this\that}\m{{\MonCast\t\x}}}}
&
 \All m \Mtype\m\Cp\Cpp \in\bmt &\wedge& \Mtype\m\t\tp\in\bmtp 
 \\[1mm]\hline
...
\IRule[width=30em]{A5}{
    \HasType\E\this\C \\ \m({\t_1}):\t_2 \in \classoff\C\K \\ \lnot\isstatic{\t_1}{\K} \\ \b{\AnaCast\K\Env{\e_2}{\e_4}{\any}}
}{
    \GenCast\K\Env{\Call{\this}\m{{\e_2}}}{\DynCall{(\MonCast\any\this)}\m{{\e_4}}}{\t_2}
}
...
\IRule{ENT}{ 
  ...
  \D\text{ fresh} \\
  \Class\D{\hspace{-0.3em}}{\Fdef\that\Cp ~ \enformt{\b{\md}}} \\ 
  \Kp = \K\,\D
}{
  \enfortype{\C}{\K} = \D\,\Kp
}
...
\IRule{ENMT1}{
  ...
  \mdpp = \Mdef\m\x\t\tp {\MonCast\tp\x} \\
  \enformt{\b\md} = \b\mdpp \\
}{
  \enformt{\md\,\b\md} = \mdpp\,\b\mdpp
}
...
\IRule{ENFG}{
  ...
  \mdpp = \Mdefz\f\t {\MonCast\t{\Get{\Get\this\that}\f}} \\
  \enformt{\b\md} = \b\mdpp \\
}{
  \enformt{\md\,\b\md} = \mdpp\,\b\mdpp
}
...
\IRule{HT1}{
  ...
  \htype\a\S{\sigma[\a \mapsto \C\{\b\ap\}]} = \C
}
...
%% class WA { that : A f():* { <|*|> this.that().f() } f(x:*):* { <|*|> this.that().f(<|*|> x)} }
