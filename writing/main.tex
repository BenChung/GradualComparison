...
\begin{mathpar}
\IRule{WH}{
\Bind\ap{\obj\C{\b\a}}~\in~\s \implies \Class\C{\b\fd}{\b\md}\in\K~\wedge~|\b\a|=|\b\fd|
}{
 \WFtype\K\s
}
\end{mathpar}
...
If $\WFp \K$ (TODO), if $\C \in \K$, $\Cp \in \K$, $\D$ free, $\b\md = \getmds\C\K$, $\bmt = \classoff\C\K$, $\bmtp = \classoff\Cp\K$, and $\k = \wrap\C{\b\md}\bmt\bmtp\D$, then $\StrSub\cdot{\K~\k}\D\Cp$.
  ...
  \begin{proofy}
    ...
    \begin{casel}
      ...
      \begin{proofy}
        ...
        \step Let $\k$ be $\k = \wrap\C{\getmds\C\K}{\classoff\C\K}{\classoff\Cp\K}\D$.
        \step Let $\sp = \s[\ap \mapsto \D\{\a\}]$.
        \step By \RuleRef{W9}, $\EnvType\cdot{\K\k}\sp\ap\D$.
        \step By correctness of $\xt{wrap}$, $\StrSub\cdot{\K\k}\D\Cp$.
        \step By \RuleRef{W2}, $\EnvType\cdot{\K\k}\sp\ap\Cp$.
      \end{proofy}
      \caseof{$\t = \any$}
    \end{casel}
  \end{proofy}
