...
Reticulated Python implements three different semantics, through not simultaneously in the same language. 
A separate translation is, therefore, present for each of the three semantics.

The most notable difference between Reticulated Python and the gradually typed languages is the use of \emph{consistency},
...
In effect, if there is some most specific type for two given types, then the two types are consistent. 
% It is possible that a value of one could be converted into the other with a runtime cast.
...
At the surface language level, the Monotonic semantics are very similar to that used by the guarded semantics and Typed Racket/Racket.
We insert casts at locations at type boundaries.
