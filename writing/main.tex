...
To help bridge this gap, we will use a 
small object calculus and present versions of each of the four approaches
that work with our calculus. The calculus was
...
To illustrate the operation of an example gradual typing system within this framework,
we will show what happens to a program $p$, consisting of a list of classes $c \ldots$ and
an expression $e$:
\begin{itemize}
\item For every $c$, we translate $c$ into $c'$ through the \emph{class translation}
mechanism $\TransClass{c}{c'}$.
\item For the entry point $e$, we translate $e$ to $e'$ through the \emph{cast insertion}
judgment $\Gamma \vdash e \hookrightarrow e'$.
\item The resulting program $p'$ composed of $c' \ldots$ and $e'$ is then well-typed with 
respect to the common core's type system, which then means that the output is can be 
executed without getting stuck.
\end{itemize}
