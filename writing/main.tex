...
Figure~\ref{redu} depicts the source language terms $\K$ and $\e$ (marked
with an underline) being translated into a \kafka program which is then
executed.  The translation guarantees a few key properties. A translated
program should be well-formed in \kafka. Well formed \kafka programs should
execute the \kafka semantics correctly (not get stuck, except at casts and
dynamic calls).  The TypeScript translation produces terms that type check
