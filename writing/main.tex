...
is actually of that type. Strongscript aims to fix this issue.

However, a naive sound implementation would cause problems for developers.
If Strongscript made every type sound, then performance would decrease (TAKIAWA ET AL),
and programs would produce far more errors. As migrating code to Typescript
is already considered difficult, this may have presented an unsurmountable 
barrier to migration. Instead, Strongscript provides programmers with the 
ability to choose if they want a soundness guarantee or not.
...
A notable property about all three gradual typing systems for Reticulated Python is that
they all support the \emph{gradual guarantee}. The details of the gradual guarantee will
be explored later in REFME, but informally it can be stated as ``if it is possible to
fully type a program, than any subset of those type annotations will still work.''
