...
in the object bound to variable \xt{x}. Many static type systems only
provide a partial guarantee as they make an exception for \xt{null} or
\xt{undefined} references; the call may fail as the test whether \xt{x} is
...
employed to enforce it. It it interesting to observe the variety of subtly
different design choices that have been adopted in different languages.  One
of the key difference is what it means for a variable \xt{x} to be of type
...
In this case, we say that \xt{T} is a \emph{concrete} type. In some
proposals a variable \xt{x} of type \xt{T} can refer to an instance of any
class, we say that \xt{T} is an \emph{unconstrained} type. Lastly, \xt{x}
may refer to any value but the language implementation will ensure that the
value behaves as if it was of type \xt{T}, we call this a \emph{promised}
type.
...
\begin{figure}
For every type \t in the program, assign an index $i$ from $1$ through $n$. Then for every set of types $\t_{l_1}$ through $\t_{l_i}$ where $\stcons{\t_{l_1}}{\stcons{\ldots}{\t_{l_i}}}$, take $\t_{l_1,\ldots,l_i} = \meet{\t_{l_1}}{\meet{\ldots}{\t_{l_i}}}$.

	For each $\t_{l_1,\ldots,l_i}$, we generate a class $\Class{\C_{l_1,\ldots,l_i}}{~\b{\md}}$ where $\md = \Mdef\m\x{\b{\tp1}}{\tp2}\e$ 
