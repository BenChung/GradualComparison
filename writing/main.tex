...
inspired by modern language runtimes such as the Java Virtual Machine and the
CLR in .Net. They have a static type system with classes and subtyping but they
also allow for dynamic resolution of method dispatch.\footnote{Both runtimes
...
state (a heap address \a refers to an object, each field \f is reifed into a
...
are inserted at type boundaries.  Two different \emph{structual} casts are
...
%The behavioral cast \BehCast\t\a generates a wrapper object that
%monitors that methods invoked on \a abide by the interface of type \t. The
%monotone cast \MonCast\t\a returns a wrapper monitoring all invocations to the
%object referenced by \a, furthermore this wrapper can only be refined by
...
Moreover, this kind of cast establishes value idenity exactly, allowing us to
...
order to be well formed in \kafka.  The most notablely is the rule
...
untyped modules. To simplify the dicussion, we will refer to the combintion of
...
The third argument of the \xt{tmeet} function, \P, becomes important when we consider the example
