...
$\EnvType\cdot\s\K\ep\t$, then $\Reduce \K{\Call\a\f{\b\ep}}\s \K\epp\s$ where $\EnvType\cdot\s\K\epp\tp$.
...
\begin{itemize}
  ...
  \item \RuleRef{W4}
      ...
      We apply the IH to the first precondition, finding that either $\e$ is a value $\a$, $\e$ is a stuck state, or or $\Reduce\K\e\s\Kp\ep\sp$, $\WFp\Kp{\ep~\sp}$, and $\EnvType\cdot\sp\Kp\ep\C$, then case analyze.
      \begin{itemize}
        \item $\e$ is $\a$: 
      \end{itemize}
  \item \RuleRef{W5}
