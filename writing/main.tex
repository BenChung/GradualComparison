...
To see why the impact exists, consider figure~\ref{fig:mutup_b}. Here, we have
incompatible types \xt{A} and \xt{B}, alongside both a typed and an untyped
...
function, the reference to \f under type \xt{B} from \xt{d} will be invisibly
invalidated internally, causing the final field access to \f on \xt{d} to be
...
This is the key issue for many of the systems we have discussed. Thorn
...
later when we try to use the reference.
...
This guarantee has several interesting concequences, both for design and
implementation. The monotonic semantics ensures that the type pointing to a
value is always going to be correct up to \any types, so as a result, we can now
dereference without any additional checks, allowing (fully statically typed)
dereferences to be as fast as ones in a statically typed language. Moreover, as
a result of this property, the monotonic semantics can execute fully typed
programs as fast as a fully typed language can, unlike the transient approach.

The main design issue is that a language that uses the monotonic
...
If we switch the roles of the user code and the \xt{evil} method, as seen in
figure~\ref{fig:invisbreak}, then this effect is clearly illustrated. From the
...
implementation changes in library code can cause substantial knock-on effects
