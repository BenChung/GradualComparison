...
The auxiliary function \xt{names} return the list of function names in a
class or type, functions that have \any as return values are prefixed with
the symbol \any. The auxiliary function \xt{typeof} return the type of the
object at the location \a in the heap \s. The auxiliary function \xt{read}
return the location \ap pointing to the field \f of the object at location
\a. The auxiliary function \xt{write} return the heap \sp with the field \f
of the object at location \a updated to the location \ap.
...
\CondRule{E10}{  %% Subtypecast
  \BasicType {\K} {\C \Sub \D} \AND \obj\C{\b\a} = \App\s\a
}{ 
...
\begin{equation*}
\mdef\K\C = \Mdef\m\x\t\t\e ~~\mathit{if}~~\Class\C{\b\fd}{\Mdef\m\x\t\t\e\ \b\md} \in\K
\end{equation*}

\begin{equation*}
\mdef\K\C = \Mdef\f\x\t\t\e ~~\mathit{if}~~\Class\C{\b\fd}{\Mdef\f\x\t\t\e\ \b\md} \in\K
\end{equation*}


\begin{equation*}
\mdef\K\C = \Mdefz\f\t\e ~~\mathit{if}~~\Class\C{\b\fd}{\Mdefz\f\t\e\ \b\md} \in\K
\end{equation*}
