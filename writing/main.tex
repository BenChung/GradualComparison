...
Gradual types systems in the real world comes in all different constraints and complexity,
with their own strengths and weakeness.

We use the \kafka syntax for our examples even if it will only be explained later.

The examples presented in this section are written in our 
...
However, Typed Racket is still sound, as the only failures will come from inside
the wrappers. If an object breaks its type, then the wrapper will catch it 
before it reaches the typed code, and therefore, from the perspective of the 
typed code, the object is fully typed.
...
fairly simple, with the only rule that differs substantialyl from the 
optionally-typed case being \RuleRef{TRAASC2}.
