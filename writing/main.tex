...
\usepackage{xspace,listings,url,framed,amssymb,
            ...
            stmaryrd, graphicx, tikz % double brackets llbracket
}
...
\subsubsection{Behavioral Generative Casts}
...
\IRule{}{
  \App\s\a = \obj\C{\b{\a_1}}  \\
  \fresh\D \\
  \k = \wrap\C{\classoff\C\K}{\classoff\t\K}\D\\
  \fresh\ap \\
  \sp = \Map \s{\Bind\ap{\obj\D{\a}}} \\  
}{
  \behcast\a\t\s\K {\k\,\K}\ap\sp
}
\end{mathpar}


At its core, the wrap function protects types. If a function on an object
has a declared argument type, then the wrap function will produce a wrapper
for that function that ensures that any argument passed in matches the
declared type. Likewise, if we assert that an untyped function has a return
type, then the generated wrapper guarantees that the returned value of the
untyped function is of the asserted type by inserting a cast to the right
type.

In our context, wrappers are just generated classes, produced when the
runtime system encounters a behavioral cast. These casts need to ensure two
key properties, which we will refer to as \emph{soundness} and
\emph{completeness}. In the context of casts, soundness refers to correct
enforcement of types, whereby protected methods will not observably violate
their type guarantees, while completeness ensures that a cast will not lose
methods.

This last requirement is somewhat unconventional, and its need is
...
In this example, if we were to implement wrappers by wrapping over only the
declared methods, despite D having a method f, it is "lost" when D is cast
to C. As a result, when we cast it back from C to D, the wrapper added to
ensure that C's invariants held does not have a method f, and our call will
produce a method not understood exception. To avoid this issue, our wrappers
will have to retain all untyped methods when casting untyped to typed, and
typed methods if a typed cast does not mention them, so that they may be
recovered in a later cast.
...
\[\begin{array}{@{}ll@{}l@{}r@{~}r}
    \wrap\C{\b\mt}{\b\mtp}\D = \\
\SP \class ~\D ~ \{\\
\SPP \Fdef\that\C \\

\SPP \Mdefz\f\tp{~\Call{(\BehCast\tp{\this.\that})}\f{}~}
&    \All f \Mtype\f{}\t\in {\b\mt} &\wedge~ \Mtype\f{}\tp \in \b{\mtp}
\\
\SPP \Mdef\f\x\tp\tp {~\Call{(\BehCast\tp{\this.\that})}\f{\BehCast\t\x}~}
&    \All f \Mtype\f\t\t \in {\b\mt} &\wedge~ \Mtype\f\tp\tp \in \b{\mtp}
\\
\SPP \Mdef\m\x{\t_2}{\tp_2} {~\Call{(\BehCast{\tp_2}{\this.\that})}\m{\BehCast\Cp\x}~}
&     \All m \Mtype\m\Cp\Cpp\in{\b\mt} &\wedge~ \Mtype\m{\t_2}{\tp_2} \in\b{\mtp}
\\
\SPP \Mdef\m\x{\t_2}{\tp_2} {~\DynCall{(\SubCast{\any}{\this.\that})}\m{\BehCast\any\x}~}
&     \All m \Mtype\m{\any}{\any} \in {\b\mt} &\wedge~ \Mtype\m{\t_2}{\tp_2} \in \b{\mtp}
\\
\SPP \Mdefz\f\t { ~\Call{\this.\that}\f{}~}
&    \All f \Mtype\f{}\t \in {\b\mt} &\wedge~ \Mtype\f{}\tp \not\in \b{\mtp}
\\
\SPP \Mdef\f\x\t\t { ~\Call{\this.\that}\f\x~}
&    \All f \Mtype\f\t\t \in {\b\mt}&\wedge~ \Mtype\f\tp\tp \not\in \b{\mtp}
\\
\SPP \Mdef\m\x{\Cp}{\Cpp} {~\Call{\this.\that}\m\x~}
&    \All m \Mtype\m{\Cp}{\Cpp} \in {\b\mt} &\wedge~ \Mtype\m{\t_2}{\tp_2} \not\in \b{\mtp}
\\
\SPP \Mdef\m\x\any\any {~\Call{(\SubCast\any{\this.\that})}\m\x~}
&    \All m \Mtype\m\any\any\in{\b\mt} &\wedge~ \Mtype\m\t\tp\not\in\b{\mtp}
\\
\SP \}
\\
\wrap\C\mt\D = \\
\SP \class~\D~\{\\
\SPP \Fdef \that \C\\ 
\SPP   \Mdefz\f\any{\BehCast\any{\this.\Call\that\f{}}} 
&  \All \m  \Mtype\f{}\t \in \mt\\
\SPP   \Mdef\f\x\any\any{\BehCast\any{\this.\Call\that\f{\BehCast\t\x}}} 
&  \All \f  \Mtype\m\t\t \in \mt\\
\SPP   \Mdef\m\x\any\any{\BehCast\any{\this.\Call\that\m{\BehCast\t\x}}} 
...
%% \\
%% \>\>$\Mdefa\n{\b{\HT\x{\t_2}}}{\tp_2} { (\SubCast{\any}{\this})@\m(\SubCast{\any}{\x}) }$\\
%% \>\>\>$\tfa \Mtype\n{\b{\t_1}}{\tp_1} \not\in \classoff\C\K \wedge \Mtype\n{\b{\t_2}}{\tp_2} \in \classoff\Cp\K \wedge \fresh\m $\\
%% \>$\}$
%% \\
%% \end{tabbing}


wrap is more complex than TODO, because different wrappers are needed,
depending on if the target type is a concrete type \C, or is the dynamic
type \any. If the target type is \C, then it will generate the wrapper
methods for the type \C, then insert ``passthrough'' methods. methods of the
same type as the original and that just call the original internally, that
provide the completness property mentioned above. Likewise, if the target
type is \any, then wrap will simply generate a method that casts its argument
to the right type and its return type to \any for every one of the typed
methods in the source, ensuring soundness and completness.


\subsubsection{Monotone Generative Casts}
...
\IRule{CMS1}{
  ...
  \Kp = \K~\classgen{\Cp,\classoff\D\K,\EMxt E,\K} \\
  \sp = \Map\s{\Bind\a{{\EMxt E}\{\ap\}}}
}{
  \spec \a\S\s\K = \sp~\Kp
}
...
\IRule{TM4}{
  \fresh\EC\\
  (\C,\D) \not\in\P \\
  \Pp = \Map\P{\Bind{(\C,\D)}\EC} \\
  \mtypes\C\K = {\b\mt}\\
  \mtypes\D\K = {\b\mtp}\\
  \mmeet{\b\mt}{\b\mtp}\Pp\K = \b\mtpp\,\Kp\\
  \Kpp = \Kp~\classgen{\C,\b\mtpp,\EC,\Kp}\\
}{
    \tmeet\C\D\P\K = \EC\,\Kpp
}
...
\>\>$\that : \C$ \\
\\
\>\>$\Mdefa\m{{\HT\x\any}}\any { \MonCast\any{\MonCast{\tp_2}{\this.\that.\m({\MonCast{\t_1}{\MonCast{\t_2}\x}})}}}$\\
\>\>\>$\tfa \Mtype\m{{\t_1}}{\tp_1} \in \classoff\C\K \wedge \Mtype\m{{\t_2}}{\tp_2} \in \b\mt \wedge \t_1 \neq \any$\\
\\
\>\>$\Mdefa\m{{\HT\x\any}}\any { \MonCast\any{\MonCast{\tp_2}{(\MonCast{\any}{\this.\that})@\m({\MonCast{\t_1}{\MonCast{\t_2}\x}})}}}$\\
\>\>\>$\tfa \Mtype\m{{\t_1}}{\tp_1} \in \classoff\C\K \wedge \Mtype\m{{\t_2}}{\tp_2} \in \b\mt \wedge \t_1 = \any$\\
\\
\>\>$\Mdefa\f{{\HT\x\any}}\any {\MonCast\any{\MonCast{\tp_2}{\this.\that.\f({\MonCast{\t_1}{\MonCast{\t_2}\x}})}}}$\\
\>\>\>$\tfa \Mtype\f{{\t_1}}{\tp_1} \in \classoff\C\K \wedge \Mtype\f{{\t_2}}{\tp_2} \in \b\mt$\\
\\
\>\>$\Mdefa\f{}\any {\MonCast{\any}{\MonCast{\tp_2}{\this.\that.\f()}}}$\\
\>\>\>$\tfa \Mtype\f{}{\tp_1} \in \classoff\C\K \wedge \Mtype\f{}{\tp_2} \in \b\mt$\\
\\
\>\>$\Mdefa\n{\b{\HT\x\any}}\any { {\this.\that.\n(\b{\MonCast\any{\MonCast{\t_1}\x}})}}$\\
\>\>\>$\tfa \Mtype\n{\b{\t_1}}{\tp_1} \in \classoff\C\K \wedge \Mtype\n{\b{\t_2}}{\tp_2} \not\in \b\mt \wedge (\n = \f \vee \t_1 \neq \any)$\\
\\
\>\>$\Mdefa\m{\b{\HT\x\any}}\any { {(\MonCast{\any}{\this.\that})@\m(\b{\MonCast\any{\MonCast{\t_1}\x}})}}$\\
\>\>\>$\tfa \Mtype\m{\b{\t_1}}{\tp_1} \in \classoff\C\K \wedge \Mtype\m{\b{\t_2}}{\tp_2} \not\in \b\mt \wedge \t_1 = \any$\\
\\
\>\>$\Mdefa\m{{\HT\x{\t_2}}}{\tp_2} { \MonCast{\tp_2}{\this.\that.\m({\MonCast{\t_1}\x})}}$\\
\>\>\>$\tfa \Mtype\m{{\t_1}}{\tp_1} \in \classoff\C\K \wedge \Mtype\m{{\t_2}}{\tp_2} \in \b{\mt} ~$\=$\wedge~ \statictype{\t_2}\K\cdot $\\
\>\>\>\>$\wedge~ \statictype{\tp_2}\K\cdot$
\\
\>\>$\Mdefa\m{{\HT\x{\t_1}}}{\tp_1} { {\this.\that.\m(\x)}}$\\
\>\>\>$\tfa \Mtype\m{{\t_1}}{\tp_1} \in \classoff\C\K \wedge \Mtype\m{{\t_2}}{\tp_2} \not\in \b{\mt} ~$\=$\wedge~ \statictype{\t_1}\K\cdot $\\
\>\>\>\>$\wedge~ \statictype{\tp_1}\K\cdot$\\
\>$\}$
\end{tabbing}
...
\IRule[width=30em]{A2}{
    \GenCast{\K}{\E}{\e_1}{\e_3}{\C} \\ \m(\b{\t_1}):\t_2 \in \classoff\C\K \\ \b{\AnaCast{\K}{\E}{\e_2}{\e_4}{\t_1}}
}{
    \GenCast{\K}{\E}{\Call{\e_1}\m{\b{\e_2}}}{\ShaCast{\t_2}{\Call{\e_3}\m{\b{\e_4}}}}     {\t_2}
}
...
\IRule[width=30em]{A2}{
    \GenCast{\K}{\E}{\e_1}{\e_3}{\C} \\ \n(\b{\t_1}):\t_2 \in \classoff\C\K \\ \isstatic{\t_1}{\K} \\ \b{\AnaCast{\K}{\E}{\e_2}{\e_4}{\t_1}}
}{
    \GenCast{\K}{\E}{\Call{\e_1}\n{\b{\e_2}}}{\Call{\e_3}\n{\b{\e_4}}}{\t_2}
}
...
\IRule[width=30em]{A3}{
    \GenCast{\K}{\E}{\e_1}{\e_3}{\C} \\ \n(\b{\t_1}):\t_2 \in \classoff\C\K \\ \lnot\isstatic{\t_1}{\K} \\ \b{\AnaCast{\K}{\E}{\e_2}{\e_4}{\any}}
}{
...
\IRule{A11}{
  ...
  \Class \C {\b{\Ftype\f\t}} {\b{\md}} \\
  \D~\text{fresh} \\
  \D = \classgen{\C,\classoff\C\K,\D,\K} \\
  }{\GenCast{\K}{\E}{\New\C{\b{\e_1}}}{\New\D{\New\C{\b{\e_2}}}}{\C}}
\end{mathpar}
...
\IRule{FT1}{
 \App\s\a=\obj\D{\ap} \\ % we know that a refers to a wrapper with a that field of the wrapped object
  \App\s\ap =\obj\EC{\b\app}  \\ % getting the fields out of ap
 \Class \EC {\b{\Fdef\f\t}}{\b\md} \in\K \\
 \wftype{\b\f}\C\K =\b\tp
}{
...
\item $\MonCast\tp\ep $
\end{itemize}
We proceed with rule induction on the judgement used to conclude $\EnvType\E\s\K\e\t$. Note that we refer to rule preconditions from left to right.
\begin{itemize}
  \item W1
