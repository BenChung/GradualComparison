...
\newcommand{\bind}[4]{\M{\xt{bind}(#1, #2, #3,#4)}}
\[ 
\bind{\sp1}{\ap1}{\Obj{\b\a}\t\C}\s \equiv
  \s=   \Heap{\sp1}{\Bind{\ap1}{\Obj{\b\a}\t\C\}}}
\]

\newcommand{\dispatch}[5]{\M{\xt{dispatch}(#1,#2,#3,#4,#5)}}

\[
  \dispatch\s{\a}\m{\b\x}\e \equiv   \Mdef\m\x{\tp 1}{\tp 2}\e\inc \classof{\Sel\s\a}
\]
\newcommand{\readfield}[4]{\M{\xt{read}(#1,#2,#3,#4)}}
\[
 \readfield\s\a\f{\ap1}
     \equiv \Fdef\f\t{\ap 1}\inc \Sel\s\a
\]
\newcommand{\setfield}[5]{\M{\xt{write}(#1,#2,#3,#4,#5)}}
\[
 \setfield\s\a\f{\ap1}{\sp1}
     \equiv \sp1=\Fdef\f\t{\ap 1}\inc \Sel\s\a
\]
...
\CondRule{E2}{ %% new C -> a
 \bind{\sp1}{\ap1}{\Obj{\b\a}\C\C}{\sp2}
}{ 
    \ReduceA{ \New\C{\b\a} }{\sp1}{\ap1}{\sp 2}
}
\CondRule{E3}{ %% a.m -> e
  \dispatch\s{\a}\m{\b\x}\e
}{
   \ReduceA{\Call\a\m{\b{\ap 1}}}\s{[\a/\this~\b{{\ap 1}/\x}]\e}\s
}

\CondRule{E3}{
 \xt{not}   \dispatch\s{\a}\m{\b\x}\e
}{
...
\IRule{T3}{
  \TransExp{\E}{\ep1}{\ep3}{\tp1} \\
  \Mtype\m{\b{\tp2}} \in \tp1\\
  \b{\TransExp{\E}{\ep2}{\ep4}{\tp1}}
}{
  \TransExp{\E}{\Call{\ep1}\m{\b{\ep2}}}{\Call{\ep3}\m{\b{\ep4}}}{\t}
}

\IRule{T4}{
  \b{\TransExp\E{\ep1}{\ep2}\t} \\ 
  \Class \C {\b{\Ftype\f\t}} {\b{\md}}
}{
  \TransExp\E{\New\C{\b{\ep1}}}{\New\C{\b{\ep2}}}\C
}

\IRule{T5}{
  \TransExp\E{\ep1}{\ep2}{\tp1}
}{
   \TransExp\E{\Cast\t{\ep1}}{\Cast\t{\ep2}}\t
}


\IRule{T6}{
  \TransExp\E{\ep1}{\ep2}{\tp2}\\
  \tp2 \Sub \tp1\\
}{
   \TransExp\E{\ep1}{\ep2}{\tp1}
}
...
code. We can solve this by havin gh the compilation process insert the types
for the programmer, which we describe using a cast insertion system.

Our cast insertion approach is based on a birdirectional type
