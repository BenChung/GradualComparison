...
inserted on method entry and before return. Thus we get the following:
...
conformance of types. The return type is similarly checked.
...
\noindent Class \code B expects the argument to method \code n to be of type
\code B, yet it will be given an \code A.  Typed Racket mediates at
boundaries between typed and untyped code by inserting wrappers around the
values being exchanged. These wrappers ensure that values behave according
to their \emph{promised} types. The translation to \kafka is such that typed
code remains untouched, while untyped code is extended with \emph{behavioral
  generative casts} at creation of instances of typed classes. Thus the body
of \code m becomes {\tt ({\BehStart \any \BehEnd}new B())@n(x)}.  The
instance of class \code B is cast to type \any, and the call becomes a
dynamically resolved call. The behavioral cast creates a new wrapper class
...
resolution) but calls can still fail at casts within the wrapper.

\subsection{Monotonic Python}

A drawback of the Typed Racket approach is the impact of performance on
fully-typed code, especially at heap access sites. The monotonic semantics
for Reticulated Python aims to prevent this cost in exchange for a new
dynamically-enforced guarantee.
