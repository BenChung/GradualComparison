...
code is ensured by inserting casts as shown in their previous work 
targeting functional languages~\cite{SiekTaha06}.  Ten years later,
...
surprisingly elusive.  One possible reason being that the original paper did not
...
also opportunities for improving on existing techniques.  This paper also aims to
lay the agenda for future investigations.
...
and allow for comparisons. But which language should we target? The common 
language should be simple to avoid linguistic clutter, while the key
...
.Net environment. They each have
...
resolved method dispatch.\footnote{Both languages can invoke methods
  ...
  for custom method dispatch.} As well as reasonably efficient implementations,
and in both environments, primitive types can be
unboxed as the static typing an accurately final types.

One contribution of this work is the design of \kafka, a statically
typed object calculus.  \kafka is class based (with an explicit class table
\K), with mutable state (a heap address \a refers to an object, each field \f
...
explicit casts at type boundaries.  Four different casts are built-in \kafka, 
two of these are called structural because they only inspect the
structure of the objects, while the other two are called generative as they create
new wrapper classes.  The subtype cast \SubCast\t\a is a structural cast that checks if
...
\t. The monotone cast \MonCast\t\a returns a wrapper monitoring all
invocations to the object referenced by \a, furthermore this wrapper can only
be refined by removing occurences of \any in its type.

\renewcommand{\u}[1]{\underline{#1}\xspace}

Another contribution of the work is the translation of the gradual type
...
involved proofs), the soundness of the gradual type system is more
...
wrapper has a target object, and the wrapper's role is to check if the target
...
following the design of C\# 4.0. The dynamic type of \kafka is denoted by \any.
...
(i.e. call where the type of the receiver is \any) are denoted by \DynCall\x\m\e.
...
methods or field names. \this is a distinguished variable name denoting 
the current object, and \that is a distinguished field name denoting the 
target of a wrapper class. \D is a meta-variable used to range over dynamically 
...
\kafka's semantics are a typical small step operational semantics, though,
in order to best accomodate the various systems, we introduce some unusual
features. A \kafka program is defined by the combination of a class table 
\K, which consists of a list of classes \k, and a evaluation expression \e.

\kafka classes can contain both methods and fields, the latter of which
can be declared either via an explicit field declaration in the class
(e.g. \HT\f\t) or by a pair of a getter (\Get\this\f) and setter 
(\Set\this\f\x). Semantically, both explicit and implicit fields behave 
identically, which has the concequence that untyped field access is forbidden, 
as explicit field semantics would then become dependent on the approach to 
typing used.
...
To execute a \kafka program, the system will use a heap (which we denote
\s) that maps memory locations \a to objects, each of which is a combination
of a list of field values \a and the class name \C, with notation
\Map\s{\Bind\a{\obj\C{\b\a}}}. When a program \K\e and heap \s are combined,
we call it a configuration, and one configuration \K\e\s reducing to another
configuration \Kp\ep\sp is denoted \Reduce\K\e\s\Kp\ep\sp.

We define our reduction semantics, seen in figure~\ref{fig:semantics}

The execution can halt either if it gets stuck, if it encounters an expression
that is not a value and that no evaluation rule applies to, or if the expression
is a value. \footnote{Intuitively we would
...
To illustrate the use of the first cast, using subtyping, we will examine the 
translation from a subset of the Thorn language to \kafka. 

\paragraph{Translation: Thorn}
...
generates no runtime casts and therefore has lower overhead.
...
&&&\multicolumn{3}{l}{\wedge~\rtranst{\bmt}\bmtp{(\BehCast\Cp\x)}\x{\e}{\ep}}
\\
...
&&&\multicolumn{3}{l}{\wedge~\rtranst{\bmt}\bmtp{(\BehCast\any\x)}\x\e{\ep}}
\\
...
\paragraph{Translation: Typed Racket}
...
The Type Racket source level translation allow programmers to write partially typed classes, in order to 
highlight the semantic of each behavioural approache independent of the surface language.

At the top level classes, denoted with an underscore, are translated to \kafka classes. The 
main expression of the source language is also translated to an expression in \kafka, as shown in PT.
The translation for methods is shown in MT, where it simply translates the expression inside the method body.
...
The remaining translations is relatively straightforward. The translation within our 
bidirectional system, including the synthetic and analytic cases, proceeds in the typical manner.
...
The TRAASC2 rule describes when the Typed Racket translation encounters a casting type that's not a subtype
of the original type. A behavioural and name cast will be inserted around the potentially ill-typed target.
This is because Typed Racket only does runtime checking on the nature of the values that are passed in from Racket,
and therefore, even nonsensical types can be asserted for untyped code. TRAASC2 accounts for this by aggressive wrapping, 
where a type can be of any other type.

Lastly, TRAASC2 will insert a name cast as the behavioural cast is very weak, passing virtually 
every object through it in a wrapper, even if the object underneath diverges from the type asserted by it. A classic example
being missing methods. To compensate for this, we also add the name cast, which ensures that all of the required methods are present.

\paragraph{Translation: Reticulated Python - Transient}
...
\paragraph{Translation: Reticulated Python - Monotonic}
