...
explicit casts at type boundaries.  Two different structual casts are built-in
to \kafka, these are called structural because they only inspect the
structure of the objects. \kafka also supports two additional casts
called generative as they create new wrapper classes.  
The subtype cast \SubCast\t\a is a structural cast that checks if
...
their signatures. The behavioral cast \BehCast\t\a generates a wrapper object
...
\hline
\multicolumn{3}{|l|}{Statics} & \multicolumn{3}{l|}{Dynamics}\\
\hline
$\K~\e$ & $\progtrans{}{}$ & $\Kp~\ep$ & 
$\Kp~\ep~\cdot$ & $\longrightarrow^*$ & $\Kpp~\a~\s$ \\
\hline
\multicolumn{3}{|l|}{Guarantees} & \multicolumn{3}{l|}{Guarantees}\\
&& $\WFp\Kp\ep$ & 
