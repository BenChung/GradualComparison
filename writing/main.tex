...
Our basic type system is typical for calculi that support objects, notably including 
...
and Amber-rule style support for recursive types. Notably, however, this system is not
a \emph{gradual} type system, as it has no support for performing operations other than
cast on any value of type $\any$.

%TODO: Explain what's up with sigma

To define a gradual type system based on this core type system, we use
cast insertion to translate gradually typed code, where we can perform
actions on values of type $\any$, into terms that typecheck against this
stricter concrete semantics. 

Soundness for this base system is typical, though with a carve-out for improper casts
to fail:
...
A well typed program in this calculus can only go wrong at cast sites, and there it has
a well defined error value - the system will never get stuck. We will go on to use this 
property to show soundness in the gradual typing systems in question.
\begin{figure}
...
types to an aforementioned underlying fully typed language. In this vein,
...
systems. The fundamental building block is judgments of the form $\E \vdash \e : \t$, 
which means that $\e$ inherently has type $\t$. For example, a type mismatch
would look like trying to conclude a judgment $\E \vdash \e : \any$ when only
...
mechanism. In a bidirectional system, we have two judgments:

\begin{itemize}
\item The \emph{analytic} judgment $\E \vdash \e \Rightarrow \t$, which says that
$\e$ inherently has type $\t$ against environment $\E$, equivalently to the
traditional bottom-up type system.
\item The \emph{synthetic} judgment $\E \vdash \e \Leftarrow \t$, which implies that
...
this can require the introduction of a non algorithmic rule to conclude this final judgment. Instead, in a 
...
synthetic judgment as $\tp2$ is the type that the expression can be known to have in the absence of any
external information.

To support cast insertion, we extend these basic type checking judgments with an output term, of the form 
$\GenCast{\E}{\ep1}{\ep2}{\t}$ for the synthetic case and $\AnaCast{\E}{\ep1}{\ep2}{\t}$ for the analytic,
case, where the former translates $\ep1$ to $\ep2$ producing type $\t$, and the latter translates the same
ensuring type $\t$.

An important note in the construction of our bidirectional type system is that our handling of the key case,
function application, is less specified than it could be. Other works in this area, including (DUNFIELD \& KRISHNASWAMI CITEME)
use a third bidirectional judgment to handle function application, largely to handle the analytic case. If
we are trying to typecheck a function call against some known type, then we can infer a type for the receiver
by using synthetic type checking on the arguments. However, in our context, detailed handling of this case
is not required, as our type inference does not have strict specificity requirements. 
...
setter methods in a phase we call \emph{class translation}, occurring at the same time as cast insertion.
...
arity and a related name, but only has arguments of type $\any$.

\subsection{Soundness}

This gradual type system sits on top of the ``base'' type system, which is in turn sound. To show that
the gradual type system is sound, we need to show that the terms produced by the translation steps are
well typed, which then can be used with the soundness proof of the underlying base type system to show
soundness of the system as a whole.

Based on this reasoning, the following correctness theorem for translation is sufficient to demonstrate
the soundness of the system.

\begin{thm}
If $\GenCast{\E}{\ep1}{\ep2}{\t}$ or $\AnaCast{\E}{\ep1}{\ep2}{\t}$ then $\EnvType{\E~\emptyset}{\ep2}{\t}$
\end{thm}

We can then use this correctness theorem to prove the overarching soundness theorem,

\begin{thm}
If $\GenCast{\E}{\ep1}{\ep2}{\t}$ or $\AnaCast{\E}{\ep1}{\ep2}{\t}$ then one of the the following holds
\begin{itemize}
\item $\ep2 \rightarrow \ep2'$ and $\EnvType\Es{\ep2'}\t$
\item $\ep2 \rightarrow \xt{v}$ and $\EnvType\Es{\xt{v}}\t$
\item $\ep2$ is $\Cast{\t}{\ep2'}$ and $\ep2 \rightarrow \err$
\end{itemize}
\end{thm}
...
\IRule{MUTWrap}{
  ...
  \utw{\Mtype{\m}{\b{\tp1}}\t} = \Mdef{\m_\xt{u}}\x{\any}{\any}{\Cast{\any}{\this.\xt{s}.\m(\b{\Cast{\tp1}{\x}})}}
}
...
Consider any type $\t$ in the program, then examine some class $\C$ such that $\vdash \C : \t'$ where $\meet{\tp1}{\tp2}= \t''$.
\begin{mathpar}
\IRule{}{
\Mtype\m{\b{\tp3}}{\tp4} \in \t
}{
\adapt{\Mdef\m\x{\tp1}{\tp2}\e, \t} \equiv \\ \Mdef \m\x\any{\any}{\Cast{\any}{\xt{this}.\m'(\b{\Cast{\meet{\tp1}{\tp3}}{x}})}},\\
