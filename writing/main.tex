...
\newcommand{\Reduce}[5]{\M{ #1~#2 \rightarrow_#3 #4~#5}}
\newcommand{\ReduceA}[5]{\M{ #1 ~ #2 } &  \M { \rightarrow_#3 #4 ~ #5}}
\newcommand{\inc}{\M{\in}}
...
\begin{minipage}{8cm}
\opdef{\Reduce{\ep 1}{\sp 1}c{\ep 2}{\sp 2}}{\ep1 \sp1 evaluates to {\ep2} {\sp2} or \err in a step}\\[-1mm]
\begin{tabular}{@{}l@{}l@{~}l@{~}l}
\CondRule{E1}{ %% e -> e'
  \Reduce {\ep 1}{\sp 1}{c}{\ep 2}{\sp 2}
}{
  \ReduceA {\Ctx{\ep1}}{\sp 1}c{\Ctx{\ep2}}{\sp 2}
}
\CondRule{E2}{ %% new C -> a
   \alloc{\sp2}{\ap1}{\sp1}{\Obj{\b\a}\C}
}{ 
    \ReduceA{ \New\C{\b\a} }{\sp1}{c}{\ap1}{\sp 2}
}
\CondRule{E3}{ %% a.m -> e
   \dispatch{\b\x}\e\s\a\m
}{
   \ReduceA{\Call\a\m{\b{\ap 1}}}\s{c}{[\a/\this~\b{{\ap 1}/\x}]\e}\s
}

\CondRule{E3}{
   \notdispatch{\b\x}\e\s\a\m
}{
   \ReduceA{\Call\a\m{\b{\ap 1}}}\s{c}{\err}{}
}
\CondRule{E3}{ 
     \readfield{\ap1}\s\a\f
}{
  \ReduceA{\Get\a\f}{\s}{c}{\ap 1}{\s}
}
\CondRule{E4}{
     \setfield{\sp2}{\sp1}\a\f{\ap1}
}{
     \ReduceA{\Set\a\f{\ap 1}}{\sp 1}{c}{\ap 1}{\sp 2}
}
\NoCondRule{E5}
{ 
   \ReduceA{ \Cast\any\a}\s{c} \a\s
}
\CondRule{E6}{
  \typeof\s\a \Sub \tp 1
}{ 
    \ReduceA{\Cast{\tp 1}\a}\s{c}\a\s
}
\CondRule{E7}{
  \typeof\s\a \NotSub \tp 1
}{ 
    \ReduceA{\Cast{\tp 1}\a}\s{c}\err{}
}
\CondRule{E8}{
    \Reduce\e{\sp 1}c\err{}
}{
    \ReduceA{\Ctx\e}{\sp1}{c}\err{}
}
\end{tabular}\end{minipage}
%%%%%%%%%%%%%%%%%%% CONTEXTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\[3mm]
...
At this point, we have defined a complete (allbeit small) statically typed 
language, with everything that is required to check and evaluate it, which 
required a typing judgment, a subtyping relationship, and a dynamics. We can
then describe a statically typed language as a triple of $(\vdash,<:,\rightarrow_c)$,
fully specifying the semantics of the language.
...
\opdef{$\ESub{\xt{H}}{\tp1}{\tp2}$}{\tp 1 is a subtype of \tp 2 against environment $\xt{H}$}
\begin{mathpar}
\IRule{L1}{\ESub\emptyset{\tp1}{\tp2}}{\tp1 \Sub \tp2}

\IRule{S1}{}{\ESub{\xt{H}}\t\t}

\IRule{S2}{}{\ESub{\xt{H}}\t{\Type{}}}

\IRule{S3}{\vdash \C : \tp2 \\ \ESub{\xt{H}}{\tp2}{\tp1}}{\ESub\xt{H}\C{\tp1}}

\IRule{S4}{\vdash \C : \tp2 \\ \ESub{\xt{H}}{\tp1}{\tp2}}{\ESub\xt{H}{\tp1}\C}
...
\IRule{S6}{
    \xt{H}' = \xt{H},~{\t \leq \Type{\Mtype\m{\b{\tp1}}{\tp2} ~ \b{\mt} \,}} \\
    \ESub{\xt{H}'}\t{\Type{\b{ \mt } }}\\
     \Mtype\m{\b{\tp3}}{\tp4} \inc \t \\
    \b{\ESub{\xt{H}'}{\tp3}{\tp1}} \\
    \ESub{\xt{H}'}{\tp2}{\tp4} \\    
}{
   \ESub{\xt{H}}\t{\Type{\Mtype\m{\b{\tp1}}{\tp2} ~ \b{\mt} \,} } 
}
\end{mathpar}
\caption{Subtyping}
\label{fig:sub}
...
\begin{figure}
\opdef{\TransClass{\C_1}{\C_2}}{$\C_1$ translates to $\C_2$}
\begin{mathpar}
\IRule{CT1}{
...
\newcommand{\tv}{\xt{v}}
\begin{figure}
\opdef{\Reduce{\ep 1}{\sp 1}m{\ep 2}{\sp 2}}{\ep 1 with heap \sp 1 evaluates to \ep 2 and \sp 2 in a step}
...
\IRule{MS2}{
	...
	\castfn{\ap1}{\tp1}{\s} \equiv \s\\
}

\IRule{MSE}{
	\sp2 = \castfn{\ap1}{\tp1}{\sp1}
}{
	\Reduce{\Cast{\tp 1}{\ap1}}{\sp1}m{\ap1}{\sp2}
}
\end{mathpar}
