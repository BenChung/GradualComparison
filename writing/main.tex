...
this is not an essential part of the design, it could be replaced with a
traditional nominal system with multiple subtyping as in Java or C\#.  The
last noteworthy feature is that to support transparent wrappers, \kafka has
a distinguished \that field which does not appear in a class' type.
...
Another contribution of this work is the translation from gradual type
system into \kafka. Each expression \HT{\u\e}{\u\T} in the gradually typed
language is translated to the \kafka term \HT\e\T, for each gradual type
\u\T there is a corresponding type \T in \kafka, and, similarly, each
expression \u\e has an equivalent \e. While \kafka has simple notion of
soundness, soundness of gradual types is a more interesting story. At the
\kafka level, a well type program can get stuck at a cast or at a
dynamically resolved call. For a gradually typed system, an expression of
the form \Call{\u\x}\m{\u{\x'}} where \u\x is declared to belong to some
class \u\C can have significantly different behaviors. In optional type
systems, e.g.  TypeScript, the type \u\C translates to \any, the actual
value denoted by \u\x can be an object of any class. In this case, we say
that \u\C is an \emph{unconstrained} type. The TypeScript compiler can still
catch static errors when \u\x is used in a way that is inconsistent with
\u\C.  In type systems that support \emph{concrete types}, such as C\# and
StrongScript, \u\C maps directly to \C and the object denoted by \u\x must
of the class \C or a subtype. Lastly, in sound gradual type systems like
that of Typed Racket, \u\C is a \emph{promised type}, that is \u\x can
either refer to an instance of class \C or to a wrapper over an object that
has all the methods of \C but may, or may not, behave as \C. In case it
doesn't the computation will get stuck.
