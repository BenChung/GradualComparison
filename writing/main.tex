...
\begin{mathpar}
\IRule{WH}{
\forall~\Bind\ap{\obj\C{\b\a}}~\in~\s~.~\Class\C{\b\fd}{\b\md}\in\K~\wedge~|\b\fd|=|\b\a|
}{
 \WFtype\K\s
}
\end{mathpar}
...
The function \xt{dynamic}(\t,\,\K) denotes every method in type \t contains only \any. 
...
If $\WFp\K{\e~\s}$, then $\forall~\Class\C{\b{\Fdef\f\t}}{\b\md}~.~\WFtype\K{\Fdef\f\t}~\wedge~\WF\cdot\s\K\md$
...
\begin{itemize} 
\item $\a$
\item $\EE[\DynCall\ep\m{\epp}]$
\item $\EE[\SubCast\tp\ep]$
\item $\EE[\ShaCast\tp\ep]$
\end{itemize}
  ...
  \item \RuleRef{W4}
      ...
      \begin{itemize}
        ...
        \begin{itemize}
          ...
          \item $\e'$ is stuck at $\EE[\DynCall\ep\m{\epp}]$, $\EE[\SubCast\tp\ep]$, or $\EE[\ShaCast\tp\ep]$. Apply the definition of $\EE$ to find that the term as a whole is stuck.
        \end{itemize}
        \item If $\e$ is stuck at one of the three stuck states, then we can bubble the stuck state up by definition of $\EE$, and the theorem holds.
      \end{itemize}
  \item \RuleRef{W5}
