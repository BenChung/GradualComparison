...
\inferrule*[lab=EInvError]{v = n \vee (v = a \wedge \s[a \is \{\ldots | \C\}] \wedge \m \not\in meths(\C))}{ v.\m(\bar{v'}),\s \ereduce \error,\s'}
\end{mathpar}
...
\end{mathpar}
We remove rule TCallAny

\opdef{$\Gdash e \tlate e' :\t$}{$e$ translates to $e'$ producing type $\t$ in context $\G$}
\begin{mathpar}
\inferrule*[lab=CICallAny]{ 
  \Gdash e \tlate e'':\any \\  
  \bar{\Gdash e' \tlate e''':\t'} \\ 
}{
  \Gdash e.\m(\bar{e'}) \tlate e''.\m_u(\bar{e'''}) : \any
}
...
\inferrule*[lab=CICallAny]{ 
  ...
  \Gdash e.\m(\bar{e'}) \tlate (\cast{\xt{Wm}}e'').\m(\bar{e'''}) : \any
}
\end{mathpar}
