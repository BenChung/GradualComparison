...
\wc is more complex than TODO, because different wrappers are needed,
...
type is \any, then \wc will simply generate a method that casts its argument
...
The monotone cast \MonCast\t\a imposes type \t on the object referenced by
\a and, transitively, on other objects reachable from it. In the extreme,
one cast could cause all objects in the heap to be retyped. To prevent
pathological cases, monotone casts, only modify the heap if the target type
\t has fewer occurences of \any than the original type of the object at \a
(and transitively so for types appearing in \t and reachable from \a).  When
a type \t does not have any occurence of the dynamic type, denoted by
\statictype\t\K\V, monotone casts will leave the object and the heap
unchanged.

The monotone cast is generative, it triggers the creation of new types that
are the meet of the existing types and the requested type. Generativity is
required when objects are aliased at different types.  This cast also
...
The semantics of monotone casts is somewhat involved. Key the definitions
are shown here, the remainder is in appendix. The key challenge is that
after a monotone cast in a heap \s, every reference should still be valid.
If a reference has a type other than \any, then the cast will preserve
correctness of that reference. To accomplish this, the cast ensure two
properties hold:
\begin{itemize}
