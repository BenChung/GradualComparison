...
untyped methods, a dynamic call is surrounded by casts to \any, ensuring that
typed object's type requirements will be respected.
...
In our approach, we implement the wrapper generation as a metafunction, called
\xt{wrap}. In the formal system, \xt{wrap} uses 5 arguments, the source class
name, the list of methods in the source class, the list of method types the
source  class has, the list of method types that the wrapper is guaranteeing,
and the target class name, but for the purposes of clarity, we will use a class
...
A simple example of this can be seen in figure~\ref{fig:rktex1}. Here, we have 
an initial class \C, which we want to make act like a \D. \C has two declarations,
a field \f and a method \m, both of which are untyped. The protections around 
the field \f are typical, using \kafka's abstraction over fields, allowing both
actual fields and getter/setter pairs to be treated identically, but the generated
proxy for \m is unusual.
...
Figure~\ref{fig:rktex2} would be a more usual version of wrapper generation, 
where we forward the call to \m to the wrapped object, as the semantics would 
normally imply. Here, nothing bad happens, as the wrapped object is safely 
isolated from the typed caller with casts, and no anomalous behaviour is seen.
...
wrapper~\cite{Takikawa:2012}, as seen in figure~\ref{fig:arktex3}. Here, the
(untyped) \xt{A\%} object implements the bar method by calling \xt{foo}, and,
because the externally applied type \xt{A} ascribes  the return type of \xt{foo}
to be \xt{Integer}, when the untyped method \xt{bar} calls the untyped method
\xt{foo} and foo returns a non- integer, we see a contract violation.
...
To model this in \kafka, we need to use a different approach to translation. At
the most basic level, if we use the same naive translation approach seen in
figure~\ref{fig:rktex2}, we would end up calling into the wrapped object, which
would then call the \xt{foo} method without any type guards, and the program
would only encounter a cast error once \xt{bar} returned.

Illustrating this issue is figure~\ref{fig:intbeh}. Here, we have a class \C
that we force to become another class \D, which is typed in such a way that \C's
implementation of \n will violate the type guarantees imposed by \D and enforced
by the wrapper, \xt{DW}. Under the naive translation scheme, illustrated in the
figure, the arguments to \m will be cast to \any, then passed onwards into the
untyped object. Then, the untyped object \C will simply use its own definition
...
reference in the underlying object trivially refer to the wrapper, a translation
we illustrate in figure~\ref{fig:intbeh2}. However, as noted there, this
translation is still type incorrect - we are trying to make an untyped call to a
typed call site.
...
figure~\ref{behtrans}, which ensures the type correctness of wrapped method
bodies once lifted into the wrapper. This, finally, defines a behavioural cast
semantics that preserves the execution behaviour of Typed Racket.

The corrected translation is depicted on the right side of
figure~\ref{fig:intbeh2}, which shows the final behaviour of the wrapper, which
...
figure~\ref{behtrans} is equivalent to the set of casts that would have occured
...
Another challenge is that multiple wrappers can be layered on top of a target
object. Each time a reference crosses a boundary between typed and untyped code,
it will accrue an additional wrapper. When a method is invoked on a wrapper
chair, that method will be invoked on every wrapper, until finding its way to
the target object. A key property of the wrapper generation functions of
\figref{wrap} is that methods should not be ``lost''. That is to say, any method
in the target object, even if it is not in the type it is being cast to, will be
retained in the wrapper.  This property is somewhat unconventional, and its need
is illustrated in \figref{ctod}. Class \C and \D are preexisting.  They are not
related by subtyping as \any is not a supertype of \E. The first cast from \C to
...
a subtype cast. The argument on the other hand is converted to dynamic with a
...
\begin{mathpar}
\IRule{MOA1}{\HasType{\E}\x\t}{\GenCast{\K}\E\x\x\t}
...
\IRule[width=30em]{MOA3}{
    \GenCast\K\Env{\e_1}{\e_3}{\C} \\ \m({\t_1}):\t_2 \in \classoff\C\K \\ \lnot\statictype{\t_1}{\K}{\cdot} \\ \b{\AnaCast\K\Env{\e_2}{\e_4}{\any}}
}{
...
\section{Generative Behavioral Casts}

\subsection{Expression translation for Behavorial semantics}
...
\subsection{Class wrapper for Behavorial semantics}
...
\classoff\C\K returns the type signutare for every method, including field getters and setters, in the class \C.
