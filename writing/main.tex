...
The function \names\k returns the names of the untyped methods of the class \k.
...
\CondRule{E10b}{  %% Shallow Structural
 \names{\App\K\C}  $\supseteq$  \names{\App\K\D} \AND \obj\C{\b\a} = \App\s\a
}{ 
...
\IRule{SRec}{
 ...
 \M' = \M~\C\Sub\D\\
 \forall~\fd\in\b\fd \implies \StrSub{\M'}\K \C \fd\\
 \forall~\md\in\b\md \implies \StrSub{\M'}\K \C \md\\
}{
 \StrSub \M\K \C \D 
}
\end{mathpar}
...
We write \readf\s\a\f to denote a read of field \f from the object
stored at \a in \s.

\begin{equation*}
\readf \s\a{\f} = \ap ~~\mathit{if}~~  \s(\a) = \obj\C{\a_1\dots\a_n \ap \dots} \AND
 \K(\C) = \Class \C {\Fdef{\f_1}{\t_1}\dots\Fdef{\f_n}{\t_n}\Ftype\f\t\dots}{\b\md}
\end{equation*}

\subsection{Write field}

We write \setf\s\a\f\ap to denote the write of value \ap into field \f of
the object stored at \a in \s.

\begin{equation*}
\setf \s\a{\f_{i}}\ap = \sp ~~\mathit{if}~~ \begin{cases}
 \s(\a) = \obj\C{\a_1\dots\a_n\, \a'' \dots}\\
 \K(\C) = \Class\C{\Fdef{\f_1}{\t_1}\dots\Fdef{\f_n}{\t_n}\,\Fdef\f\t\dots}{\b\md}\\
 \sp = \s[\Bind{\a}{\obj\C{\a_1\dots\a_n\, \ap \dots}}]
\end{cases}
\end{equation*}
