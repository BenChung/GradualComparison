...
If $\Mtype\m{\t}\tp \in \classoff{\C}{\K}$, $\EnvType\cdot\s\K\e\C$ and 
$\EnvType\cdot\s\K\ep\t$, then $\Reduce \K{\KCall\e\m{\ep}{\t}{\tp}}\s \K\epp\s$ where $\EnvType\cdot\s\K\epp\tp$.

Alternately, if $\Mtype\f{\b\t}\tp \in \classoff{\C}{\K}$, $\EnvType\cdot\s\K\e\C$ and (if applicable)
$\EnvType\cdot\s\K\ep\t$, then $\Reduce \K{\Call\e\f{\ep}}\s \K\epp\s$ where $\EnvType\cdot\s\K\epp\tp$.
