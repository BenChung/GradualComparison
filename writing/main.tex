...
of \t. \figref{behave} details the semantics of the behavioral cast. 
...
\xt{wrap}. The \xt{wrap} function is formally presented in section \ref{wrap} of the appendix. 
The \xt{wrap} function takes 5 arguments, the source class
...
At the surface language level, the Monotonic semantics are very similar to that
used by the transient semantics, using consistency on top of a similar type
system to the one used by TypeScript.

\begin{mathpar}
\IRule{C1}{\tmeet\t\tp\cdot\K = \tpp\,\Kp}{\consistent\K{\t}{\tp}} 
\end{mathpar}

Common to both transient and monotonic is the idea of consistency, which will
become substantially more important in our discussion of the monotonic
semantics. Consistency is effectively the idea that two types \emph{do not
contradict one another}. For example, the types \xt{int} and \any are
consistent, but the types \xt{int} and \xt{string} are not, since no member of
one could be in the other.

To implement consistency in our formal system, we will use the \texttt{tmeet}
operator, which we will see again later in our discussion of monotonic. The
purpose of \xt{tmeet} is to find the type common to both given types, which, if
it exists, implies that conistency holds. Using the earlier example, the result
of \xt{tmeet} of \xt{int} and \any would be \xt{int}, as \xt{int} is the most
restrictive of the two given types.

In effect, \texttt{tmeet} takes the place of the $\sqcap$ operator
in~\cite{Siek2015}, fufilling the same purpose. In short, the \texttt{tmeet}
operator takes two types and finds a third type that retains all of the type
assertions in the two input types.

For example, consider figure~\ref{fig:tmeet_ex}, where we compute the meet of
two types, both of which have the same function, but alternate the typing for
that function, where \texttt{A} has the return type typed, whereas \texttt{B}
has the argument typed. \texttt{tmeet} will compute the result type as a new
\texttt{C}, where both the argument and return type  are typed.
...
invoking \RuleRef{TM4}, which, via \xt{mmeet}, computes the meet of each of the
arguments to \xt{m}, invoking \RuleRef{TM1} and \RuleRef{TM2} for the argument
and return type of \xt{m}.
...
in example~\ref{fig:tmeet_rec_ex}, where the meet  of \xt{A} and \xt{B}
recursively depends on the meet of \xt{A} and \xt{B}. If we proceeded naievely,
without \P, \RuleRef{TM4} would invoke \RuleRef{TM4} again to compute the meet
of the arugments to \m. We use \P and \RuleRef{TM5} to avoid this.
...
\xt{A} and \xt{B} (which, in this example, is \xt{E}) into \P, which will then
...
Now that we have discussed the key operator used by monotonic, we will now
present the surface to \kafka translation. Of special note is \RuleRef{MCT1},
and the two rules which use it, \RuleRef{MT} and \RuleRef{CR1}. In these rules,
we are actively replacing every type the user wrote with ones generated by the 
...
This last decision is further evidenced in our expression translation, most
notably in the new handling of calls to both methods and fields, shown in
\RuleRef{MOA2}, \RuleRef{MOA3}, and \RuleRef{MOA4}.
...
Figure~\ref{fig:mono_ex2} illustrates the nature of these wrappers and how they
are added into the heap. In this example, as in figure~\ref{fig:mono_ex1}, we
show the state of the heap at two discrete times, before and after we cast the
\a reference to \xt{E}. Each wrapper is associated with a specific type that it
enforces, with \xt{CW} and \xt{DW} enforcing \C and \D, respectively, and wrap
over an underlying object of known type, enforcing its type guarantees, as seen
in \xt{CW} and \xt{DW}.
...
site for \m, with the exception of the final definition in \xt{FDW}, which will
...
wrapper for the object that replaces the previous wrapper. \xt{ECW} and \xt{FDW}
...
it to the required type (in this case \xt{FW}, the wrapper type that would 
protect objects of type \xt{F}, which is used instead of \xt{F} for techincal
...
up casts on getters and setters, as seen in \xt{ECW}.

Also of interest is the alterations that happened in \xt{FDW}. Here, when we 
specialized \xt{D} to \xt{F}, we both added in a lot of additional (and, in this
...
\subsection{Expression translation for Behavioral semantics}\label{behtrans}
...
\subsection{Class wrapper for Behavioral semantics}\label{wrap}
