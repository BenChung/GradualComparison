...
classes.  The behavioural cast \BehCast\t\a generates a wrapper object that
...
the cast \BehCast\t\e depictes the behavioural cast, and the typing and semantic 
...
monotonic cast semantics in \kafka. The monotonic cast semantices of \kafka are
...
As a concequence, every call, even to a surfically typed call site (one with an argument
...
to the type of the underlying storage cell. For retreiving values, we reverse this process.
...
The functionarlity of the \xt{moncast} rule is broken into two functions.
...
byt the monotonic cast. 
The second function (\xt{spec}) updates the heap and class table with the
objects and types listed in \S.

The \xt{retype} function utilies the meet operation in determining the correct
casting type for every object transitively reachable from the cast object. 
The workings of the meet operator, and the \xt{tmeet} function,
its implementation, is described in detail with the example presented in
figure~\ref{fig:tmeet_rec_ex}.
