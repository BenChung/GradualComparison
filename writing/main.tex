...
cottage industry of static type systems, often referred to as \emph{gradual
  type systems}, that allow developers to annotate legacy untyped code
piecemeal. Soundness for programs which contain a mixture of typed and
untyped code entails catching some errors at type checking time and other
errors as the program executes. After a decade of research on gradual types
it has become clear that the combination of mutable state, self references
and subtyping present serious challenges to implementers of gradual type
systems for object-oriented languages.  This papers overviews the state of
the art in gradual types for objects. We introduce \kafka, a class-based
object calculus with a static type system, dynamic method dispatch,
transparent wrappers and dynamic class generation. We model key features of
several type systems for dynamic languages by translation to \kafka and
discuss the implication of the respective designs.
\end{abstract} 
...
calculus~\cite{cardelli:1996:theory-of-objects}. Their type system featured
a subtype relation that combined structural subtyping with the consistency
relation between terms that only differ in dynamic type annotations.
Soundness at the boundaries between typed and untyped code is ensured by
inserting casts as in their previous work that targeted purely functional
languages~\cite{SiekTaha06}.  Ten years later, many systems support the
gradual addition of type annotations to untyped object-oriented
...
%%
Yet, faithful realizations of that elegant idea have proved to be
surprisingly elusive.  One reason is that the paper did not consider mutable
state.  The combination of mutable state, aliasing and subtyping complicates
enforcement strategies as one must consider the case where one object is
being accessed at different types. While several solutions have been
proposed to address this, the performance implications of their
implementation strategies seems daunting.\footnote{Most practical
  \emph{sound} type system reports order of magnitude pathologies, e.g.~5x
  slow downs for Gradualtalk~\cite{allende13}, 10x for Reticulated
  Python~\cite{siek14}, 22x for Safe Typescript~\cite{safe-typescript}, and
  121x for Typed Racket~\cite{popl16}. These numbers merely indicate the
  existence of configurations that hurt performance.  They are neither
  comparable nor worst case indicators.  Most systems lack rigorous
  evaluations.  } Predictably, industrial strength systems have chosen to
compromise on soundness to avoid degrading performance.\footnote{Industrial
  languages (Dart, Typescript, Hack) have unchecked modes that won't catch
  all type errors.}
...
than by encoding. We propose a target language inspired by modern runtime
systems such as the Java Virtual Machine and the .Net environment.  Both
have reasonably efficient implementations. They have a static type systems
with classes and subtyping but allow for dynamically resolved method
dispatch.\footnote{Both languages support reflective method invocation based
  on the string name of the invoked method. Since C\# 4.0, dynamic
  resolution is part of the language~\cite{BAT14}. In Java,
  \xt{invokedynamic} allows for custom method dispatch.} In both
environments, primitive types can be unboxed as the static typing an
accurately final types.

One contribution of this work is \kafka, a statically typed object calculus.
\kafka is class based (with an explicit class table \K), has mutable state
(a heap address \a refers to an object, each field \f is reifed into a pair
of getter and setter methods), and allows the dynamic generation of wrapper
classes (by update of the class table \K and allocation of new objects \a).
Methods can be statically resolved, denoted by a simple call \Call\a\m\x
operation, or can be dynamically resolved, denoted by a dynamic call
\DynCall\a\m\x. The \kafka type system is based on structural subtyping,
although this is not essential, we could also use a nominal type system with
multiple subtyping as in Java or C\#.  \kafka has a dynamic type, denoted
\any. It's last noteworthy feature is support for transparent wrappers:
\kafka has a distinguished \that field which does not appear in a class'
type.

The heart of the translation to \kafka is the insertion of explicit casts.
\kafka includes four cast operations. Two of the cast are purely structural.
The subtype cast \SubCast\t\a checks if the object references by \a is a
subtype of type \t (\t can be either a class \C or the dynamic type \any).
The shallow cast \ShaCast\t\a merely checks that the object has all the
requested methods specified by type \t. In a full fledged language, this
cast could also test for tags of primitive types. \kafka, also has two, so
called, generative casts. The behavioral cast \BehCast\t\a generates a
wrapper object that monitors that methods invoked on \a abide by the
interface of type \t. The monotone cast \MonCast\t\a returns a wrapper which
monitors all invocations to \a from any reference.

\renewcommand{\u}[1]{\underline{#1}\xspace}

Another contribution is the translation from gradual type systems into
\kafka. Each gradually typed expression \HT{\u\e}{\u\T} is translated to a
...
in \kafka is straightforward, gradual type system are a more interesting
story in that respect.  In \kafka, a well-typed program can only get stuck
at a cast or a dynamically resolved call.  With gradual types, an expression
...
\figref{over} gives a synthetic overview of gradual type systems for objects
that have publicly available implementations. All languages are class-based,
even TypeScript which simply does not have types for plain JavaScript
objects. Most languages have a nominal type system as this is fairly natural
for class-based systems with inheritance. The exceptions are TypeScript,
likely because of its JavaScript roots, Reticulated Python because
consistency does not mix with nominal types and Typed Racket because of its
support for first-class classes.


\newcommand{\rot}[1]{\begin{rotate}{80} #1 \end{rotate}}
\newcommand{\X}{\EM{\bullet}}
\newcommand{\XX}{\EM{\bullet^1}}
...
\end{tabular}}
\caption{Overview of implemented systems.}\label{over}
\end{figure}

Optional types are the main mode of execution for Dart, Hack and
TypeScript. Thogh Dart has a checked mode that can catch more errors (but is
still not sound [CHECK]). Types in Transient Python are also optional, in
the sense that a variable \x can refer to any value, but casts are applied
before use.  

Concrete types are supported by C\#, StrongScript and Thorn. In the case of
Thorn and C\# when primitives are concretely typed they can be unbox and do
not require tagging.

Subtype casts are used in most nominal languages, they are fast and easy to
implement. Shallow casts are used by Racket and Python to ensure that
wrappers are applied to objects that have the right methods.

Generative casts are casts that require associating information with
objects, they typically allocate data structures.

Blame assignment allows to track where an error comes from.

Performance pathologies are self reported worst cases. These numbers should
not be considered definitive performance indicator as they are self-reported
values taken from very small sample sizes.
...
determine that the real value of \xt{evil} is incompatible with A. As a
...
\newcommand{\rtranst}[4]{#1 \Rightarrow #2 \vdash #3 \looparrowright_t #4}
\begin{mathpar}
\IRule{REW1}{ }{ \rtranst{\b{\mt_s}}{\b{\mt_t}}\x\x }
\\
\IRule{REW2}{ \Mtype\f{}\t \in \b{\mt_t} }{\rtranst{\b{\mt_s}}{\b{\mt_t}}{\Get\this\f}{\BehCast\t{\Get\this\f}} }

\IRule{REW3}{ \e \neq \this \\ \rtranst{\b{\mt_s}}{\b{\mt_t}}{\e}{\ep} }{\rtranst{\b{\mt_s}}{\b{\mt_t}}{\Get\e\f}{\Get\ep\f}}

\IRule[width=20em]{REW4}{ \Mtype\f\t\t \in \b{\mt_s} \\ \Mtype\f\tp\tp \in \b{\mt_t} \\ \rtranst{\b{\mt_s}}{\b{\mt_t}}{\e}{\ep} }{\rtranst{\b{\mt_s}}{\b{\mt_t}}{\Set\this\f\e}{\BehCast\tp{\Set\this\f{\BehCast\t\ep}}}}

\IRule[width=20em]{REW5}{ \e_1 \neq \this \\ \rtranst{\b{\mt_s}}{\b{\mt_t}}{\e_1}{\e_2} \\ \rtranst{\b{\mt_s}}{\b{\mt_t}}{\ep_1}{\ep_2} }{\rtranst{\b{\mt_s}}{\b{\mt_t}}{\Set{\e_1}\f{\ep_1}}{\Set{\e_2}\f{\ep_2}}}

\IRule[width=25em]{REW6}{ \Mtype\m{\t_1}{\tp_1} \in \b{\mt_s} \\ \Mtype\m{\t_2}{\tp_2} \in \b{\mt_t}  \\ 
\rtranst{\b{\mt_s}}{\b{\mt_t}}{\e}{\ep}}{\rtranst{\b{\mt_s}}{\b{\mt_t}}{\Call{\this}\m{\e}}{\BehCast{\tp_1}{\Call{\this}\m{\BehCast{\t_2}{\ep}}}}}

\IRule[width=20em]{REW7}{ \e_1 \neq \this \\ \rtranst{\b{\mt_s}}{\b{\mt_t}}{\e_1}{\e_2} \\ \rtranst{\b{\mt_s}}{\b{\mt_t}}{\ep_1}{\ep_2}}{\rtranst{\b{\mt_s}}{\b{\mt_t}}{\Call{\e_1}\m{\ep_1}}{\Call{\e_2}\m{\ep_2}}}

\IRule{REW8}{ \rtranst{\b{\mt_s}}{\b{\mt_t}}{\e_1}{\e_2} \\ \rtranst{\b{\mt_s}}{\b{\mt_t}}{\ep_1}{\ep_2}}{\rtranst{\b{\mt_s}}{\b{\mt_t}}{\DynCall{\e_1}\m{\ep_1}}{\DynCall{\e_2}\m{\ep_2}}}

\IRule{REW9}{ \rtranst{\b{\mt_s}}{\b{\mt_t}}{\b\e}{\b\ep} }{\rtranst{\b\mt_s}{\b\mt_t}{\New\C{\b\e}}{\New\C{\b\ep}}}
\end{mathpar}

\newcommand{\ignorethis}[1]{}
\ignorethis{
\newcommand{\rtransut}[3]{#1 \vdash #2 \looparrowright_u #3}
\begin{mathpar}
\IRule{REU1}{ }{ \rtransut{\b\mt}\x\x }

\IRule{REU2}{ \Mtype\f{}\t \in \b\mt }{\rtransut{\b\mt}{\Get\this\f}{\BehCast\t{\Get\this\f}} }

\IRule{REU3}{ \e \neq \this \\ \rtransut{\b\mt}{\e}{\ep} }{\rtransut{\b\mt}{\Get\e\f}{\Get\ep\f}}

\IRule{REU4}{ \Mtype\f\t\t \in \b\mt }{\rtransut{\b\mt}{\Set\this\f\e}{\BehCast\t{\Set\this\f{\SubCast\any\e}}}}

\IRule{REU5}{ \e_1 \neq \this \\ \rtransut{\b\mt}{\e_1}{\e_2} \\ \rtransut{\b\mt}{\ep_1}{\ep_2} }{\rtransut{\b\mt}{\Set{\e_1}\f{\ep_1}}{\Set{\e_2}\f{\ep_2}}}

\IRule{REU6}{ \rtransut{\b\mt}{\e_1}{\e_2} \\ \rtransut{\b\mt}{\ep_1}{\ep_2}}{\rtransut{\b\mt}{\Call{\e_1}\m{\ep_1}}{\Call{\e_2}\m{\ep_2}}}

\IRule{REU7}{ \rtransut{\b\mt}{\b\e}{\b\ep} }{\rtransut{\b\mt}{\New\C{\b\e}}{\New\C{\b\ep}}}
\end{mathpar}
}
...
\SPP   \Mdef\m\x\t\tp {~\BehCast\any{{[\BehCast\t\x / \x] \ep}}~}
&  \All \m \Mdef\m\x\t\tp\e\in\b\md  &\wedge& \rtranst{\bmt}\cdot{\e}{\ep}\\
\SP \}\\
\end{array}\]
%}%%END IGNORE
...
\begin{mathpar}
\IRule{A1}{\HasType{\E}\x\t \\ \x \neq \this}{\GenCast{\K}\E\x\x\t}

\IRule[width=30em]{A2}{
    \GenCast\K\Env{\e_1}{\e_3}{\C} \\ \n(\b{\t_1}):\t_2 \in \classoff\C\K \\ \isstatic{\t_1}{\K} \vee \n=\f \\ \b{\AnaCast\K\Env{\e_2}{\e_4}{\t_1}}
}{
    \GenCast\K\Env{\Call{\e_1}\n{\b{\e_2}}}{\Call{\e_3}\n{\b{\e_4}}}{\t_2}
}

\IRule[width=30em]{A3}{
    \GenCast\K\Env{\e_1}{\e_3}{\C} \\ \m({\t_1}):\t_2 \in \classoff\C\K \\ \lnot\isstatic{\t_1}{\K} \\ \b{\AnaCast\K\Env{\e_2}{\e_4}{\any}}
}{
...
\IRule{WThis}{
  \HasType\Env\this\C \\
  \Mtype \n{\b\tp}\tpp\in \classoff\C\K  \\
  \b{\EnvType \Env\s\K\ep\tp}
}{
  \EnvType \Env\s\K{\Call\this\n{\b\ep}}\tpp
}    
...
\begin{mathpar}
\IRule{ENMT1}{
  \md = \Mdef\m\x\t\tp{ \MonCast\tp{\Call{\Get\this\that}\m{{\MonCast\t\x}}}} \\
...
\IRule{ENMT2}{
  \md = \Mdef\m\x\any\any {\SubCast\any{\MonCast\tp{\Call{\Get\this\that}\m{\MonCast\Cp{\MonCast\t\x}}}}}\\ 
  \enfortype\t\W\K = \tpp\,\Wp\,\Kp \\
  \enfortype\tp\Wp\Kp = \tppp\,\Wpp\Kpp \\  
  \mdpp =\Mdef\m\x\tpp\tppp {\MonCast\tppp\x}\\
  \enformt{\b\md}{\Wpp}{\Kpp} = \b\mddp \\
...
\IRule{ENDT}{
  \md = \Mdef\m\x\any\any{\SubCast\any{\MonCast\tp{\DynCall{(\SubCast\any{\,\Get\this\that})}\m{\SubCast\any{\MonCast\t\x}}}}} \\ 
  \enfortype\t\W\K = \tpp\,\Wp\,\Kp \\
  \enfortype\tp\Wp\Kp = \tppp\,\Wpp\Kpp \\   
  \mdpp =\Mdef\m\x\tpp\tppp {\MonCast\tppp\x}\\
  \enformt{\b\md}{\Wpp}{\Kpp} = \b\mddp \\
...
\IRule{ENFS}{
  \md = \Mdef\f\x\any\any{\SubCast\any{\MonCast\t{\Set{\Get\this\that}\f{\MonCast\tp{\MonCast\t\x}}}}} \\ 
  \enfortype\t\W\K = \tpp\,\Wp\,\Kp \\
  \enfortype\tp\Wp\Kp = \tppp\,\Wpp\Kpp \\   
