...
\IRule{RC1}{
  \htype \a\S\s = \tp\\
  \meett{\cdot, \K, \tp, \t} = \tpp~\Kp\\ 
  \tp\not\EQ\tpp \\
  \b{\a~\t} = \fieldtypes \tpp\Kp\s\a \\
  \Sp = \Heap\S{\Bind\a\tpp} \\
  \retype{\b\a}{\b\t}\s\K\Sp = \Spp\,\Kpp
}{
  \retype \a\t\s\K\S = \Spp\,\Kpp
}
...
\IRule{RC2}{
  \retype\a\t\s\K\S = \Sp\,\Kp\\
  \retype{\b\a}{\b\t}\s\Kp\Sp = \Spp\,\Kpp
}{
  \retype {\a\,\b\a}{\t\,\b\t}\s\K\S = \Spp\,\Kpp
}
...
\IRule{SP1}{
  ...
  \spec{\a : \D ~ \S, \s[\a \mapsto \C\{\b{\ap}\}], \K} = \sp[\a\mapsto \D\{\b\ap\}]
}
...
\IRule{M4}{
  ...
  \meett{\Pp, \K, {\Type{\b\mt}}, {\Type{\b\mtp}}} = \tpp~\Kp\\
  \Kpp = \Kp~\classgen{\C,\tpp,\EC,\Kp}\\
}{
    \meett{\P, \K, \C, \D} = \EC~\Kpp
}
