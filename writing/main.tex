...
Likewise, when converting from a statically typed expression to an untyped one,
as seen in \RuleRef{TRAASC3}, we use the behavioural cast operator, applying a 
wrapper. We do not need to use a name cast here, as no methods are strictly 
required on a any-typed expression, and as a result, no additional checks are 
needed.
...
\IRule{TRAASC3}{
  ...
  \AnaCast\K\Env\e{\BehCast\any\ep}\any
}
\end{mathpar}
