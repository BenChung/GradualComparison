...
We consider gradual typing systems as a combination of three components:
\begin{itemize}
\item \emph{A type-driven cast insertion system}, which ensures the 
correctness of the code it produces, as well as providing the surfical
type system. Cast insertion takes code that works directly with \any
values and adds casts to ensure safety.
\item \emph{An underlying type system}, which works on the target of
cast insertion. The base type system prevents any \any values from
being used without previously checking them, ensuring soundness.
\item \emph{A dynamic semantics}, evaluating the result of translation,
and allowing the code to be run.
\end{itemize}

We then characterize our four gradual typing systems by constructing them
within this common framework. However, the four systems in fact share a 
considerable amount of their behaviour, as features such as variable typing
remain the same across all four systems. To prevent needless duplication,
we introduce a \emph{fifth}, \emph{core} gradual typing system, that provides
a basis for us to modify. We will introduce the details of these stages in 
a discussion of the core, then explore how the systems that we consider differ.
...
function application, is less specified than it could be. Other works in this area, including (DUNFIELD et al)
...
is not required, as our type inference does not have strict specifity requirements. 
...
\IRule{A4}{
	...
	\GenCast{\E}{\Call{\e_1}\m{\b{\e_2}}}{\Call{\e_3}{\m_\t}{\b{\e_4}}}{\tp 3}
}
...
\IRule{A5}{
	...
	\GenCast{\E}{\Call{\e_1}\m{\b{\e_2}}}{\Call{\Cast{\Type{\Mtype{\m_\xt{u}}{\b{\any}}\any}}{\e_3}}{\m_\xt{u}}{\b{\Cast\any{\e_4}}}}{\Weak\tp3}
}
...
\end{mathpar}
\caption{Strongscript typing extensions}
\end{figure}
...
\end{mathpar}
\caption{Monotonic typing extensions}
\end{figure}
