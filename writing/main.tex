...
\newcommand{\ThorSub}[4]{#1~#2 \vdash #3 \Sub_t #4}
...
\IRule{THSRec}{
 ...
 \M' = \M~\C\Sub\D\\
 \f:\t \in\b\fd \wedge \f \neq \that \implies \ThorSub{\M'}\K \C \f:\t\\
 \md\in\b\md \implies \ThorSub{\M'}\K \C \md\\
}{
 \ThorSub \M\K \C \D 
}
\end{mathpar}
...
\IRule{THSMet}{
 ...
 \ThorSub \M\K \C {\Mdef\m\x\t\tp\e}
}

\IRule{THSFie}{
  \Fdef\f\t\In\App\K\C
}{
 \ThorSub \M\K \C {\Fdef\f\t}
}

\IRule{THSSet}{
  \Fdef\f\t\In\App\K\C
}{
 \ThorSub \M\K \C {\Mdef\f\x\t\t\e}
}

\IRule{THSGet}{
  \Fdef\f\t\In\App\K\C
}{
 \ThorSub \M\K \C {\Mdefz\f\t\e}
}
\end{mathpar}
