...
One contribution of this work is \kafka, a statically typed, class-based,
object calculus with mutable state and structural subtyping.



In order to present the essence of
gradual typing for objects we introduce \kafka, a statically typed
class-based calculus with mutable state and structural subtyping. Untyped
method calls are supported with a distinct dynamic dispatch operation. The
calculus allows for class generation, in particular, it allows the
generation of transparent wrapper classes. We define the semantics of
various gradual type systems by translation from a surface syntax into
\kafka extended with generative casts.
