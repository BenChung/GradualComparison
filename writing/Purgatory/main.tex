\documentclass[a4paper,USenglish]{tex/llncs}
...
\institute{Northeaster University, CVUT and INRIA}
...
%\vspace{-5mm}\epigraph{\small ``Because half the problem is seeing the problem''}\vspace{-5mm}
...
The first contribution of this work is the design of \kafka, a statically typed object
...
Our other contribution is the translation of five type systems representative
...
%\vspace{-5mm}\epigraph{\small ``If you know the enemy and know yourself...''}\vspace{-5mm}
...
%\vspace{-2mm}\epigraph{\small ``Oh, let me see your beauty when the witnesses are gone''}\vspace{-5mm}

\noindent While gradual type systems for objects come in many shapes and sizes,
with different constraints and complexity, as well as their own strength and
weaknesses. This section gives a brief introduction to the five representative
systems that we are studying in this paper. We conclude with a discussion of the
trace preservation property and how the systems differ in terms of catching
errors. Rather than speaking directly to the source systems, we present their
\kafka modelization. The details of this modelization are the topic of the rest
of the paper, so this section can be read as a preview. We leave blame for
future work, and discuss semantics in terms of their translation to a typed
class-based calculus with structural subtyping.
...
%\vspace{-2mm}\epigraph{\hspace{-1cm}\small ``Aux chenilles du monde entier et aux papillons qu'elles renferment''}\vspace{-5mm}
...
The small operational step operational semantics for \kafka appears in
\figref{fig:semantics}. A configuration is a triple \K\e\s, where \K is a class
...
subtype of \C, while shallow casts check inclusion of method names. Evaluation
contexts are deterministic and enforce a strict evaluation order.


\section{Translating Gradual Types}

%\vspace{-2mm}\epigraph{\small ``Was ist mit mir geschehen? dachte er. Es war kein Traum''}\vspace{-5mm}
...
example, the types \xt{int} and \any are consistent, but the types \xt{int}
and \xt{string} are not as no member of one could be in the other.  The
consistency operator ($\sim$) in our formal system is implemented using the
concept of meet described by the \texttt{tmeet} function. The \xt{tmeet}
...
types. Using the earlier example, the meet between \xt{int} and \any would
be \xt{int}, as \xt{int} is the more restrictive of the two types. In
...
presented.
%% TODO WE CAN'T HAVE METHODs THAT ARE PARTIALLY TYPED.
...
The translation from the Monotonic surface language is in \figref{mono}. The
rule \RuleRef{PT} translates programs, \RuleRef{CR1} and \RuleRef{CR2}
translates classes, and \RuleRef{MT} translates methods. In the \RuleRef{MT}
and \RuleRef{CR1} rules, we are actively replacing every type the user wrote
with ones generated by the \xt{monWrap} function. The details of the
\xt{monWrap} function will be discussed later, but for the purposes of the
translation, we can assume that \xt{monWrap} replaces every typed function
...
a type contains only typed methods.  The rule \RuleRef{MOS2} translates
typed methods, \RuleRef{MOS2} translate methods that are not purely typed.
\RuleRef{MOS4} translate untyped methods, \RuleRef{MOS2} translate object
creation.  The purpose of the \xt{monWrap} function is further evidenced as
...
The monotonic cast \MonCast\C\a imposes the type \C onto the object at \a
and every object transitively reachable from \a. The motivation for this
cast comes from maintaining the type correctness of every reference in the
program~\cite{Siek2015}.
...
Figure~\ref{fig:mono_ex1} presents an example of the monotonic cast.  A new
instance of class \D is created, then an instance of class \C is created
pointing to it. As shown by heap snapshot 1, the field \f in \C has type
\any, and the method \m in \D has dynamic argument and return type. Classes
\E and \xt{F} are introduced and they are structurally identical to \C and
\D, but with specific types for their fields and methods.  When the instance
of class \C referenced by \a is casted to \E, the monotonic semantics will
ensure any access to field \f or method \m from anywhere in the program will
follow the type invariants asserted by class \E.

However, as shown in heap snapshot 2, the reference \xt{d} still refer to
the instance of \D, from earlier, and \xt{d} does not know the existence of
the new invariant applied to class \D. Despite being typed \any, when \xt{d}
try to invoke method \m on the apparently dynamic object, a cast failure
will occur. The monotonic semantics ensures that the provided argument must
be of type \E.

This example highlights the need for the monotonic cast semantics to be
generative.  In order to guarantee the type correctness of all references,
monotonic has to generate guards that check the argument and return type of
every method, and the type of any field set, ensuring that they maintain
consistency with the maximally static type. Additionally, monotonic ensures
that prior references respect future type invariants by performing an
in-place update of any object whose type is altered by the cast.
...
exception of the final definition in \xt{FDW}.  Under the monotonic
...
previous wrapper. The wrappers \xt{ECW} and \xt{FDW} are created when \a is
casted to \xt{E}.  To ensure that types for fields are respected, we take a
conservative approach, as seen in \xt{ECW}. When a field on an underlying
object is set, we first cast it to the required type (in this case
\xt{FW}). Then a further $\star$ cast is applied to the type of the
...
the getter and setter methods, as seen in \xt{ECW}.  Another point of
interest is the alteration happening in \xt{FDW}. When the class \xt{D} is
specialized to \xt{F}, several additional casts are inserted, as well as a
...
additional static call site because we know that \xt{E} will not be able to
be further altered under the monotonic semantics, as \xt{E} and every type
...
The first function (\xt{retype}) identities the correct target type for
...
second function (\xt{spec}) updates the heap and class table with the
...
\opdef{~\xt{overloading}(\b\md ~\b\fd)}{Method overloading}
\begin{mathpar}
\IRule{MO1}{
}{
...
\opdef{$\methtrans {\K}{\C}{\md}{\md'}{\K}$}{TypeScript translation for methods} 
...
\IRule{CR1}{ 
  ...
  \classtrans {\K}{\Kp}\Kpp
   ...
   \classtrans {\K}{{\Class \C{\b{\Ftype\f\t}}{\b\md}~\Kp}}{\Class \C {\b{\Ftype\f\any}}{\b{\md'}}~\Kpp}}
...
\IRule[width=20em]{TSS5}{
    \HasType{\E}\this\C \\ \f():\t_2 \in \classoff\C\K 
}{
    \GenCast\K\Env{\Call{\this}\f{}}{\Call{\this}\f{}}{\any}
}
...
\IRule{THS3}{
    \GenCast\K\Env{\e_1}{\e_3}{\dt\C} \\ 
    \src{\m({\D_1}):\D_2 \in \classoff\C\K} \\ 
    \AnaCast\K\Env{\e_2}{\e_4}{\D_1}
}{
    \GenCast\K\Env{\Call{\e_1}\m{{\e_2}}}{\SubCast\D{\DynCall{\e_3}\m{\SubCast\any{\e_4}}}}{\D_2} %Q: do I need to cast the return value of non-bang
}

\IRule{THS4}{
    \GenCast\K\Env{\e_1}{\e_3}{\dt\C} \\ 
    \src{\m({\t_1}):\t \in \classoff\C\K} \\ 
    \t \neq \D \\
    \AnaCast\K\Env{\e_2}{\e_4}{\t_1} 
}{
...
\IRule{THS5}{
  ...
  \src{\Mtype\m\t\tp \in \classoff\D\K} \\
  \AnaCast\K\Env{\e_2}{\e_3}{\t}
}{
...
\opdef{$\methtrans\K\C{\md}{\md}{\K}$}{Transient translation for methods}
...
\IRule{MTT}{
  \AnaCast{\K}{\HT\this\C~\HT\x\C}{\e}{\ep}{\Cp}
}{\methtrans\K\C{\Mdef\m\x\C\Cp\e}{\Mdef\m\x\any\any{\SubCast{\any}{\New{\EMxt{A2}}{\SubCast\C\x, \e}.{\EMxt{f2}}()}}}{}}
\end{mathpar}
...
\begin{mathpar}
\IRule{GRS1}{\HasType{\E}\x\t}{\GenCast{\K}\E\x{\SubCast\t\x}\t}
...
\IRule{GRS3}{
    ...
    \GenCast\K\Env{\Call{\e_1}\m{{\e_2}}}{\DynCall{\e_3}\mdu{{\e_4}}}{\any}
}
...
\opdef{$\progtrans{\e~\K}{\e'~{\K'}}$}{Type Racket translation for programs}
\opdef{$\classtrans {\K}{\Class \C{...}{...}~\K}{\Class \C{...}{...}~\K}$}{Type Racket  translation for classes}
\opdef{$\methtrans {\K}{\C}{\md}{\md'}{\K}$}{Type Racket  translation for methods}
...
\IRule[width=30em]{TRS3}{
    \GenCast\K\Env{\e_1}{\e_3}{\C} \\ \m(\t):\tp \in \classoff\C\K \\ \b{\AnaCast\K\Env{\e_2}{\e_4}{\t}}
}{
    \GenCast\K\Env{\Call{\e_1}\m{{\e_2}}}{\KCall{\e_3}\m{{\e_4}}\t\tp}{\Cpp}
}
...
&  \Mtype\f\t\t \in \bmt\\
\SPP   \Mdef\m\x\any\any {~\BehCast\any{{\e}}~}
...
\step[inpswf] $\WFq \s$ \basis By inversion of \refby{progform}
\step Apply IH to \refby{base} and case analyze.
\begin{casel}
  \caseof{ \begin{iknown}
    ...
    \end{iknown}} 
  \begin{proofy}
  \step $\Reduce\K{\BehCast{\t}{\e}}\s\Kp{\BehCast{\t}{\ep}}\sp$ \basis By defn. of evaluation environments on \refby{kred}
  \step[ktype] $\EnvType\cdot\sp\Kp{\BehCast\t\ep}\t$ \basis By \RuleRef{WB1}
  \step $\WFp\Kp{\ep~\sp}$ \basis By \refby{kbase, ktype}
  \end{proofy}
  ...
  \begin{proofy}
    ...
    \begin{casel}
      ...
      \begin{proofy}
        ...
        \step[b2] $\EnvType\cdot{\K\k}\sp\ap\D$ \basis \RuleRef{W9}
        \step[b1] $\StrSub\cdot{\K\k}\D\Cp$ \basis correctness of $\xt{wrap}$ on \refby{wcorr}
        \step[wth] $\EnvType\cdot{\K\k}\sp\ap\Cp$ \basis \RuleRef{W2} on \refby{b1,b2}
        \step $\WFp{\K ~ \k}{\ap~\sp}$ \basis \RuleRef{WP} on \refby{wth, inpswf}
      \end{proofy}
      \caseof{$\t = \any$}
      \begin{proofy}
        \step Let $\D$ be fresh.
        ...
        \step Let $\sp = \s[\ap \mapsto \D\{\a\}]$.
        \step[b21] $\EnvType\cdot{\K\k}\sp\ap\any$ \basis \RuleRef{W10}
        \step $\WFp{\K ~ \k}{\ap~\sp}$ \basis \RuleRef{WP} on \refby{b21,wcorr2}
      \end{proofy}
    \end{casel}
  \end{proofy}
\end{casel}
...
\subsection{Typescript Translation}
