...
\end{lstlisting}
\caption{A simple use of meet}
\label{fig:meetex}
\end{figure}

For example, if we were to compute the meet between \C and \D as 
shown in figure~\ref{fig:meetex}, we would end up with the type
\xt{E}. The \any typed argument to \m in \C is taken to be of type
\D from class \D, while the converse is used for \n in \D. The class
\xt{E} then is more specific than both \C and \D. 
...
At run-time the monotonic cast invokes the moncast metafunction, which takes
the address \a of an aobject of class \C, a target type \t, a heap \Env and a
class table \K; it returns an updated heap \Envp and an updated class table
\Kp. What it does is compute a new class \Cp for the object at \a. That class
is obtained by taking the meet of \C (the current class) and \t (the target
type).
