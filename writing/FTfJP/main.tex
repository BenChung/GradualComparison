...
safe, even though \xp is fully typed. If we run \xt{\x.\f = "hello"} at some
...
type (e.g. that \f has type \xt{int}) will be enforced upon usages of
\x. Now, our assignment \xt{\x.\f = "hello"} will fail, because we are
...
\\
\K\HS \FReadR\a{\f[i]} \> \s           \>\Red\>     \K \>$\a[i]$ \> \s  \> \WHERE \>\App\s\a=\obj\C{\a[1],..\a[i],\a[n]..}
\\
\K\HS {\FWriteR\a{\f[i]}\ap} \> \s     \>\Red\>     \K \> \ap \> \sp \>  \WHERE \>\App\s\a=\obj\C{\a[1],..\a[i],\a[n]..} \HS  
\\ \> \> \> \> \> \> \> \sp = \Map\s{\Bind\a{\obj\C{\a[1],..\ap,\a[n]..}}}
\\
