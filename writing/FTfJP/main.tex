...
higher on the type lattice. Two types have a meet if they are structurally the
same up to \any types distributed throughout their structure. The meet itself
consists of replacing all \any types in both types with the most specific known type
in that position from either type.
...
The formal implementation of this operation consists of the meet and mmeet
metafunctions. The meet metafunction takes the two types being met, a context
(for use in recursive types), and the class table. It then outputs the meet of
the two types and a class, which may consist of a new class in the returned
class table. The context \xt{P} is used to ensure termination of the meet
operation on recursive types by storing the names of future meet-generated
classes. The meet of previously-encountered types will be identical, so that
type is produced and termination is ensured. The mmeet operation simply applies
the meet operation, but for method definitions. It will pick out the comparable
method definition in the target method, and generate a suitable method.

The result of the mmeet operation is passed into the typegen method, which
generates a trivial class with all of the met method definitions. However, the
result of typegen is not a semantics-preserving class. Suppose we start out
with $\a = \New\C{}$, then alias it with $\ap = \MonCast\D\a$. If we then call
$\DynCall\a\m{\New\D{}}$, and the underlying object is an instance of \xt{E},
the dynamic call will fail as no dynamically-typed method exists. As a result
of this, our system does not deem \xt{E} to be in a subtyping relationship
with either \C or \D. Moreover, typegen makes no attempt to produce correct
method bodies.
...
system is therefore class \xt{F} as shown in figure~\ref{fig:monwrap}, which
has both the typed and untyped methods specified in both \C and \D. Class
\xt{F} is a proper structural subtype of both \C and \D, and its methods
