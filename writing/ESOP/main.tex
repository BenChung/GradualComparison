...
operations are translated verbatim.
For the optional semantics, we have eliminated the type of the \this
introduction form by casting to \any. The optional semantics make no other
alterations to these terms, as it has no soundness guarantee to ensure.
Transient does not guarantee the values passed to functions or field
assignment respect the declared type of those expressions. Class translation
inserts checks for invalid variables upon method entry, so while the
transient translation has to insert casts to appease the \kafka type system,
these casts will never fail. However, fields are not protected and requires
a guard after dereferencing, as values can be passed through them to violate
their types.
...
additional checks. However, in the three sound semantics, the behavior
differs depending on the strength of the soundness guarantee.  Under the
transient semantics, field assignment is unchecked, deferring type checking
to dereferencing. As a result, we use the analytic translation operator to
...
\figref{litmus} of section \ref{litmustest}. The source code are the four
gradual typing semantics: Concrete, Optional, Behavioral, and Transient.
