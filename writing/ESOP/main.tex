...
their performance implications appear daunting. Developers of industrial
languages have chosen to compromise on soundness to avoid degrading
performance.

This paper explores the design space of gradual type systems for
object-oriented languages by studying four approaches to gradual typing of
objects with different runtime characteristics in terms of errors and
overheads.  The four approaches are representative of languages with
implementations that available in open source. The \emph{optional} approach,
chosen by TypeScript, Dart, and Hack, amounts to a static check followed by
type erasure at runtime.  The \emph{concrete} approach, used in C\#, Thorn,
and StrongScript, combines optional types with traditional statically
typing.  The \emph{behavioral} approach, followed by TypedRacket and
Gradualtalk, 
...
different gradual typing philosophies.  TypeScript stands for unsound
systems that work by erasure, such as Dart and Hack.  Thorn combines dynamic
and static types inspired by C\# and further developed in
...
The type system for this surface language is a simple one, allowing implicit
...
The major differentiating factor between a gradually typed language and a
language that simply has an \any type is the ability to implicitly down-cast
from \any to some known type \t, and the mechanisms through which this is made
safe are what differentiate the different gradual typing semantics.

The naive typing rules would not allow an implicit downcast from \any to a
non-\any type, as it is not safe. However, as we want to make our surface
...
In the surface language, it is possible to pass subtypes as arguments and new
field values (\RuleRef{STGC-SUB}), as in the \kafka type system and as is
traditional. However, the surface language compatiblity relation adds two new
...
If the source program typechecks correctly, we can proceed with translation to
\kafka, the key component of our surface language semantics. For each of the
four semantics in question, we provide a translation from this common,
...
\figref{fig:traclass} presents the class translation for each gradual typing
...
design allows method \m, which expects an instance of class \C as argument,
to be called with a value of type \any at any point. However, this forces \m
to check its arguments at every method invocation. One interesting consequence 
of this requirement is that programs using the
transient semantics actually acquire more casts as they get typed, with the
fully-typed version of a program having the most casts, as seen by \m being
required to check the type on every argument. As a result, the transient
semantics will cause checks to be inserted even in fully typed methods, with
