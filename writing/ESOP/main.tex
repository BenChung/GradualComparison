...
\figref{fig:travar} presents the translation for the field access and variables. For the
...
makes no alterations to these terms.

The guarantee provided by the transient semantics does not extend to correct
argument types, and therefore, does not guarantee that the type of variables
or fields is correct, which is modeled in \kafka by having all argument types
be translated to \any. Notably, the cast introduced by the transient semantics
on variables will never fail - as in the original presentation~\cite{siek14},
we check that the value is correct on function entry - but in order to produce
a well-typed \kafka term, we are forced to cast every invocation site.

However, under the transient semantics, fields can take on ill-typed values.
Consider the following source language program:
