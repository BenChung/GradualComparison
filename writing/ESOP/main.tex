...
\titlerunning{\kafka: A common framework for gradual typing objects}
\author{Benjamin Chung, Paley Li, Francesco Zappa Nardelli, Jan Vitek}
\authorrunning{Chung et al.} 
\institute{Northeastern University, CVUT and INRIA}
...
%TODO: The paragraph above seems to justify the previously proposed solutions are 
% not great because they have poor performance. Which pushes us towards
% declaring performance as a major factor for any solution we present.
...
a runtime check for it.  Anecdotal evidence suggests that structural
subtyping is rarely needed in TypeScript~\cite{ecoop15}. StongScript extends
TypeScript changing subtyping back to nominal.  The consistency relation
used in Reticulated Python is fundamentally structural, it would be
nonsensical to use it in a nominal system.\footnote{Consistency relates
  classes that differ in the number of occurrences of \any in their type
  signature and not whether they are declared to extend one another.}  For
Racket, the heavy use of first-class classes and class generation naturally
leads to structural subtyping as many of the classes being manipulated have
no names.  Optional types are the default execution mode for Dart, Hack and
TypeScript.  Transient Python is, in some senses, optionally typed as any
...
is a cast at almost every call. Finally, languages with generative
casts tend to suffer prohibitive slow downs in pathological cases.
...
the four type systems. The three programs of \figref{litmus} are statically
...
In \figref{litmus}, we present 3 programs, consisting of a class table and a
...
behavioral semantics as the method invocation of \xt{.a(new C())} in the top-
...
reference that is equal to our entire program, or $\exists$ \verb|a : ref, e = Ref a|.
\end{description}
\begin{lstlisting}[basicstyle=\ttfamily\footnotesize, language={}, 
morekeywords={forall, exists, $\forall$}, keywordstyle={\ttfamily\color{blue!80}}, escapeinside={--(}{)--}, mathescape=true]  
...
\emph{retains{\textunderscore}references}) and class table \verb|k| (expressed using \emph{ct{\textunderscore}ext}).
\end{description}
...
dynamic call expression of the form \emph{DynCall} (\emph{Ref} \texttt{a) m (}\emph{Ref} \texttt{a}$'$) can get stuck if \verb|a| refers to an instance of class \verb|C| in \verb|s| 
...
morekeywords={forall, exists, $\forall$}, keywordstyle={\ttfamily\color{blue!80}}, escapeinside={--(}{)--}, mathescape=true]
 (*case 4*) ($\exists$ (t$'$:type) (a:ref) (C:id) (aps:list ref),
       (e = --(\emph{equivExpr})-- (--(\emph{SubCas})-- t$'$ (--(\emph{Ref})-- a)) E) $\wedge$
       --(\emph{In})-- (a, --(\emph{HCell})--(C,aps)) s $\wedge$
...
interest by translating them into \kafka.  For each semantics,
...
Our source semantics has no explicit casts, instead silently coercing
...
To avoid unnecessary clutter, we represent the source semantics using the
...
We then translate this same source language to \kafka, modelling the semantics
of each of the gradual type systems. Each of the type systems in question
provides a different soundness guarantee, which then drives their
translations, and what programs are allowed dynamically.

\subsection{Optional}

%TODO: The cite needs to be filled out.  

The optional semantics are used by languages such as TypeScript [CITEME] and
Hack [CITEME] to provide easy backwards compatibility without being burden
with static checks. In our formalization, equivalence is structural and
subtyping of recursive types is supported. The following three expressions
illustrate the language. The first expression is ill-typed because the
receiver is of type \C, so has no method \n. The second expression will also
be flagged as erroneous as the argument to \m is \D rather than a \C as
expected. The last expression is statically correct, as the instance of \D is
cast to a \C via method \mp, and, since \m uses none of the functionality of
\C, it will execute without error.
...
\begin{tabbing}
  \hspace{.5cm}
   ...
   \> \Call{\New\C{}}\m{\New\D{}}                                  \>  \HS  \Mdef\m\x\C\C{~\this~} \\
   \> \Call{\New\C{}}\m{\Call{\New\C{}}\mp{\New\D{}}}              \>  \HS  \Mdef\mp\x\any\C{~\x~}\\
   \>                                                              \>  \}   \\
   \>                                                              \> \Class\D{}{}
\end{tabbing}}


\noindent This last case illustrates the soundness guarantee of the optional
semantics, as we are able to take a value that supports no operations, then
use it in place of one that exposes methods, methods that will not exist if
used. As a result, while the optional semantics does provide type checking,
it provides the same guarantees as an untyped language would.

The translation from the optional source language to \kafka
...
\noindent
Observe that the optional translations does insert some structural casts
...
%All optional types are represented with the dynamic \kafka type, \any, and
...
%\(\kty\t\) be a mapping from source types to \kafka types -- for the optional semantics
...
foster program evolution.  

%Convertibility thus includes a \(\dt\C\Mapsto\C\)
...
In Thorn, any method can be called statically and dynamically. For
...
and the other is optionally typed (or dynamic) object. As an example, 
the above translates to:

%More precisely, given a Thorn method
...
that of TypeScript. To illustrate the semantics of Typed Racket, 
consider the following example:

%The translation, shown in \figref{trtr} maps classes to
...
%TODO: Do we really need this paragraph?
...
\begin{tabular}{ll}
\multicolumn{1}{c}{\footnotesize{\textbf{Source}}} & \multicolumn{1}{c}{\footnotesize{\textsf{\textbf{KafKa}}}} \\
\footnotesize
\(
\begin{array}{l@{\,}l}
...
\subsection{Translation discussion}
...
%% Going by x, this, and field assignment -- compare them individually and not as a group.
\figref{fig:travar} presents the translation for field access and variables. For the
most part, these operations are translated verbatim, with notable exceptions in the transient 
and optional semantics.

In the optional semantics, in order to produced well-typed \kafka terms, we
need to eliminate the type on one of the two typed introduction forms, \this,
which is accomplished by casting to \any. Otherwise, the optional semantics
makes no alterations to these terms, as it has no soundness guarantee to
ensure.

Transient does not guarantee that the values passed to functions or assigned
to fields respect the declared type of those functions or fields. Class
translation inserts checks for invalid variables upon method entry, so while
the transient translation has to insert casts to appease the \kafka type
system, these casts will never fail. However, fields are not protected such,
and values can  be passed through them that violate their types, requiring a
guard after dereference. 
...
Field assignment illustrates the continuum of strength provided by each
gradual type system. Under the unsound optional semantics, we simply translate
the assignment directly, with no guards or additional checks. However, in the
three sound semantics, the behaviour differs depending on the strength of the
soundness guarantee.

Under the transient semantics, assignment to a field is unchecked, instead
deferring type checking to dereference time. As a result, we use the analytic
translation operator to translate the argument according to the transient
semantics, but targeting type \any, instead of the known type of the field,
enabled by the class translation erasing field typing. Under the behavioral
and concrete semantics, which do guarantee heap type consistency, however,
we translate the argument expression targeting the known type of the field.

One observation of this figure is that the other two sound translations demand
the correct type to be returned from the argument expression, while the
transient semantics only needs it to be well-formed. As a consequence, the two
stronger sound type systems will raise an error when set tries to assign an
value of the wrong type to the field, whereas the transient semantics raises
the error upon dereference.
...
\figref{fig:tranew} presents the translation for the object creation expression. 
...
only difference lies in which recursive translation to call for the arguments of the object, 
whereby each semantics call their respective translation. The optional semantics 
require an additional any cast to ease any typing information from the object.
%TODO: Need to talk more about comparing the semantics.
...
Figure~\ref{fig:trtype} depicts the type requirement translation mode. Under
this translation, we are given the expression to translate, the environment
which it should be translated against, and the \kafka type of the resulting
expression. Under the optional system, all terms have type \any, rendering its
type requirement translation useless.

In the sound systems, however, all differentiate between the case where the
statically-known type is compatible with the required type, and the case where
something has to be done to convert the statically-known type to that required
by the invoking expression. The main differentiation comes from in what each
system then does when confronted with a difference in types.

The transient and concrete semantics use the standard dynamic subtype test,
but while concrete uses the classes as defined by the programmer, the
...
in functions and field, causing the subtype test cast in the context of the
transient class translation to simply check for the existence of a method of
the required name during dynamic subtype testing, rather than checking that
the entire class type is compatible.
...
\begin{figure}    
  \begin{tabular}{llc@{\hspace{.25cm}}l}    
    ...
    & \HS\HS\HS\HS\HS\HS\HS\HS\HS \HS\HS\HS\HS\HS\HS\HS\HS\HS\HS\HS \textbf{\IF} {\t[1]} = \any \\    
...
In the concrete source language, any method can be called statically and    
dynamically. For this to work out it is necessary to generate a dynamic   
version of all concrete source methods.  Class translation relies on    
overloading to provide two implementations of each method: one is used with   
concrete receiver and the other is optionally typed (or dynamic) object.  More    
precisely, given a Thorn method \(\md\) with type \(\t[1] \to \t[2]\), the    
\kafka translation first generates a method \(\mdp\) with type \(\kty{\t[1]}    
...
For an example of how translation drives semantics, consider the key class
{\xt F} in Litmus Test 3, shown in figure~\ref{litmus}. We will translate
class {\xt F}'s definition using each of the four translations, clearly
indicating where the behavior of the program comes from.
...
Translation under the optional semantics is extremely simple - the types are
erased and all invocation is dynamic. When we introduce soundness, however,
translation becomes much more complex.

The first sound, and weakest, gradual type system we consider is the transient
semantics. Under the transient semantics, all types are erased from the class
- as argument and return types are not guaranteed. Since transient guarantees
that all the required methods will exist on a type (if not their types), we
can make a statically typed invocation to the untyped method \m, but have to
pass it untyped arguments, and test its return type before returning it again
as an \any.

The weak guarantee provided by the transient semantics means that we are not
guaranteed the return type of any methods, and cannot trust the given argument
type. However, the behavioral semantics recovers these properties, by using
the behavioral cast operator. By inserting wrappers, the behavioral system
allows methods to trust declared argument and return types, enabling the call
to \m to have known types. However, the cost of this is that the behavioral
cast is a complex and expensive operation.

Like the behavioral semantics, the concrete semantics allows the use of a
typed version of \m, and has similarly simple semantics to the transient
version, as only values that have statically been checked to be safe are
allowed to pass casts. However, the similarity of the concrete translation to
the transient - at least in principle, using the same subtype cast operator -
is marked. How is the concrete translation able to provide a much stronger
guarantee, while using the same cast?

The reason why is illustrated if we show the translation of {\xt E} in the
four systems, seen in figure~\ref{fig:l3etrans}, which  drives the semantics
of the subtype cast in particular.
...
In the transient semantics, when \x is cast to {\xt E}, all of the types on
{\xt E} have been erased by the transient class translation, so casting to
{\xt E} is tantamount to asking if the method \m exists at all. In contrast,
the concrete semantics retained all of the types on \m, so in the concrete
translation, checking if a class is a subtype of {\xt E} is equivalent to
checking of method \m exists \emph{taking and returning an {\xt E}}.

While this allows the concrete semantics to provide a strong type guarantee,
it comes at the cost of the ability to easily migrate code from untyped to
typed. If we were to suppose that both the optional and concrete versions of
{\xt E} existed (under different names) in the same system, only the concrete
version of {\xt E} could be used with the concrete {\xt F}, as despite
implementing the same behavior, the optional version of {\xt E} would not be
able to be used with the concrete {\xt F}, as it is not statically known to
implement an \m taking and returning an {\xt E}.

The behavioral semantics is able to use the same representation for {\xt E} as
the concrete semantics while still allowing the optional {\xt E} to be used
because it uses the much more complex behavioral cast. In effect, if an
optional {\xt E} were to be passed through where a behavioral {\xt E} was
expected, then a wrapper, implementing the fully typed {\xt E}, would be
generated on top of the untyped, optional, {\xt E}, ensuring type safety.
...
\subsection{Transient (Reticulated Python)}
...
\figref{litmus} of section \ref{litmustest}. The source code are
...
The code for the litmus tests in Transient. \\
