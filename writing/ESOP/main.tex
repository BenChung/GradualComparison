...
semantics allows us to implement very different approaches to gradual typing
...
allow similar programs to go wrong.
...
with static checks. In our formalization, equivalence is structural and
 subtyping of recursive types is supported. The
...
\noindent
We illustrate the type system by focusing on the example above. 
For the first expression, the receiver has type \C, but the class does 
not have method \n. The second expression is ill-typed because the 
argument to method \m is of type \C and \C is not convertible to \D. 
The third expression is correct because the argument to \mp is of type
 \C and \C is convertible to \any, the expected type of \mp.
...
code. Grey background is used to identify translated terms. 
...
is correct, which is modeled in \kafka by having all argument types be
translated to \any. In the original presentation~\cite{siek14}, the transient
semantics  implemented this check on function entry, but due to the single-
expression construction of \kafka, we instead insert casts at every variable
use site.
...
\begin{figure}
	\begin{tabular}{llc@{\hspace{.25cm}}l@{\HS}l@{\HS}l}
		...
		{\scriptsize Optional} \\
		\TR[\OTS]{\Class\C{\fd[1]..}{\md[1].. }} & =  \src{\Class \C {\fdp[1]..}{\mdp[1].. } } & \WHERE & \fdp[1] = \src{\Ftype\f\any}..\HS\HS \fd[1] = \Ftype\f\t .. \\ 
		& & & \md[1] = \Mdef\m\x{\t[1]}{\t[2]}\e \\
		& & & \mdp[1] = \src{\Mdef\m\x\any\any\ep}..\\
		& & & \ep = \TR[\OTS]{\e} \\
		
		{\scriptsize Transient} \\
		\TR[\TTS]{\Class\C{\fd[1]..}{\md[1].. }} & =  \src{\Class \C {\fdp[1]..}{\mdp[1].. } } &
		\WHERE & \fdp[1] = \src{\Ftype\f\any} .. \HS
		\fd[1] = \Ftype\f\t ..\HS\HS \\
		& & & \mdp[1] = \src{\Mdef\m\x\any\any{\SubCast\t\x ~; ~\ep[1]}} .. \\
		& & & \md[1] = \Mdef\m\x\t\tp\e .. \\
		& & & \ep[1] = \TAG[\TTS]\e{\x:\t\,\this:\C}\tp~ .. \\		
		
		{\scriptsize Behavioral} \\
		\TR[\BTS]{\Class\C{\fd[1]..}{\md[1].. }} & =  \src{\Class \C {\fd[1]..}{\mdp[1].. } } & \WHERE & \mdp[1] = \src{\Mdef\m\x\t\tp{\ep[1]}} ..\HS\HS \\
		& & & \md[1] = \Mdef\m\x\t\tp{\e[1]} ..\HS\HS \\
		& & & \ep[1] = \TRG[\BTS]{\e[1]}{\x:\t\,\this:\C} \\
		
		{\scriptsize Concrete} \\
		\TR[\CTS]{\Class\C{\fd[1]..}{\md[1].. }} & = \src{ \Class \C{ \fd[1]..}{\mdp[1].. \mdpp[1]..}} & \WHERE & \md[1] = \Mdef\m\x{\t[1]}{\t[2]}\e .. \\
		& & & {\mdp[1]} = \src{\Mdef\m\x{{\t[1]}}{{\t[2]}}{\ep}} .. \\ 
		& & & \ep = \TAG[\CTS]{\e}{\this:\C\,\x:\t[1]}{\t[2]}~  ..\\        
		& & & {\mdpp[1]} = \src{\Mdef\m\x\any\any{\SubCast\any{\KCall\this\m{\SubCast{{\t[1]}}\x}{\t[1]}{\t[2]}}}} \\  & & & \HS\HS\HS\HS\HS\HS\HS\HS\HS\HS \textbf{\IF} {\t[1]} = \any \\
		& & & \HS\HS\HS\HS\HS\HS \src{empty} \\ 
		& & & \HS\HS\HS\HS\HS\HS\HS\HS\HS\HS\HS {\bf otherwise}  ..
	\end{tabular}
...
In concrete source language, any method can be called statically and dynamically. For
this to work out it is necessary to generate a dynamic version of all concrete source
methods.  Class translation relies on overloading to provide two
implementations of each method: one is used with concrete receiver
and the other is optionally typed (or dynamic) object. 
More precisely, given a Thorn method
\(\md\) with type \(\t[1] \to \t[2]\), the \kafka translation first
generates a method \(\mdp\) with type \(\kty{\t[1]} \to \kty{\t[2]}\)
(protecting the the return value with appropriate cast to \kty{\t[2]} if
...
\begin{figure}[!t]
	...
	\hrulefill
	\caption{Thorn subtyping}\label{subth2}
\end{figure}	
\begin{figure}[hb]	
	\hrulefill  \small
	\vspace{-3mm}
	...
	\hrulefill
	\caption{Thorn type convertibility}\label{convth2}
\end{figure}	
\begin{figure}[hb]	
	...
	\hrulefill
	\caption{Thorn type system}\label{thts2}
\end{figure}	
