...
guard after dereference. Consider the following program:

\begin{tabular}{cc}
\begin{tabular}{l@{}l}
\class&\hspace{0.3em}\C \{ \\
    &\Ftype\f{\xt{E}} \\
    &\Mdef{\xt{get}}\x{\xt{D}}{\xt{E}}{\FRead\f} \\
    &\Mdef{\xt{set}}\x\any\any{\FWrite\f\x} \\
\} \\
\class&\hspace{0.3em}\xt{E} \{ \Mdef\m\x{\xt{E}}{\xt{E}}\x \} \\
\class&\hspace{0.3em}\xt{D} \{ \} \\
\multicolumn{2}{l}{\xt{c} = \New\C{\New{\xt{E}}}} \\
\multicolumn{2}{l}{\Call{\xt{c}}{\xt{set}}{\New{\xt{D}}}} \\
\multicolumn{2}{l}{\Call{\xt{c}}{\xt{get}}{\New{\xt{D}}}} \\
\end{tabular} &
\begin{tabular}{l@{}l}
\class&\hspace{0.3em}\C \{ \\
    &\Ftype\f\any \\
    &\Mdef{\xt{get}}\x{\any}{\any}{\SubCast{\D}{\x}; \SubCast{\xt{E}}{\FRead\f}}\\
    &\Mdef{\xt{set}}\x{\any}{\any}{\SubCast{\any}{\x}; \SubCast{\any}{\FWrite\f\x}}\\
\} \\
\class&\hspace{0.3em}\xt{E} \{ \Mdef\m\x{\any}{\any}{\SubCast{\xt{E}}{\x};\SubCast{\xt{E}}{\x}} \} \\
\class&\hspace{0.3em}\xt{D} \{ \} \\
\multicolumn{2}{l}{\xt{c} = \New\C{\SubCast{\any}{\New{\xt{E}}{}}}} \\
\multicolumn{2}{l}{\SubCast\any{\Call{\xt{c}}{\xt{set}}{\SubCast{\any}{\New{\xt{D}}{}}}}} \\
\multicolumn{2}{l}{\SubCast{\xt{E}}{\Call{\xt{c}}{\xt{get}}{\SubCast{\any}{\New{\xt{D}}{}}}}} \\
\end{tabular}
\end{tabular}

This is well-typed, according to the surface type system, as the type-
violating operation is untyped, when set is called with a value of type \any.
Moreover, the get operation is well-typed itself, because \f has type \xt{E},
so dereferencing would seem to be safe. However, owing to the weak soundness
guarantee of the transient semantics, no dynamic check is done on assignment
of an untyped value to a typed field. As a result, we are dynamically allowed
to assign an instance of \D to \f, requiring a check at dereference time to
avoid returning a value of the wrong type.
...
Notably, this last difference is what causes the behavioral semantics to
differ from the transient semantics in litmus test 3. Consider the
translations of class \xt{F}.
...
The argument to the function \m is declared to be of type \xt{E}, so the
translation mechanism will insert casts as needed to achieve this type. The
transient semantics only needs to ensure an \any argument is passed, so only
inserts an \any cast, while checking the result with a cast to \xt{E}, using
subtype casts throughout. The behavioural semantics needs to guard the both
the argument and return types, and inserts behavioural casts to meet this
requirement. 

When the input, an instance of class \C, is passed through the transient
translation of \n, no changes will be made to the underlying object, as the
subtype cast operation simply returns the same value as it was passed if the
cast succeeds. In contrast, when given to the behavioral translation, it will
be wrapped inside a behavioral wrapper guaranteeing the \xt{E} type, which is then
retained.

As a result, when litmus test 3 then calls \xt{a} on the return and it returns
a \xt{D}, instead of the expected \xt{C} (due to the different underlying
value), the behavioral wrapper is now in place to observe this violation of a
now-gone type contract, while the transient semantics observes no anomalous
behavior at this point, as the new return type is completely consistent with
