...
available at \emph{https://github.com/BenChung/GradualComparison/tree/master/impl}. In Coq syntax, the
...
to the Coq functions \verb|HasType| and \verb|WellFormedState| respectively. 
...
morekeywords={forall, exists, $\forall$}, keywordstyle={\ttfamily\color{blue!80}}, escapeinside={--(}{)--}, mathescape=true]
    (*case 4*) ($\exists$ (t$'$:type) (a:ref) (C:id) (aps:list ref),
       ...
       --(\emph{In})-- (a, --(\emph{HCell})--(C,aps)) s $\wedge$
       ((--(\emph{Subtype})-- empty_mu k (--(\emph{class})-- C) t$'$) -> False)) $\vee$
\end{lstlisting}
\begin{description}
  \item[Case 4] The program could have gotten stuck on a subtype cast. 
In an expression of the form \texttt{SubCast t$'$ (Ref a)}, the program
...
morekeywords={forall, exists, $\forall$}, keywordstyle={\ttfamily\color{blue!80}}, escapeinside={--(}{)--}, mathescape=true]
    (*case 5*) ($\exists$ a aps C C$'$, 
       e = equivExpr (BehCast (class C') (Ref a)) E $\wedge$
       In (a, HCell(C, aps)) s $\wedge$
       ~(incl (method_names C$'$ k) (method_names C k))))
\end{lstlisting}
\begin{description}
  \item[Case 5] The program may get stuck on a behavioral cast, of the form
\verb|BehCast| \texttt{(class C$'$) (Ref a)}, if \verb|C|, the class of \verb|a| in
