...
\begin{equation*}
\methdef\a\s\K = \b\md\begin{cases}
...
\begin{equation*}
\classof\a\s\K = \classoff\C\K \begin{cases}
...
\begin{equation*}
\classoff\C\K = \Mxt{MT} \begin{cases}
...
\opdef{\convert{\md} = \mt}{Method definition ($\md$) to method type ($\mt$) conversion function}
...
\opdef{\convertFD{\Ftype{\f}{\t}} = (\Mtype\f\t\t, \Mtype\f{}\t)}{Field definition to field methods conversion function}

% \convertFD{\Ftype{\f}{\t}} = (\Mtype\f\t\t, \Mtype\f{}\t)

\hrulefill

\opdef{\names{\mt} = \x}{Naming of method types}
...
\opdef{\names{\md} = \x}{Naming of method definition\footnote{Some of these should go into the appendix.}}
...
\wm{(\xt{wrap},\Mdef\m\x{\t_1}{\tp_1}\e,\Mtype\m{\t_2}{\tp_2})} &\text{\;\emph{where}\;} \t_1 \sim \t_2 \wedge \tp_2 \sim \tp_1\\
...
\>\>\>$\emph{for every } \Mdef\m\x{\t_1}{\tp_1}\e \in \C \wedge \Mtype\m{\t_2}{\tp_2} \in \Cp \wedge \t_1 \sim \t_2 \wedge \tp_1 \sim \tp_2 $\\
