...
\opdef{\BasicType \Sk {$~\Class \C {\b{\Ftype\f\t}}{\b\md} \OK$}}{class well-formedness [TODO]}
\begin{mathpar}
\IRule{WFClass}{
}{
 \BasicType \Sk {\Class \C {\b{\Ftype\f\t}}{\b\md} \OK}
}
\end{mathpar}

\hrulefill

\opdef{\BasicType \Sk {$~\md \OK$}}{method well-formedness [TODO]}
\begin{mathpar}
\IRule{WFMeth}{
}{
 \BasicType \Sk {\Mdef\m\x\t\t\e \OK}
}

\IRule{WFFieldS}{
}{
 \BasicType \Sk {\Mdef\f\x\t\t\e \OK}
}

\IRule{WFFieldR}{
}{
 \BasicType \Sk {\Mdefz\f\t\e \OK}
}
...
\opdef{\methdef\a\s\K}{method definition auxiliary function}

\begin{equation*}
\methdef\a\s\K = \b\md ~~s.t.~~ \begin{cases}
...
\opdef{\classof\a\s\K}{type definition auxiliary function}
...
\opdef{\readf \s\a\f}{field read auxiliary function}
...
\opdef{\setf \s\a\f\ap}{field write auxiliary function}
...
\setf \s\a{\f_{i}}\ap = \sp ~~s.t.~~ \begin{cases}
 ...
 \s(\a) = \obj\C{\dots \a_{i} \dots}\\
 \K(\C) = \Class \C {\dots \Ftype{\f_{i}}{\t_{i}} \dots}{\b\md}\\
 \sp = \s[\Bind{\a}{[\ap/\a_{i}]\obj\C{\dots \a_{i} \dots}}]
\end{cases}
\end{equation*}

\hrulefill

\opdef{\convert{\md}}{Method definition ($\md$) to method type ($\mt$) conversion function}
...
\opdef{\convertFD{\Ftype{\f}{\t}}}{Field definition to field methods conversion function}

\convertFD{\Ftype{\f}{\t}} = (\Mtype\f\t\t, \Mtype\f{}\t)

\hrulefill

\opdef{\names{\mt}}{Naming of method types}
...
\opdef{\names{\md}}{Naming of method definition\footnote{All of these simply functions should go into the appendix.}}
...
\>$\xt{class}~\D$\=$~\{$\\
\>\>$\xt{that} : \C$ \\
\>\>$\wm($\=$\getmdef(\m,\C),\getmtype(\m,\Cp))$ $\emph{for every } \m \in \C \wedge \m \in \Cp $\\
\>\>$\Mdef\m\x{\t_1}{\tp_1}{\this.\xt{that}.\m(\x)} \text{ \emph{for every} } \Mtype\m{\t_1}{\tp_1} = \getmtype(\m,\C) \wedge \m \not\in \Cp$\\
\>\>$\Mdefz\f\t{\this.\xt{that}.\f()} \text{ \emph{for every} } \f:\t \in \Cp \vee \Mtype\f{}\t \in \Cp$\\
\>\>$\Mdef\f\x\t\t{\this.\xt{that}.\f(\x)} \text{ \emph{for every} } \f:\t \in \Cp \vee \Mtype\f{\x:\t}\t \in \Cp$\\
\>$\}$\\
$\wc(\Heap\s{$\=$\a\mapsto \{ \b{a}\B\C\} }, \a, \any) = $\\
\>$\xt{class}~\D$\=$~\{$\\
\>\>$\xt{that} : \C$ \\
\>\>$\Mdef\m\x{\any}{\any}{\BehCast{\any}{\this.\xt{that}.\m(\BehCast{\t_1}{\x})}} \text{ \emph{if} } \Mdef\m\x{\t_1}{\tp_1}{\e} \in \C$\\
\>\>$\Mdefz\f\any{\BehCast{\any}{\this.\xt{that}.\f()}} \text{ \emph{for every} } \f:\t \in \C \vee \Mtype\f{}\t \in \C$\\
\>\>$\Mdef\f\x\any\any{\BehCast{\any}{\this.\xt{that}.\f(\BehCast{\t}\x)}} \text{ \emph{for every} } \f:\t \in \C \vee \Mtype\f{\x:\t}\t \in \C$\\
\>$\}$\\
\end{tabbing}
