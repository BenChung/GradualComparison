...
\CondRule{E5}{ %% a.f() -> e
    \Mdefz\f\t\e \In \method{\s(\a)_{\downarrow\C}}{\K} 
}{
   \ReduceA \K{\Get\a\f}\s \K{[\a/\this]\e}\s
}
\CondRule{E4}{ %% a.f(a) -> e
    \Mdef\f\x\t\t\e \In \method{\s(\a)_{\downarrow\C}}{\K} 
}{
...
\CondRule{E3}{ %% a.m(a) -> e
  \Mdef\m\x\t\tp\e \In \method{\s(\a)_{\downarrow\C}}{\K}  $\wedge~ \t \not= \any$
}{
 \ReduceA \K{\Call\a\m\ap}\s \K{[\a/\this~{\ap/\x}]\e}\s
}
\CondRule{E3}{ %% a@m(a) -> e
    \Mdef\m\x\any\any\e \In \method{\s(\a)_{\downarrow\C}}{\K} 
}{
  \ReduceA \K{\DynCall\a\m\ap}\s \K{[\a/\this~{\ap/\x}]\e}\s
}
\CondRule{E10}{  %% Subtypecast
   \BasicType {\none} {\classoff{\s(\a)_{\downarrow\C}}{\K} \Sub \t} $~\vee~$ \t $= \any$
}{ 
  \ReduceA \K{\SubCast \t\a}\s \K\a\s
}
\CondRule{E11}{
 \names{\classoff{\s(\a)_{\downarrow\C}}{\K}}  $\supseteq$  \names\t
}{ 
...
\>\>$\wm(\methz\m\C\K,\getmtype(\m,\Cp,\K))$ \\
...
\>\>\>$\tfa (\f:\t \in \Mxt{meth\C} \vee \Mtype\f{}\t \in \Mxt{meth\C})$\\
\>\>\>\>$\wedge~~ (\f:\tp \not\in \Mxt{meth\Cp}  \wedge \Mtype\f{}\tp \not\in \Mxt{meth\Cp} )$\\
\\
...
\>\>\>$\tfa (\f:\t \in \Mxt{meth\C} \vee \Mtype\f{\x:\t}\t \in \Mxt{meth\C})$\\
\>\>\>\>$\wedge~~ (\f:\tp \not\in \Mxt{meth\Cp}  \wedge \Mtype\f{\x:\t}\tp \not\in \Mxt{meth\Cp} )$\\
\\
...
\>\>\>\>\qquad $\text{ \emph{if} } \Mdef\m\x{\t_1}{\tp_1}{\e} \in \method\C\K$\\
\>\>$\Mdefz\f\any{\BehCast{\any}{\this.\xt{that}.\f()}}$ \\
\>\>\>\>\qquad $\text{ \emph{for every} } \f:\t \in \field\C\K ~~\vee~~ \Mtype\f{}\t \in \classoff\C\K$\\
\>\>$\Mdef\f\x\any\any{\BehCast{\any}{\this.\xt{that}.\f(\BehCast{\t}\x)}}$ \\
\>\>\>\>\qquad $\text{ \emph{for every} } \f:\t \in \field\C\K ~~\vee~~ \Mtype\f{\x:\t}\t \in \classoff\C\K$\\
\>$\}$\\
\end{tabbing}
