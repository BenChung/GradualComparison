...
\opdef{
  ...
  Specializing $\t_1$ with $\t_2$ produces $\t$, with assumptions \P and classes \K, producing classes 
  $\K'$
}
\begin{mathpar}
...
\>$\xt{class}~\D$\=$~\{$\=\\
\>\>$\f : \t_1$\\
\>\>\>$\tfa \f : \t_1 \in \classoff\C\K \wedge \f:\tp_1 \not\in \typez{\t}$\\
\\
\>\>$\f : \tp_1$\\
\>\>\>$\tfa \f : \t_1 \in \classoff\t\K \wedge \f:\tp_1 \in \typez{\t}$\\
\\
\>\>$\Mdefa\n{\b{\HT\x{\any}}}{\any} { \MonCast{\any}{\this.\n(\b{\MonCast{\t_1}{\x}})}}$\\
\>\>\>$\tfa \Mtype\n{\b{\t_1}}{\tp_1} \in \classoff\t\K$\\
\\
\>\>$\Mdefa\m{\b{\HT\x{\t_1}}}{\tp_1} { \MonCast{\tp_1}{\e}}$\\
\>\>\>$\tfa \Mtype\m{\t_1}{\tp_1} \in \classoff\t\K$\\
\\
\>\>$\Mdefz\f{\t_1} { \this.\f }$\\
\>\>\>$\tfa \Mtype\f{}{\t_1} \in \classoff\t\K$\\
\\
\>\>$\Mdefa\f{{\HT\x{\t_1}}}{\t_1} { \this.\f = \x }$\\
\>\>\>$\tfa \Mtype\f{\t_1}{\t_1} \in \classoff\t\K$\\
\>$\}$
\end{tabbing}
