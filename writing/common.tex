...
\IRule{Cast-Mono (monotonic)}{
  \rectype{\cdot, \cdot, \s, \a, \t} = \Sigma,\Omega\\
  \spec{\Sigma, \Omega, \K,\s} = \sp,\Kp
}{
  \moncast \a\t\s\K   \Kp {\sp}\\
}
\end{mathpar}
...
\IRule{RC1}{
  ...
  \Sigma,\Omega,\K \vdash \meet{\tp}{\t} \equiv \tpp \dashv \Omega~\Omega' \\
  \tp \neq \tpp \\
  \fieldtypes{\tpp,\K,\s,\a} = \b\ap,\b{\t_f}\\
  \b{\rectype{\meet\Sigma{\ap:\tpp},\Omega,\sigma,\ap,\t_f} = \Omega~\Omega''}
}
{\rectype{\Sigma, \Omega, \sigma, \a, \t} = \left(\bigsqcap\b{\Sigma'}\right),\Omega~\Omega'~\b{\Omega''}}

\IRule{RC2}{
  \htype{\a,\Sigma,\sigma} = \tp\\
  \emptyset,\Omega,\K \vdash \meet{\tp}{\t} \equiv \tpp \dashv \Omega\\ % is this Omega right?
  \tp = \tpp \\
}{
  \rectype{\Sigma,\Omega,\sigma, \a, \t} = \Sigma~\Omega
}
\end{mathpar}

\begin{mathpar}
\IRule{SP1}{\D \text{ Free}\\
%             \classgen{\C,\t} = \D \\
            \K' = \K~\D\mapsto\classgen{\C,\t, \D} \\
            \spec{\Sigma,\sigma,\K} = \sigma',\K'}{
            \spec{\a : \t ~ \Sigma, \sigma[\a \mapsto \C\{\b{\ap}\}], \K} = \sigma'[\a\mapsto \D\{\b\ap\}]~\K'
            }


\IRule{SP2}{}{
            \spec{\cdot, \cdot, \K} = \cdot~\cdot
            }
\end{mathpar}
...
\begin{mathpar}
\IRule{M1}{ }{\Phi~\Omega~\K \vdash\meet{\t}{\any} \equiv \t\dashv\Omega}

\IRule{M2}{ }{\Phi~\Omega~\K \vdash\meet{\any}{\t} \equiv \t\dashv\Omega}

\IRule{M3}{ }{\Phi~\Omega~\K \vdash\meet{\t}{\t} \equiv \t\dashv\Omega}

\IRule{M4}{
      \b{\Phi~\Omega~\K\vdash\meet{\t_3}{\t_1}  \equiv \t_5\dashv \Omega~\Omega'} \\
    \Phi~\Omega~\K \vdash \meet{\t_2}{\t_4} \equiv \t_6\dashv \Omega~\Omega''\\
    \Phi~\Omega~\K \vdash \meet{\Type{\b{\mt_1}}}{\Type{\b{\mt_2}}} \equiv \Type{\b{\mt_3}} \dashv \Omega~\Omega'''
}{
  \Phi~\Omega~\K \vdash \meet{\Type{\Mtype\n{\b{\t_1}}{\t_2}~ \b{\mt_1}}}{\Type{\Mtype\n{\b{\t_3}}{\t_4} ~ \b{\mt_2}}} \equiv \Type{\Mtype\m{\b{\t_5}}{\t_6} ~ \b{\mt_3}}\dashv\Omega~\b{\Omega'}~\Omega''~\Omega'''
}

\IRule{M5}{ 
}{ \Phi~\Omega~\K \vdash \meet{\Type{\b\mt}}{\Type{}} \equiv \Type{\b\mt} \dashv \Omega}

\IRule{M6}{ }{
  \Phi~\Omega~\K \vdash \meet{\Type{}}{\Type{\b\mt}} \equiv \Type{\b\mt} \dashv \Omega
}

\IRule{M7}{
  \meet{\C}{\D} \not\in \text{dom}(\Phi)\\
  \Phi~\meet{\C}{\D}={\Mxt E}~\Omega~\K \vdash \meet{{\text{typeof}}(\C,\K)}{{\text{typeof}}(\D,\K)} \equiv \t \dashv \Omega'
}{
  \Phi~\Omega~\K \vdash \meet{\C}{\D} \equiv {\Mxt E} \dashv \Omega'~{\Mxt E}\mapsto\t
}

\IRule{M8}{
  \Phi(\meet{\C}{\D}) = \Mxt E
}{
  \Phi~\Omega~\K \vdash \meet{\C}{\D} \equiv {\Mxt E} \dashv \Omega
}
\end{mathpar}
% 
% \hrulefill
...
\>$\xt{class}~\D$\=$~\{$\=\\
\>\>$\f : \tp$\\
\>\>\>$\tfa \f : \t \in \classoff\t\K \wedge \f:\tp \in \typez{\t}$\\
\\
