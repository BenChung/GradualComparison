...
$\t \Rightarrow \t$ - safe, subtype test.

$\t \Rightarrow \any$:

Requirement: typed arguments are protected from unsafe use.
...
Requirement: Methods which appear in the type are forced to behave like described. Methods that do not appear are retained.
...
$\any \Rightarrow \any$: identity.

Compositions: Collapse redundancies, leaving

$\t \Rightarrow \any \Rightarrow \t$:

Apply above description. Let \C be start, \D be next, \Cp be final. Assume that \t same.

For every typed \m in \C, generate untyped \m in \D, which calls \C.\m with casted arguments and casts the return type.

For every untyped method \m in \D which has a corresponding \m in \t (which is all of them, in this case), generate a typed \m in \Cp, which casts the arguments and casts the return type.

An unrolled call to \m(\tp1):\tp2 on \Cp would then look like, letting \C be the terminal value:

\BehCast{\tp2}{\BehCast{\any}{\Call{\C}\m{\BehCast{\tp1}{\BehCast{\any}{\x}}}}}

$\any \Rightarrow \t \Rightarrow \any$

Let \C be the start, \D be next, \Cp be final.

For every \m in \C whose type is asserted in \t, add a typed \m in \D which casts its arguments to any and its return value to the correct type, then passes them along to \C.\m. For any \m in \C whose type is not asserted in \t, add an untyped \m to \D which forwards directly.

For every typed \m in \D, add an untyped \m to \Cp which calls \D with arguments cast to the asserted types. For every untyped \m in \D, add a untyped \m to \Cp which calls \D.\m directly.

An unrolled call to \m(\any):\any (\t $ = \{\m(\tp1):\tp2\}$) on \Cp would look like

\BehCast{\any}{ \BehCast{\tp2}{\Call\C\m{\BehCast{\any}{\BehCast{\tp1}{\x}}}}}
