...
\CondRule{E10}{
  \cast \a\t\s\K{sub}  \K\a\s
}{ 
   \ReduceA \K{\SubCast \tp\a}\s \Kp\e\sp
}
\CondRule{E11}{
  \cast \a\t\s\K{sha}  \K\a\s
}{ 
   \ReduceA \K{\ShaCast \tp\a}\s \Kp\e\sp
}
\CondRule{E8}{
  \cast \a\t\s\K{beh}  \Kp\ap\sp
}{ 
    \ReduceA  \K{\BehCast \tp\a}\s \Kp\ap\sp
}
\CondRule{E9}{
  \cast \a\t\s\K{mon}  \Kp\ap\sp
}{ 
    \ReduceA \K{\MonCast\tp\a}\s \Kp\ap\sp
}
\CondRule{E1}{ %% e -> e'
...
\IRule{UTWrap}{
   \Class\C{\b{\Mdef\m\x{\tp1}{\tp2}\e}~\b{\Mdef{\m'_u}\x{\tp1}{\tp2}\e}}
    ...
    \wrapper{\C\Rightarrow\any,\D} = \Class \D{\xt{s}:\C}{\b{\Mdef{\m_\xt{u}}\x{\any}\any{\SubCast \any{\this.\xt{s}.\m(\b{\SubCast{\tp1}{\x}})}}} ~ \b{\Mdef{\m'_u}\x{\tp1}{\tp2}{\xt{s}.\m'_u(\b{x})}}}
}
