...
\begin{mathpar}
\IRule{EQ1}{ }{\J \vdash \t \approx \t }

\IRule{EQ2}{\t \approx \tp \in \J}{\J \vdash \t \approx \tp}

\IRule{EQ3}{ \J~\C \approx \D \vdash \typeof\C \approx \typeof\D }{\J \vdash \C \approx \D}

\IRule{EQ4}{\J\vdash \t_1 \approx \t_2 \\ \J \vdash \tp_1 \approx \tp_2}{\J \vdash \{\Mtype\n{\t_1}{\tp_1} ~ \b{\mt_1}\} \approx \{\Mtype\n{\t_2}{\tp_2} ~ \b{\mt_2}\}}
\end{mathpar}
...
\IRule{RC1}{
  ...
  \cdot \vdash \tp \not\approx \tpp \\
  \fieldtypes{\tpp,\K,\s,\a} = \b\ap,\b{\t^*}\\
  \text{update}(\a : \tpp, \Sigma) = \Sigma_1\\
  \K_1 = \K'\\
%   \b{\rectype{\ap, \t_f, \s, \K',\meet\Sigma{\a:\tpp}} = \Sigma'~\K''}
  \forall i \in 1~\ldots~|\b{\a'}|~.~\rectype{\ap_i, {\t^*}_i, \s, \K_i, \Sigma_i} = \Sigma_{i+1}~\K_{i+1}
}
{\rectype{\a, \t, \s, \Kp, \Sigma} = \Sigma_{|\b{\a'}|+1}, \K_{|\b{\a'}| + 1}}
...
\IRule{M7}{
  ...
  \K'' = \K'~\Mxt E\mapsto\classgen{\C,\t,\Mxt E,\K'}
}{
