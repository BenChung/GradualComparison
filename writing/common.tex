...
\IRule{RC1}{
  ...
  \emptyset,\Omega,\K \vdash \meet{\tp}{\t} \equiv \tpp \dashv \Omega' \\
  \tp \neq \tpp \\
  \fieldtypes{\tpp,\K,\s,\a} = \b\ap,\b{\t_f}\\
  \b{\rectype{\meet\Sigma{\ap:\t},\sigma,\ap,\t_f} = \Sigma'}
}
{\rectype{\Sigma, \Omega, \sigma, \a, \t} = \left(\bigsqcap\b{\Sigma'}\right),\Omega'}
...
\begin{mathpar}
\IRule{SP1}{\classgen{\C,\t} = \D \\
            \spec{\Sigma,\sigma,\K} = \sigma',\K'}{
            \spec{\a ~ \Sigma, \sigma[\a \mapsto \C\{\b{\ap}\}], \K} = \sigma'[\a\mapsto \D\{\b\ap\}], \K';\D
            }
...
\>$\xt{class}~\D$\=$~\{$\=\\
\>\>$\f : \tp$\\
\>\>\>$\tfa \f : \t \in \classoff\t\K \wedge \f:\tp \in \fieldtypes{\t,\K}$\\
\\
