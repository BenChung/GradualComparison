...
\begin{minipage}{8cm}
  \opdef{\Reduce \K\e\s \Kp\ep\sp}
        {\e\s evaluates to \ep or \err in a step}\\[-1mm]
...
\CondRule{E12}{
 \cast \a\t\s\K\_  \K\err \s
}{ 
    \ReduceA \K{\wCast \tp\a}\s\K\err\s
}
\CondRule{E13}{
    \Reduce \K\e\s\K\err\sp
}{
    \ReduceA \K{\Ctx\e}\s\K\err\sp
}
\end{tabular}\end{minipage}
