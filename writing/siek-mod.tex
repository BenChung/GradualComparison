...
e &::= k \B  e.\m(\bar{e}) \B e.\xt{f} \B e.\xt{f} = e \B  \ifthenelse{e}{e}{e}\B e + e \\
...
\mathit{CF} &::= \{\texttt{x}=v\ldots,\texttt{y}=\square,\texttt{z}=cv,\ldots,\texttt{f(a:}\tau\texttt{):}\tau'=e,\ldots\}\\
\end{align*}

\begin{mathpar}
\inferrule*[right=SubRefl]{ }{ \tau <: \tau }

\inferrule*[right=SubClass]{\texttt{class }\C\texttt{ extends }\xt{C}}{ \C <: \xt{D} }

\inferrule*[right=SubAny]{ }{\tau <: \anyt}
\end{mathpar}
...
\inferrule*[right=TNew]{\bar{\Gamma \vdash e : \tau \vee \Gamma \vdash e : \anyt} \\ \Gamma(C) = \{\ldots, \bar{\xt{f} : \tau}, \ldots\}}{\Gamma \vdash \NEW{\xt{C}}{\bar{e}} : C}

\inferrule*[right=TCast]{\Gamma \vdash e : \tau'}{\Gamma \vdash e\cast{\tau}}

\inferrule*[right=TClass]{ 
\bar{\Gamma \vdash D} \\  
\bar{\xt{m}'(\bar{\xt{x}:\tau''}):\tau' \in D \implies \xt{m}'(\bar{\xt{x}:\tau''}):\tau' \in C }\\
\bar{\xt{f}:\tau \in D \implies \xt{f}:\tau \in C}\\
\bar{\Gamma,\xt{this}:C,\bar{\xt{x}:\tau''} \vdash e : \tau'}\\
}{\Gamma \vdash \texttt{class } C \texttt{ implements } \bar{D}\;\{\bar{\xt{f} : \tau}, \bar{\xt{m}(\bar{\xt{x}:\tau''}):\tau' = \{ e \}}\}}
\end{mathpar}
...
\inferrule*[right=ENew]{\xt{class}\;C\{\bar{\xt{f}:\tau}, \bar{\xt{m}(\bar{\xt{p}:\tau}):\tau' = e}\}\\\mu'=\mu(a\mapsto\{\bar{f=v},\bar{\xt{m}(\bar{\xt{p}:\tau}):\tau' = e}\}) }{ \xt{new }C(\bar{v}),\mu \expreduce a,\mu'}

\inferrule*[right=EInv]{a\mapsto \{\ldots,\xt{m}(\bar{\xt{p}})=e\}]}{a.\xt{m}(\bar{v}),\mu \expreduce [a/\xt{this}][\bar{v/\xt{p}}]e,\mu}

\inferrule*[right=EField]{ \mu[a\mapsto\{\ldots,\xt{f}=v,\ldots\}] }{a.f,\mu \expreduce v,\mu}

\inferrule*[right=EAssign]{ \mu' = \mu[a\mapsto \{\ldots,\xt{f}=v,\ldots\}] }{a.f = v,\mu \expreduce v,\mu'}

\inferrule*[right=EIfZ]{ }{ \ifthenelse{\intv{0}}{e_1}{e_2},\mu \expreduce e_1,\mu }

\inferrule*[right=EIfNZ]{n\neq 0}{ \ifthenelse{\intv{n}}{e_1}{e_2},\mu \expreduce e_2,\mu }

\inferrule*[right=EPlus]{ n_1 + n_2 = n_3 }{ \intv{n_1} + \intv{n_2},\mu \expreduce \intv{n_3},\mu}
\end{mathpar}

Cast reduction:
\begin{mathpar}
\inferrule*[]{ e,\cstctx \castreduce e',\cstctx'}{F[e],\cstctx \castreduce F[e'],\cstctx'}

\inferrule*[]{ e,\cstctx\castreduce e',\cstctx' }{ CF[e],\cstctx \castreduce CF[e'],\cstctx'}

\inferrule*[]{ }{ v\cast{\iota},\cstctx \castreduce v,\cstctx}

\inferrule*[]{ I_1 \sim I_2 }{ v\cast{I_1!}\cast{I_2?},\cstctx \castreduce v\cast{I_1 \Rightarrow I_2},\cstctx}
...
State reduction:
\begin{mathpar}
\inferrule*[]{ e,\cstctx\castreduce e',\cstctx' }{ e,\cstctx \longrightarrow e',\cstctx'}

\inferrule*[]{ e,\mu\expreduce e',\cstctx }{ e,\mu \longrightarrow e',\cstctx}
