\documentclass{report}
\usepackage{mathpartir,amsmath,amssymb,xspace,listings,fullpage}


\input{common}
\newcommand{\spec}{\vartriangleright}
\begin{document}

Syntax:
\syntax

Consistency:

\begin{mathpar}
\inferrule*[lab=CNSym]{ }{\tau \sim \tau}

\inferrule*[lab=CNCons1]{ }{\any \sim \tau}

\inferrule*[lab=CNCons2]{ }{\tau \sim \any}

\inferrule*[lab=CNSub1]{\CLASS \C {\bar{\xt{D}}} {\ldots}}{\C \sim \xt{D}}

\inferrule*[lab=CNSub2]{\CLASS \C {\bar{\xt{D}}} {\ldots}}{\xt{D} \sim \C}
\end{mathpar}


Typing rules:

\begin{mathpar}
\inferrule*[lab=TCons]{ \tau \sim \tau' \\ \G \vdash e : \t }{\G \vdash e : \t'}

\TVar 

\TInt

\TUApp

\TApp

\TIf

\TPlus

\TNew

\inferrule*[lab=TClass]{ 
  \bar{\G \vdash \f : \t}\\
  \bar{\G \vdash \MTYPE{\m}{\bar{x:\t''}}{\t'} = e} \\ 
  \f:\t \in \xt{D} \implies \f:\t \in \C \vee \t = \any\\
  \MTYPE{\m}{\bar{x:\t_D''}}{\t_D'}\in \xt{D} \implies \MTYPE{\m}{\bar{x:\t_C''}}{\t_C'} \in \C \wedge (\t_C' = \t_D' \vee \t_D' = \any) \wedge \bar{(\t_C'' = \t_D'' \vee \t_D'' = \any)}\\
 }{
  \Gdash \CLASS \C{\xt{D}}{\bar{\f:\t},\bar{\MDEF\m{\bar{x:\t''}}{\t'}e}}
  }
\end{mathpar}

\wellformedness

\newcommand{\cicast}{\hookrightarrow}

\begin{mathpar}
\inferrule*[]{\Gdash e : \C \\ \CLASS \C {\bar{\xt{D}}} {\ldots}}{\Gdash e \cicast e' : \xt{D}}


\inferrule*[]{\Gdash e : \xt{D} \\ \CLASS \C {\bar{\xt{D}}} {\ldots}}{\Gdash e \cicast \cast{\xt{C}}e' : \C}
\end{mathpar}

\subsection{Cast Insertion}

\begin{mathpar}
\inferrule*[lab=CICons]{
  \Gdash e \tlate e' : \t \\
  \t \sim \t'\\
  \t \neq \t'
}{
  \Gdash e \tlate \cast{\t'}e' : \t'
}

\CICall

\CICallAny

\CIIf

\CIPlus

\CIVar

\CINew

\end{mathpar}


\section{Dynamic semantics:}



Reduction:

\begin{mathpar}
\EFrame

\ENew

\EInv

\EField

\EAssign

\EIfZ

\EIfNZ

\EPlus

\inferrule*[lab=ECast]{ \s' = \text{cast}(\s, v, \t) }{ \cast{\t}a,\s \ereduce a,\s'}
\end{mathpar}

\subsection{Casting}

\begin{mathpar}
\inferrule*[lab=CClass]{\s[a \is \{\bar{\f = v} \B \C\}] \\ 
\C \neq \C \spec \xt{D} \\ 
\s' = \s[a \is \{\bar{\f = v} \B \C \spec \xt{D}\}]\\
\f:\t \in \C\spec \xt{D} \implies \sigma'' = \text{cast}(\s', v, \t)}{\text{cast}(\s, a, \xt{D}) = \s''}

\inferrule*[lab=CCycle]{\s[a \is \{\bar{\f = v} \B \C,\bar{\xt{A}}\}] \\ \C = \bar{\xt{A}} \spec \xt{D}}{\text{cast}(\s, a, \xt{D}) = \s}

\inferrule*[lab=CInt]{ }{\text{cast}(\s,\xt{int}[n],\xt{\int}) = \s}
\end{mathpar}
\subsection{Meet}
\begin{mathpar}
\inferrule*[]{}{\t \spec \t = \t}

\inferrule*[]{}{\t \spec \any = \t}

\inferrule*[]{}{\any \spec \t = \t}

\inferrule*[]{B \not\in types(C)}{C \spec B = gen(C,B)}

\inferrule*[]{B \in types(C)}{C \spec B = C}
\end{mathpar}

\begin{lstlisting}
classMeet(A,B)
  A = class A {(*@\bar{\f_A:\t_A},\bar{m(\bar{x:\t''_A}):\t'_A = e_A}@*)}
  B = class B {(*@\bar{\f_B:\t_B},\bar{m(\bar{x:\t''_B}):\t'_B = e_B}@*)}
  return class AB {
    (*@\bar{\f:\t_A\spec\t_B}@*),
    (*@\bar{m(\bar{x:\t''_A\spec\t''_B}):\t'_A\spec\t'_B = \cast{\t'_A\spec\t'_B}\this.m'(\bar{\cast{\t''_A\spec\t''_B}x})}@*),
    (*@\bar{m'(\bar{x:\t''_A}):\t'_A = e_A}@*)
  }
\end{lstlisting}

Define $types(C) = \{A_1,\ldots,A_n\}$ such that $classMeet(A_1,classMeet(\ldots, A_n)) = C$

Define $gen(C, B) = classMeet(C,B)$

classMeet is applied to every string of classes, in left-to-right order
\end{document}
