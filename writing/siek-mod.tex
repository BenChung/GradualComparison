...
\mathit{CF} &::= \{\f=v\ldots,\f'=\square,\f''=cv,\ldots\B\t\}\\
\end{align*}
...
\inferrule*[right=ENew]{
     ...
     \s'=\s(a\is\{\bar{\f=v},\bar{ \m (\bar{x :\t}):\t' = e \B \t}\}) }{ \NEW \C {\bar v},\s \ereduce a,\s'}

\inferrule*[right=EInv]{a\is \{\ldots,\m(\bar x)=e \B \t\}, \ldots]}{a.\m(\bar v),\s \ereduce [a/\xt{this}][\bar{v/x}]e,\s}
...
\inferrule*[right=ECast]{ e\cast{\tau},\s \creduce e',\s' }{ e\cast{\tau},\s \ereduce e',\s'}
\end{mathpar}
...
\inferrule*[right=CREmpty]{\vdash v : \tau' \\ \tau' \sim \t} { v\cast{\t},\sc \creduce v,\sc}
 
\inferrule*[right=CRChain]{ v\cast{I},\sc \creduce v',\sc' }{ v\cast{\any}\cast{I},\sc \creduce v',\sc'}

\inferrule*[right=CRRef1]{
  \sc(a) = v : \t \\ 
  \t \sqcap \t' = \t''\\
  \t'' \neq \t\\
  v\cast{\t''},\sc \heapreduce v',\sc'\\
}{ a\cast{\t'},\sc \creduce a,\sc'(a\is v')}

\inferrule*[right=CRRef2]{
\s(a) = v : \t \\ \t \sqcap \t'
 = \t}{ a\cast{\t'},\sc \creduce a,\sc}

\inferrule*[right=CRHeap]{
  \sc(a) = co : \t\\
  co,\sc \heapreduce co',\sc'\\
  \sc'(a)_{rtti} = \t\\
 }{
  e,\sc \creduce e,\sc'(a \is co' : \t)
 }
\end{mathpar}
...
\inferrule*[right=HRObj]{
       ...
       \overline{v'\cast{\t_2},\sc \creduce v'',\sc'}\\
        o' =    \{\bar{\f= v''}, 
     \bar{\m (\bar x) = \this.\m'(\bar{ x \cast{\t_1''}}), \m'(\bar x) = e\cast{\t_2'}}\}
}{
   \{\bar{\f=v'}, \bar{\m(\bar x) = e} \B \t\}\cast{\t'},\sc \heapreduce o',\sc
}
\end{mathpar}
