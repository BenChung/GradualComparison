...
they all run without error.  Dynamic errors encounted in them when running
...
Translating a gradually typed language into a statically typed language,l such
as the JVM's bytecode or \kafka makes explicit which of the underlying type system's 
guarantees can be relied upon, and when dynamism is required.
...
source language and returning the corresponding \kafka term. The translation of
a term also entails the translation of all the classes needed to evaluate
it. Additionally, we provide a static type system to the four source languages, 
and define their dynamic semantics via translation.  While soundness of
...
the type system. TypeScript has \emph{optional types}; any well-typed
method call can get stuck, as every type is effectively \any. Thorn 
has \emph{concrete types}; a well-typed program will not get stuck at 
statically typed method calls. Typed Racket has \emph{promised types}; 
a well-typed program will not get stuck at a call to \m, because \x 
refers to an object, or wrapper, that implements \m. Wrappers may fail if 
...
objects \a).  Methods can be statically resolved, denoted by a typed
call \KCall\a\m\x\t\tp, or can be dynamically resolved, denoted by a dynamic
call \DynCall\a\m\x. The \kafka type system has two kinds of types; classes (
...
resolve cycles.\footnote{One subtle design choice was to privatize fields.  An
  earlier version of this paper materialized fields in types by adding
  getter and setter methods, adding substantial verbosity for little benefit. 
  This arose from \kafka's support for transparent wrappers, since, if fields are
  exposed, the wrappers must be able to interpose on field accesses. This feature 
  contributed substantial complexity to \kafka.} The
...
the subtype cast \SubCast\t\a checks if the object referenced by \a is a
subtype of type \t.  To support the Typed Racket translation, \kafka is
extended with a \emph{behavioral} cast operation, which creates wrappers that
enforce a specific type. This behavioral cast, \BehCast\t\a, ensures that
every method \m invoked on \a respects the type \t.\footnote{Neither the JVM or the
 CLR support behavioral casts natively, but the casts can be encoded by a
  combination of class generation and dynamic loading. To validate this claim,
  our implementation of \kafka in the CLR provides runtime support for behavioral 
  generation.} Protecting \a entails
runtime generation of a new class, interposing on calls to the target
object. We give \kafka an operational semantics and prove soundness of its
type system. With the exception of behavioral casts the proof is
straightforward.
...
of the occurrences of \any with more specific types. A single cast can propagate through
the heap as it implies casts on the objects reachable from the fields of the
...
\medskip\noindent{\bf Lemma: No duplicates.} Given $~\md[1].. \In \App\K\C~$ $ $and$~$ $\cload{\md[1]..}~$ 
and $~\names{\mdp[1]..} \subseteq \names{\md[1]..}$,
...
\medskip\noindent{\bf Lemma: Subtype presevation.} ~ Given $~\md[1]..\In\App\K\C~$ $ $and$ $ $~\mdp[1].. \In \App\K\Cp$ and 
...
compilation into \kafka is realised by a translation function that maps
...
importantly, cast operations.  When modelling the Thorn type system, the
...
we write $\vdash_{\!s}$ to emphasize that this is the source level typing (and
not the \kafka level typing).  As the various type system are mostly similar
we will only present their differences assuming that all the omitted rules
are identical.
...
For instance, \A's method \m expects a value of type \any and passes that
value down to the method \m of \D which expects a value of type \D.  The
translation will insert a behavioral cast from \any to \D at the call of
\D's \m.  Within the body of \D's \m, there is a call to \m, in our example
the value being passed is an instance of class \C which has a method \m with
interface \(\any \to \any\).  In order to protect class \D from the object
\C potentially not behaving as an object \D, Typed Racket inserts a wrapper
...
make sure that it acts as an object of type {\xt B}.  The generated \kafka
code for the example is:

{ \bf JAN: I don't really like the above text. }
...
\a, the object denoted by \a has a method \m. Of coure, if the type of \e is
...
In this example, we have two mutually incompatible classes \C and \D,
alongside potential consumers for \C and \D, \C itself and \code E,
alongside a conversion helper \code F. Here, we use \code F to acquire a
reference to \C as an \code E, which would imply that \C's \m has type \D to
\D, despite it actually having type \C to \C, then call (again, using
untyped code) it with a \C.

In Typed Racket, when we take the trip through the conversion helper \code F,
we pick up a wrapper that ensures that the argument to \m is always a \D and
that it always returns a \D. As a result, when we finally call \m with a \C,
the program would get stuck at a cast to \D. In Transient, however, the cast
to \code E is a no-op, so when we call \m with \C, no extra cast to \D is
encountered, and the

\noindent 
The Transient design allows method \m, which expects an instance of class \C
as argument, to be called with a value of type \any at any point; this
forces \m to check its arguments at every method invocation.
...
\section{Gradual Type System Shootout}\label{litm}

\epigraph{\small ``There is no perfection only life.''}

\vspace{-5mm}

One takeaway from our work is that there is no single approach to gradual
typing, and that the different choices have distinctive observable semantics.
To illustrate this, \figref{litmus} presents three small programs that can
easily differentiate between the four gradual typing approaches that we
describe. Each of the litmus tests consists of a class table and a main
expression, as is typical of \kafka programs, and behaves differently
depending on the gradual semantics chosen.
...
are sufficient to distinguish the four type systems being studied in this
...
completion without getting stuck. With Thorn, litmus tests 1 and 2 fail 
at a structural cast from \code A to \code I.  
For the other languages the situation is as follows:

\begin{itemize}
\item {\bf L1:} Fails every non-TypeScript language because \code A is not a subtype of \code I, the
  failure is at a subtype cast because \code A does not have all the methods of
  \code I.   
  \FZ{CHECK, we do not have shallow casts, is this still relevant?} yes, 
  this is still relevant.
  
\item{\bf L2:} Succeeds in Transient and Typed Racket because subtype casts
  of \code A to \code I go through. \FZ{CHECK, we do not have shallow casts, is this still relevant?  Cite monotonic here?}
  I'm not sure this is a good place to talk about monotonic, or talk about monotonic at all.
 
\item{\bf L3:} Typed Racket fails because type $\C$ cannot be casted to 
a type $\D$ in the call of method $\m$ shown in program 3. While the call of method $\n$ eases 
the type $\C$ to the type $\any$, allowing for the call to method $\m$.

  
 \item{\bf L4:} 
%  Fails in Monotonic because the object refereed to by \code x
...
{\bf Typed Racket does not fail this test. Consideration for removing this test.}
  \end{itemize}
...
In general gradual typing has matured beyond being an experimental language 
feature in the petri dish of language research in academia.
Real-world applications are now written with languages incorporating 
gradual types, further propelling the tug of war between soundness and performance
in yet another aspect of language design principles. To gain a clear picture
of the feasibility of gradual typing. It is the responsibility of language researchers to
present a clear explanation of each available gradual typing idiom.

We introduced \kafka, a formal language that serves as the foundation
for deconstructing several existing gradual typing systems: Typescript, Thorn,
Transient Python, and Typed Racket.
\kafka offers an opportunity to compare and contrast each gradual typing systems
within an unified framework. the essences of each system are highlighted in the 
translation from each gradual typing system to \kafka.

A big takeaway from constructing \kafka is understanding that gradual typing systems
are not monomorphic in their design or principles. Instead they vary, and
differentiate in what they can write, express, and even in what can and cannot 
be evaluated dynamically. There does not seem to be any right answer or 
the one true answer for gradual typing. Based on need and different use cases.

From Typed Racket, we extracted the principle that every value
is inherited of it's declared type. In \kafka this manifested itself as the types
of values are enforced before any call can be performed. Conversely, in Transient 
the method being called is tasked with ensuring the arguments have the correct types.
TypeScript with no dynamic behaviour, providing an excellent entry point for understanding
the migration of concrete types to optional types in a Thorn program.

The strength of \kafka lies in the simplicity and flexibility in it's used of casts and
wrapper generation. New gradual typing systems can be introduced via a new cast semantics.

Future work involves simply creating new casts and develop semantics for the
interactions between casts.
...
\section{Litmus tests in original language}
...
\begin{lemma}{Substitution}{}
  \begin{conds}
    \cond{$\EnvType{\b{\HT\x\tp}}\s\K\e\t$}\lbl{cond1}    
    \cond{$\b{\EnvType\cdot\s\K\a\tp}$}\lbl{cond2}
  \end{conds}
