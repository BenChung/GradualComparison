...
\begin{equation*}
\getmds\C\K = \md[1].. ~~\mathit{if}~~ \Class\C{\fd[1]..}{\md[1]..} \in \K
\end{equation*}
...
If $\WFp \K$ (TODO), if $\C \in \K$, $\Cp \in \K$, $\D$ free, $\md[1].. = \getmds\C\K$, $\mt[1].. = \classoff\C\K$, $\mtp[1].. = \classoff\Cp\K$, and $\k = \wrap\C{\mt[1].. }{\mtp[1]..}\D\that$, then $\StrSub\cdot{\K~\k}\D\Cp$ and $\WFp \k$.
