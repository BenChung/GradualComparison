...
\begin{itemize}
\item {\bf L1:} Fails every non-TypeScript language because \code A is not a
  subtype of \code I, caused by \code A and \code I having different sets of
  methods.
  
\item{\bf L2:} Succeeds in Transient and Typed Racket as, now, \code A is
  behaviorally equivalent to \code I. However, Thorn rejects this case
  because, in the source language, \code A is not a subtype of \code I.
 
\item{\bf L3:} Typed Racket has an error on this case, as the call to \code
  F.\m casts the instance of \C to \code E, in turn requiring \C.\m to only
  take and return values of type \D. We then pass \C.\m a value of type \C,
  which is then caught by the wrapper. Thorn and Transient do not persist
  the cast to \code E, and do not induce an error as a result.
\end{itemize}
...
the petri dish of language research in academia, and applications are now
written with languages incorporating gradual types.  However the proposed
designs are subtle and differ in corner behaviours: it is the responsibility
of language researchers to gain a clear picture of each approach to gradual
typing.

In this paper we introduced \kafka, a formal language that serves as the
foundation for deconstructing several existing gradual typing systems:
Typescript, Thorn, Transient Python, and Typed Racket.  \kafka offers an
opportunity to compare and contrast each gradual typing system within an
unified framework. The strength of \kafka lies in its simplicity and in the
flexibility provided by its casts and wrapper generation.
...
differentiate in what they can write, express, and even in what can and
cannot be evaluated dynamically.  From Typed Racket, we extracted the
principle that every value is inherited of its declared type. In \kafka this
manifested itself as the types of values are enforced before any call can be
performed. Conversely, in Transient the method being called is tasked with
ensuring the arguments have the correct types.  TypeScript with no dynamic
behaviour, providing an excellent entry point for understanding the
migration of concrete types to optional types in a Thorn program.
