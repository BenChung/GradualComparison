...
In Typed Racket, any call to method \m of class \code F, encounters a wrapper that ensures 
that the argument and return type of \m is always a \D. Therefore, if the method \m is called with a \C,
the program would get stuck at the cast to \D. In Transient, the cast
to \code E is a no-op, so when we call \m with \C, no extra cast to \D is
encountered. The Transient design allows method \m, which expects an instance of class \C
as argument, to be called with a value of type \any at any point. However, this
...
%TODO What should we do about the conclusion? I'm trying to touch on each language design principle
% and show why each system is special in their own way \FZ{I would remove this last sentence.} 
Another takeaway from \kafka is understanding that gradual typing systems
