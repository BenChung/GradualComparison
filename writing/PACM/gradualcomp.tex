...
\\[1mm]
\trulename{TTR4} \TRG{\FWrite\f1\e}\Env \>=  \src{\SubCast\t{\FWrite\f\ep}}
  \>\WHERE\HS
  \TypeCk\K\this\C,
  \> \Ftype\f\t\In\App\K\C, \hspace{.7cm}
  \= \ep = \TAG\e\Env\any
\\[1mm]
\trulename{TTR5}   \TRG{\Call{\e1_1}\m{\e_2}}\Env \>= \src{\DynCall{\eps 1}\m{\eps 2}}
  \>\WHERE \HS 
  \TypeCk{\K,\Env}{\e_1}\any, \HS
  \> \eps 1 = \TRG{\e_1}\Env, 
  \> \eps 2 = \TAG{\e_2}\Env\any
\\[1mm]
\trulename{TTR6} \TRG{\Call{\e1_1}\m{\e_2}}\Env \>= \src{\SubCast\tp{\KCall{\eps 1}\m{\eps 2}\t\tp}}
   \>\WHERE\HS
   \TypeCk{\K,\Env}{\e_1}\C,
   \> \Mtype\m\t\tp\In\App\K\C,\HS
   \> \eps 1 = \TRG{\e_1}\Env, \HS\HS
...
\begin{lemma}{Type correctness of Transient translation}{THTA-TRANS}
  ...
  \begin{proof} \indmsg{\EnvTypeW\Env\K\e\t}
    ...
    \begin{case}{STT-GET}
      ...
      \step{$\Fdef\f\t \in \App\Kp\C$}{\trf{cond6} on case}\lbl{STTGet3}
      \step{$\ep = \SubCast\t{\FRead\x}$}{TTR3 on \trf{STTGet2}, \trf{STTGet3}}
      \step{$\EnvType\Envp\cdot\Kp{\FRead\x}\t$}{\RuleRef{KT-REFREAD} on \trf{STTGet2}, \trf{STTGet3}}
    \end{case}

    \begin{case}{STT-SET}
      \slet{$\ep$}{$\TRG{\FWrite\f\e}\Env$}\lbl{STTSet1}
      \slet{$\epp$}{$\TRG{\e}\Env$}\lbl{STTSet2}
      \step{$\EnvType\Envp\cdot\Kp\epp\t$}{IH on \trf{STTSet2}}\lbl{STTSet3}
      \step{$\ep = \SubCast\t{\FWrite\f\epp}$}{TTR2 on \trf{cond4}, \trf{STTVar1}}
    \end{case}
