...
Python~\cite{siek14}. TypeScript was chosen as a representative of unsound
systems that work by erasure such as Dart and Hack. Thorn combines dynamic
and static types in the same way as C\#, the approach was adopted in
...
as the JVM's bytecode or \kafka is interesting as it make explicit which of
the guarantees of the underlying type system can be relied upon and when
dynamism is required.  Our contribution is thus a translation, which we
prove to always generated well-typed code, that take a gradually typed
expression \HT\e\T in the source language is translated to a well-typed
\kafka term, the translation also entails translation of all the classes
needed to evaluate the expression. While soundness in \kafka is
uncontroversial, soundness of the source gradual type systems is more
...
\Call\x\m\e where \x is declared to be of class \C can have significantly
different behavior depending on choices made while designing the gradual
type system. TypeScript has \emph{optional types}; a well-typed program can
get stuck at any method call as \C translates to the dynamic type
\any. Thorn has \emph{concrete types}; a well-typed program will not get
stuck at statically resolved method calls. Typed Racket has \emph{promised
  types}; a well-typed program will not get stuck at a call to \m, because
\x refers to an object, or wrapper, that implements \m.  Wrappers may fail
if their target does not behave like a \C. Transient Python will also allow
the call to go through but may get stuck on the checking the return value.
...
recursion.  The heart of any gradual type system implementation is the
...
references by \a is a subtype of type \t.  To support sound gradual type
systems, \kafka is extended by \emph{generative} casts which create
wrappers.  The behavioral cast \BehCast\t\a generates a wrapper object that
monitors that methods invoked on \a abide by the interface of type
\t.\footnote{Generative casts are not supported natively by the JVM or CLR,
  but they can be encoded by a combination of class generation and dynamic
  loading, we provide a naive implementation of \kafka in the CLR to
  validate this claim.} This entails runtime generation of a class that
interposes on calls to the target object. We give \kafka an operational
semantics and prove soundness of its type system. With the exception of
generative casts the proof is straightforward.


\paragraph{Threats to validity.} Our study has limitations but we believe
that these do not affect our conclusions. The source languages were
...
machine support. Lastly, while we prove that type soundness of \kafka and
that our translations generated well-typed code, we do not establish a
correspondance with the semantics of the source languages. We gained
confidence in our results by testing the implementations of the respective
languages and validating that they agree with our translation.
...
the case where $\t$ or $\tp$ is $\any$, via its \xt{dynamic} type, which has
...
\xt{ID}, which the translated version of $\C$ implements. \xt{ID} has all of
the same methods as $\D$ did, allowing the same operations to be performed
on its instances. We then refer to $\D$ by \xt{ID} in the generated C\# type
...
implementation was trivial. What our implementation does not do is provide
an accurate picture of the performance of the gradual typing systems in
question, rather focusing on semantic differences. This limitation is due to
a wide range of factors, including having none of the commonly-cited
performance optimizations, such as threesomes~\cite{siek10}, combined with
the severe inherent restrictions on what programs can be reasonably written
in \kafka. 
...
\hrulefill\caption{Thorn type convertibility}\label{f:thornts}

\hrulefill  \small
\vspace{-0.5mm}
\hspace{1.5cm}\begin{minipage}{\textwidth}\begin{tabular}{lllll}  
...
\end{mathpar}\end{minipage}& \begin{minipage}{2.2cm}\begin{mathpar}  
\Rule{STH-GET}{
  ~ \\\\
  \HasType \Env\this\C \\\\
  \Fdef\f\t \in \App\K\C
...
\end{mathpar}\end{minipage}& \begin{minipage}{3.8cm}\begin{mathpar}  
\Rule[width=20em]{STH-SET}{
  ~ \\\\
  \HasType \Env\this\C ~\,~\,~\,~
  \Fdef\f\t \in \App\K\C \\\\
  \EnvTypeW \Env\K\e\tp ~\,~\,~\,~
...
\end{mathpar}\end{minipage}& \begin{minipage}{3.9cm}\begin{mathpar}  
\Rule[width=20em]{STH-CALL}{
  \EnvTypeW \Env\K\e\tp \quad  (\tp=\C~~\vee~~\tp=\CW) \\\\
  \Mtype \m{\t[1]}{\t[2]}\in \App\K\C  \\\\
...
\hspace{1.5cm}\begin{minipage}{\textwidth}\begin{tabular}{l}
\begin{minipage}{1.5cm}\begin{mathpar}  
\Rule{STH-NEW}{
...
% a feature that is important for the specific Typed Racket programming model, but is orthogonal to our formalization.

Our formalisation of the type system of Typed Racket is analogous to the type system of TypeScript
already presented in~\figref{f:typescriptts}.  However, contrarily to TypeScript, 
...
as shown in~\figref{f:typedrackettothorn}. In our translation, classes, 
methods, and expressions all retain their types, and the \kty{t} function maps classes to classes and \any to \any. Boundaries between typed and untyped
...
% This unique behaviour would be in consideration for  possible future work.
%Secondly, 
% While mixins are not  part of \kafka, supporting  the  behavioral semantics
%required to add them is comparatively easy. Checking for  mixins in \kafka
%only requires adding additional wrappers, rather than provides potentially
%having to  rewrite code~\cite{Takikawa:2012}.

%Typed Racket has many more practical language features in comparison to the other
%source languages we've described so far. Our translation does not take into
%considersation many of these features. 
%The aim of \kafka is to focus on
%the core object system and how it interacts with gradual typing. Type Racket does 
%have two features that are of special importance for gradual typing.
