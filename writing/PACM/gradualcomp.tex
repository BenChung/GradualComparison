...
functionality Typed Racket, highlighting the key issues of type identity
raised in Typed Racket. Certain implementation details and features 
important to the operation of Typed Racket, but are not relevant for the goal 
of \kafka have been omitted. For instance, the ``macro'' model of gradual 
typing has been omitted. In Typed Racket, types must be added one module at a
time, whereas in our system individual methods can be typed.  
Omitting this feature does not affect the fundamental semantics of our Typed Racket
type system. We have also omitted first-class classes. Supporting first-class
class on a common compilation target as the JVM or .NET CLR requires
substantial complexity without offering any significant benefits to our system.  
Interestingly, Typed Racket ensures objects must always respect the type invariants 
of their callers. This mechanism, which can be disabled, is needed to  ensure
soundness in the presence of inheritance. However, this is not needed for \kafka and
therefore has been omitted. Our Typed Racket source language exihbits some similarities to
Vitousek, Kent, and Siek's Guarded Reticulated Python~\cite{siek14}. Although we do not support
their consistent subtyping.

Our type system of Typed Racket is analogous to the type system of TypeScript, presented
in~\figref{f:typescriptts}.  Contrarily to TypeScript, Typed
Racked does ensure every value that passes through a typed/untyped boundary are consistent
with their types. This enforcement mechanism constitutes the core of
any gradual typing system. Thorn handles this through \emph{concrete} typing. 
The type tag of the value is statically required be a subtype of the required type. 
Transient tasks the users to ensure the operations they require are safe. Finally, Typed
...
In the example above, we show class \code A's method \m being invoked with an object of type \code C, 
which then invokes class \code B's method \m with the same object. However, class \code B's method \m 
is expecting another instance of \code B. In order to protect class \code B from the object \code C 
potentially not behaving as an object \code B. Our translation for Typed Racket inserts a behavioral 
cast at the boundary between the invocation of class \code B's method \m in class A. 
The object with type \C is wrapped to make sure that it acts as an object of type \B, 
producing the following translated result.
...
translate the typed code in class \code B as if the language had no untyped 
...
as shown in figure~\ref{f:typedrackettothorn}. In our translation, classes, 
methods, and expressions all retain their types. Boundaries between typed and untyped
code are statically checked with the auxiliary translation function, inserted as
a cast if required.
...
considersation many of these features. The aim of \kafka is to focus on
...
Like many practical object oriented languages Typed Racket supports inheritance. 
Inheritance creates a large array of difficulties for gradual typing.
For example, if an untyped class inherits from a typed one, then the self-
...
As extraordinary and surprising as this behaviour is, we do not believe
it is warranted for \kafka, as we believe inheritance to be orthogonal
to it's purpose. This unique behaviour would be in consideration for 
possible future work.

Secondly, Typed Racket provides support for first-class classes and allows
for typed mixins. While mixins are not in \kafka, the 
behavioral semantics required to add them is comparatively easy. Checking for 
mixins in \kafka only requires adding additional wrappers, rather than potentially having to 
rewrite code~\cite{Takikawa:2012}.
