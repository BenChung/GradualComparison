...
reified into a pair of getter and setter methods), and allows the dynamic
...
expression \HT{\e}{\T} in the source is translated to a well-typed \kafka
term, \HT\e\T, each type \T has a corresponding \kafka type \T, and,
similarly, each expression \e has an equivalent \e. While soundness in
\kafka is straightforward (except for generative casts), soundness of the
...
gradual types, an expression \Call{\x}\m{{\x'}} where \x is declared to be
of class \C can have significantly different behavior depending on choices
made while designing the gradual type system. TypeScript has \emph{optional
  types}; a well-typed program can get stuck at any method call as \C
translates to \any. Thorn has \emph{concrete types}; a well-typed program
will not get stuck at statically resolved method calls, and \C maps to
itself \C. Typed Racket has \emph{promised types}; a well-typed program will
...
a \C. Finally we describe the transient variant of Reticulated Python which
has somewhat surprising semantic properties.
