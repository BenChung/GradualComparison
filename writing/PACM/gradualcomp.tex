...
\\
\K\HS {\FWriteR\a{\f_i}\ap} \> \s     \>\Red\>     \K \> \ap \> \sp \>  \WHERE \>\App\s\a=\obj\C{\a[1]..\a[i]..} \HS  \> \sp = \Map\s{\Bind\a{\obj\C{\a[1]..\ap..}}}
\\
...
\HS  \App\s\a = \obj\C{\a[1]..} \HS\HS
  ...
  \names{\md[1]..} \subseteq \names{\mdp[1]..} \HS\HS \cload{\mdp[1]..} \\\HS
...
\begin{itemize}
  ...
  \item \RuleRef{KT-REFWRITE}
        ...
        If $\e$ is a value $\ap$, then it follows from $\Fdef\f\t \in \App\K\C$ that $\Class\C{\fd[i] ..~\Fdef\f\t ~ \fd[j]..}{\md..}$ and from well-formedness of heaps that $\App\sigma\a = \Map\s{\Bind{\a}{\obj\C{\a[i]..~\app~ \a[j]..}}}$. Therefore, we can apply the definition of $\xt{write}$ to find that $\setf\s\a\f\K\ap = \Map\s{\Bind{\a}{\obj\C{\a[i].. ~\ap~ \a[j]..}}}$, which we then define as $\sp$. Since we know that $\EnvType\cdot\s\K\ap\t$, and we know that $\forall \a \in \sigma, \Map\s{\Bind{\a}{\obj\C{..}}} \implies \Map\sp{\Bind\a{\obj\C{..}}}$, it follows that $\EnvType\cdot\sp\K\ap\t$, and from there that $\WFp\K{\ap~\sp}$ and $\EnvType\cdot\sp\K\ap\t$, so the theorem holds.
...
FIXME%      \slet{\Envp}{\Env, ~\namet{\x}{\Cpp}}\lbl{E7}
     ...
     \begin{case}{\Mdef\m\x\Dp\Dpp {~\bscast\Dpp{\KCall{\FRead\that}\m{\bscast\Cp\x}\Cp\Cpp}~}}\lbl{K}
      ...
      \step
          {\EnvType\Envp\sp\Kk{\bscast\Cp\x}{\Cp}}
          {\byjdg{WB}
            {\trf{K4}}}
          \lbl{K5}
FIXME%      \step{\EnvType \Envp\sp\Kk{\FRead\that}}{\byjdg{W3}{\trf{1}, \this variable, definition of \textbf{mtypes}} \lbl{K6}
...
FIXME%     \step{\cload{\classoff\D{\K~\k}}}{\trf{cond8}, \bylemm{\ref{lem:DM-WR}}{DupMeth over wrappers}{\trf{cond7}, \trf{cond4}}}\lbl{N1}
...
\begin{lemma}{Type correctness of Typed Racket translation}{THTR-TRANS}
  ...
  \begin{proof} \indmsg{\EnvTypeW\Env\K\e\t}
    ...
    \begin{case}{STG-NEW} 
      \step{$\e = \New\C{\e_i..}$}{case}\lbl{NEW-V1}
      ...
      \step{$\EnvType\Envp\cdot\Kp{\New\C{\e_i..}}\C$}{\byjdg{\RuleRef{KT-NEW}}{\trf{NEW-V3}, \trf{NEW-V4}}}\lbl{NEW-V5}
      \done{\trf{NEW-V5}}
    \end{case}
  \end{proof}
\end{lemma}
...
\begin{lemma}{Type correctness of Transient translation}{THTA-TRANS}
  ...
  \begin{proof} \indmsg{\EnvTypeW\Env\K\e\t}
    ...
    \begin{case}{STT-NEW}
      \slet{$\e$}{$\TRG{\New\C{\e_i ..}}\Env$}\lbl{STTNew1}
      ...
      \step{$\Fdef{\f_i}\any .. \in \App\Kp\C$}{\trf{cond4}}\lbl{STTNew5}
      \step{$\EnvType\Envp\cdot\Kp{\epp_i}{\t_i}$}{lemma \ref{lem:THTA-COMP} on \trf{STTNew4}, case}\lbl{STTNew6}
      \step{$\EnvType\Envp\cdot\Kp{\New\C{\epp_i..}}{\C}$}{\RuleRef{KT-NEW} on \trf{STTNew6}}
    \end{case}
  \end{proof}
\end{lemma}
\end{document}
