...
object-oriented languages. We aim to expose the forces that influenced
existing systems and discuss the implication of key design decisions. While
there are significant practical challenges that, in the end, may prevent
adoption of some of the more ambitious type systems, there may be workable
compromises that have not been fully explored yet.  To capture the essence
of gradual typing for objects and to highlight the design choices at hand,
we present translations of representative subsets of four gradually typed
...
implementers face, we present translations of representative subsets of
gradually typed languages into a common target language. Targeting the same
language lets us reason about the type systems in the same framework and
allows for comparison. But which language should we target? The language
should avoid linguistic clutter while expressing key object-oriented
features directly rather than by encoding. We propose to target a typed
object-oriented language inspired by modern language runtime such as the
Java Virtual Machine and the CLR in .Net. They have a static type system
with classes and subtyping but they also allow for dynamic resolution of
method dispatch.\footnote{Both runtime support reflective invocation based
  on method names. Dynamic resolution was added to C\# in version
  4.0~\cite{BAT14}. In Java, \xt{invokedynamic} allows for custom method
  dispatch.} As well as reasonably efficient implementations, and in both
environments, primitive types can be unboxed.
...
%, including first class classes, 
through a system of contracts, dynamically enforcing type guarantees. 
Our translation from Typed Racket to \kafka encapsulates the core of Typed Racket's
functionality, highlighting the key issues of type identity that Typed Racket raises.  For this we leave out some
features that are important for a real programming language, but are not
relevant to  the core of the language.   For instance we omit the ``macro'' model of gradual
typing of the source language: in Typed Racket types must be added one module at a time, whereas in our system individual methods can be typed inside untyped classes.  This does not affect the fundamental
semantics of the typing system.  We also omit first-class classes: they are not easily mapped to \kafka and supporting them on a common compilation target as the JVM or .NET CLR requires substantial complexity.  More interestingly, by default Typed Racket ensures that the receiver of a method call abides
  by the externally asserted type of that receiver. This mechanism, which can be disabled, is needed to 
  ensure soundness in the presence of inheritance, but is not needed for 
  \kafka and we omit it.  The source language can also be compared to
Vitousek, Kent, and Siek's Guarded Reticulated Python~\cite{siek14}, though we
do not support consistent subtyping, as Vitousek et al do.
...
declared types. Thorn handled this problem through the use of \emph{concrete}
typing, checking that the type tag on the value is statically known to be a subtype
of the required type, and Transient tasks the user with ensuring that the operations
that they require are safe. In contrast, Typed Racket ensures type correctness of 
untyped code through the insertion of wrapper contracts, interposed between typed
and untyped code. 

\begin{lstlisting}
 class A { m(x: *): * { new B().n(x) } }
 class B { n(x: B): C { x.n(x) } }
 class 
 new A().m( new A())
\end{lstlisting}
