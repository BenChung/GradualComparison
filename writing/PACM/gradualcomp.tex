...
f%ield definitions and method definitions, \Class\C{\b\fd}{\b\md}. Field
%definitions consist of a field and its type, \Fdef\f\t. Method definitions
%have (for simplicity) a single argument and an expression, denoted \Mdef\m\x\t\t\e.
...
\FZ{EXPLAIN IN ONE PARAGRAPH HOW SUBTYPING WORKS}
...
\end{mathpar}}
\hrulefill
\caption{\kafka static semantics (excerpt)}
\end{figure}
...
%\hline
%$\progtrans {\e~\K}{\e'~{\K'}}$ & $\classtrans {\K} {\Class \C{}{...}~\K} {\Class \C{}{...}~\K}$ & 
%$\methtrans {\K}{\C}{\md}{\md'}{}$ & $\fieldtrans {\fd}{\fd'}{}$ \\
%\makecell{Program} & Class & Method & \makecell{Field} \\
%\hline
%\end{tabular}
%\end{flushleft}
...
\HS \= \WHERE\HS\= \fds 1\HS\= = \Ftype\f\t, \HS \HS  \fdsp 1   = \src{\Ftype\f{\kty\t}} ..          \\
       ...
       \>          \> \ep      \> = \tr{e}{\K, \Ftype\this\C\,\Ftype\x{\t_1}, \e} \\
       \>          \> \mdspp 1 \>=  \src{\Mdef\m\x\any\any{ \SubCast{\t_2}{ \Call\this{\name\m{\t_1}{\t_2}}{ \SubCast{\t_1}\x } } }}\\
       \>          \>          \> \HS\HS \IF \kty{\t_1} = \D \OR \kty{\t_2} = \Dp\\
       \>          \>          \> \HS\HS  {\bf otherwise} empty ..                   \\      
%        \>          \>          \> \HS~  empty {\bf otherwise} ..                   \\
\\
\tr e{\K, \Env, \Call\this\f{}}  = \src{\Callt\this\f{}\t} \\
       \> \WHERE   \> \Ftype\f\t \EM{\in} \C,   \HS \TypeCk{\K,\Env}{\this}\C    \\
\\
\tr e{\K, \Env, \Call\e\f{}} = \src{\DynCallt\ep\f{}\any}  \\
      \> \WHERE   \> \ep = \tr e{\K, \Env,  \e}, \HS  \TypeCk{\K,\Env}\e\t, \HS  \kty\t=\any\\
\\
\tr e{\K, \Env, \Call\e\f{}} = \src{\Callt\ep\f{}\t}  \\
      \> \WHERE   \> \ep = \tr e{\K, \Env, \e}, \HS  \TypeCk{\K,\Env}\e\C, \HS \Ftype\f\t \IN \C  \\
\\
\tr e{\K, \Env, \Call\this\f\e} = \src{\Call\this{\name\f\t\t}{\SubCast\t\ep}} \\
\> \WHERE   \> \epp  = \tr e{\K, \Env, \e}, \HS  \TypeCk{\K, \Env}\this\C, \HS  \Ftype\f\tp\IN\C \\
      \>          \>  \t = \kty\tp, \HS  \ep = \tr e{\K, \Env, \e}\\
\end{tabbing}
\end{minipage}
&~~ \hspace{1cm} ~~~ &
\begin{minipage}{5cm}
  \begin{tabbing}
    \tr e{\K, \Env, \Call{\e_1}\f{\e_2}} = \src{ \DynCall{\e_1}{\name\f\any\any}{\SubCast{\any}{\eps 2}}}  \\
\HS\=\WHERE\HS\= \TypeCk{\K,\Env}{\e_1}\t, \HS \kty\t=\any\\
      \>          \>  \eps 1 = \tr e{\K, \Env, \e_1}, \HS  \eps 2 = \tr e{\K, \Env,  \e_2 }\\
\\
\tr e{\K, \Env,  \Call{\e_1}\f{\e_2}} = \src{\Call{\eps 1}{\name\f\t\t}{\SubCast\t{\eps 2}}} \\
      \> \WHERE   \> \TypeCk{\K, \Env}{\e_1}\C, \HS  \Ftype\f{\t'}\IN\C, \HS  \t = \kty{\tp} \\
      \>          \>  \eps 1 = \tr e{\K, \Env,  \e_1}, \HS  \TypeCk{\K,\Env}{\e_2}{\tpp}  \\
      \>          \>  \eps 2 = \tr e{\K, \Env, \e_2} \\
\\
\tr e{\K, \Env, \Call{\e_1}\m{\e_2}} = \src{\DynCall{\eps 1}{\name\m\any\any}{\SubCast\any{\eps 2}}} \\
     \> \WHERE   \> \TypeCk{\K,\Env}{\e_1}\t, \HS \kty\t=\any\\
     \>          \> \eps 1= \tr e{\K,\Env, \e_1}, \HS \eps 2=\tr e{\K,\Env,\e_2}\\
\\
\tr e{\K,\Env, \Call{\e_1}\m{\e_2}} = \src{\Call{\eps 1}{\name\m{\t_1}{\t_2}}{\SubCast{\t_1}{\eps 2}}} \\
     \> \WHERE   \>  \TypeCk{\K,\Env}{\e_1}\C, \HS  \eps 1 = \tr e{\K,\Env,\e_1} \\
     \>          \>  \TypeCk{\K,\Env}{\e_2}\tp, \HS  \Mtype\m\x{\t_1}\IN \C \\
     \>          \>   \eps 2 = \tr e{\K, \Env, \e_2} \\
\\
\tr e{\K, \Env, \New\C{\e_1..}} = \src{\New\C{\SubCast{\t_1}{\eps 1}..}} \\
     \> \WHERE   \>  \Ftype{\f_1}{\t_1}\IN\C, \HS  \eps 1= \tr e{\K,\Env,\e_1} ..
\end{tabbing}
\end{minipage}
\end{tabular}

Second pass ... this could be applied to all translations. 
All of this should be grey boxed. \\

\begin{tabular}{@{}l@{~ ~ ~ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}ll}
\hspace{-.5cm}\begin{minipage}{8cm}  
\begin{tabbing}
\rc c{\K, ~\Class \C{\fds 1..}{\mds 1 .. }} = \Class \C{ \fds 1..}{\mdsp 1..} \\
\HS \= \WHERE\HS\= \mdsp 1\HS\= =  \rc m{\K, ~\namet{\this}{\C}, ~\mds1} \\

\\
\rc m{\K, ~\Env, ~\Mdefz\f\t\e}  = \Mdefz\f\t\ep \\
       \> \WHERE\HS\= \ep =  \rc e{\K, ~\Env, ~\e} \\
\\
\rc m{\K, ~\Env, ~\Mdef\f\x\t\t\e} = \Mdef\f\x\t\t\ep \\
      \> \WHERE   \> \ep = \rc e{\K, ~\Env~\namet{\x}{\t},  ~\e}\\
\\
\rc m{\K, ~\Env, ~\Mdef\m\x\t\t\e} = \Mdef\m\x\t\t\ep  \\
      \> \WHERE   \> \ep = \tr e{\K, ~\Env~\namet{\x}{\t}, ~\e}\\
\\      
\rc e{\K, ~\Env, ~\this} = \this \\
\\
\rc e{\K, ~\Env, ~\this} = \x \\
\\        
\rc e{\K, ~\Env, ~\SubCast{\C}{\e}} = \e \\
     \> \IF   \>  \TypeCk{\K,\Env}{\e}\D, \HS \StrSub\emptyset\K\D\C \\
\\
\rc e{\K, ~\Env, ~\SubCast{\C}{\e}} = \SubCast{\any}{\e} \\
     \> \IF   \>  \TypeCk{\K,\Env}{\e}\D, \HS \StrSub\emptyset\K\D\C \\
\\
\end{tabbing}
\end{minipage}
...
\rc e{\K, ~\Env, ~\Call{\e}\f{}} = \Call{\ep}\f{} \\
\HS\=\WHERE\HS\= \ep = \rc e{\K, ~\Env, ~\e}\\
\\
\rc e{\K, ~\Env,  ~\DynCall{\e}\f{}} = \DynCall{\e}\f{} \\
\HS\=\WHERE\HS\= \ep = \rc e{\K, ~\Env, ~\e}\\
\\
\rc e{\K, ~\Env, ~\DynCall{\e_1}\f{\e_2}} = \DynCall{\ep_1}\f{\ep_2} \\
\HS\=\WHERE\HS\= \ep$_1$ = \rc e{\K, ~\Env, ~\e_1}, \HS \eps 2 = \rc e{\K,~\Env,~\e_2}\\ 
\\
\rc e{\K, ~\Env, ~\DynCall{\e_1}\m{\e_2}} = \DynCall{\ep_1}\m{\ep_2} \\
\HS\=\WHERE\HS\= \ep$_1$ = \rc e{\K, ~\Env, ~\e_1}, \HS \eps 2 = \rc e{\K,~\Env,~\e_2}\\ 
\\
\rc e{\K, ~\Env, ~\New\C{\e_1..}} = \New\C{\eps{1}..} \\
     \> \WHERE   \>  \eps 1= \rc e{\K,~\Env,~\e_1} \\
\\     
\rc e{\K, ~\Env, ~\SubCast{\any}{\e}} = \e \\
     \> \IF   \>  \TypeCk{\K,\Env}{\e}\any \\
\\
\rc e{\K, ~\Env, ~\SubCast{\any}{\e}} = \SubCast{\any}{\e} \\
     \> \IF   \>  \TypeCk{\K,\Env}{\e}\C ..
\end{tabbing}
\end{minipage}
\end{tabular}
...
% 
% \begin{figure}
%   \caption{thorn}\label{fig:thorntrans_exp}
% \end{figure}
...
\begin{minipage}{8cm}  
\begin{tabbing}
\tr c{\Class\C{\fds 1..}{\mds 1.. }} =  \src{\Class \C {\fds 1..}{\mdsp 1.. } }\\
\HS\= \WHERE\HS\= \mdsp 1 = \tr m{\mds 1}..\\[1mm]
\tr m{\Mdef\m\x\any\any\e} = \src{\Mdef\m\x\any\any\ep} \\
\> \WHERE \> \ep = \tr e\e\\[1mm]
\tr m{ \Mdef\m\x\t\tp\e} = \src{\Mdef\m\x\any\any{\SubCast\t\x ; ~\ep}}\\
\> \WHERE \> \ep = \tr e\e\\[1mm]
\tr e{\Call\this\f{}}  = \src{\Call\this{\namet\f\any}{}} \\[1mm]
\tr e{\Call\e\f{}} =  \src{\DynCall\ep{\name\f\any}{}}  \HS \WHERE \ep = \tr e\e\\[1mm]
\tr e{\Call\this\f\e} =  \src{\Call\this{\name\f\any\any}\ep} \HS \WHERE \ep = \tr e\e\\[1mm]
\tr e{\Call{\e_1}\f{e_2}} = \src{\DynCall{\eps 1}{\name\f\any\any}{\eps 2}} \HS\WHERE \eps 1 = \tr e{\e_1}, \HS \eps 2 = \tr e{e_2}\\[1mm]
\tr e{\Call{\e1_1}\m{e_2}} = \src{\DynCall{\eps 1}{\name\m\any\any}{\eps 2}} \HS \WHERE  \eps 1 = \tr e{\e_1}, \HS \eps 2 = \tr e{e_2}\\[1mm]
\tr e{\New\C{\e_1..}} =  \src{\New\C{\eps 1..}}\HS \WHERE \eps 1 = \tr e{\e_1}\\
\end{tabbing}
\end{minipage}
\end{tabular}
\caption{Transient}
...
Here, in method \m, the only type that \x can possibly have is \any, in spite of the surrounding invocation requiring that it be an instance of a \C. Likewise, the only type that makes sense for \New\C\x is \C, despite the return type being declared as \any. We have to insert casts to make these two cases be well typed, giving us the following translated code:
...
Our cast insertion system is based on this intuition, finding the places where an expression's type is absolute (like \x being of type \any), yet is surrounded by an expression that requires it to be something quite different (e.g. \New\C\x requires that \x must be a \C), drawing on a commonly used approach from type inference, bidirectional typing.

In our approach, we compute ``real'' types using the so-called \emph{synthetic} case of typing, written $\GenCast\K\Env\e\ep\t$, meaning that $\e$ will always have type $\t$, while translating to $\ep$. Likewise, we enforce ``required'' types through the \emph{analytic} judgment, written $\AnaCast\K\Env\e\ep\t$, meaning that if $\e$ is required to be of type $\t$, then the translated $\ep$ will be of type $\t$.

As a illustrative example, consider $\x$ in the above. We know that $\x$ will always have type $\any$ - expressed as $\GenCast\K\Env\x\x\any$ by rule \RuleRef{THS-VAR} - no matter what happens around it. However, we also know that $\New\C\x$ requires that $\x$ has type $\C$ - expressed as $\AnaCast\K\Env\x?\C$, using ? as a placeholder for the result of translation - and need to insert a cast so that the types make sense. We can then apply rule \RuleRef{THA-ANYCONC} to \x with \C to find out that we can make \x have type \C via casting, concluding that $\AnaCast\K\Env\x{\SubCast\C\x}\C$.
