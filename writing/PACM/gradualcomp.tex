...
of \kafka have been omitted. 

One of Typed Racket's key cosmetic differences is the ``macro'' model of
gradual typing, where types are added to entire modules at a time, whereas our
model allows individual functions to be typed. We omit this in our translation
to \kafka as it does not impact the basic semantics of the language. Another
feature that Typed Racket supports is first-class classes with mixins, a
feature that is important for the specific Typed Racket programming model, but
is orthogonal to our formalization.

Our type system of Typed Racket is analogous to the type system of TypeScript,
presented in~\figref{f:typescriptts}.  Contrary to TypeScript and Transient,
Typed Racked ``eagerly'' ensures that every value that passes through a
typed/untyped boundary is consistent with the declared types.  Thorn handles
this through \emph{concrete} typing, using the type tag to determine that the
entire object is well-typed, while Transient only guarantees the behavior
needed to ensure soundness. In Typed Racket, every value is implicitly assumed
to be of the correct type by the caller, and is checked against the type by
a wrapper.
...
Because of the strong static guarantee provided by the behavioural cast, the
Typed Racket translation is very straightforward. In Typed Racket, every typed
value is assumed to behave as specified, delegating the complexity of checking
for consistency with the type to the wrapper, described in
section~\ref{kafkacore}.
...
Secondly, Typed Racket provides support for first-class classes and allows for
typed mixins. While mixins are not in \kafka, the  behavioral semantics
required to add them is comparatively easy. Checking for  mixins in \kafka
only requires adding additional wrappers, rather than provides otentially
