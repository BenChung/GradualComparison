...
incorporate gradual types.  However the proposed designs have subtle
semantics and differ in dark corners. It is the responsibility of language
researchers to paint a clear picture of each approach to gradual typing.
...
In this paper we have compared the essence of four gradual type system
designs, namely Typescript, Thorn, Typed Racket, and Transient Python.  Our
formalization have shown that they differ in what programs are viewed as
statically correct. The Thorn type system makes a difference between
concrete and like types, whereas as Transient uses the notion of consistency
to determine if values may be compatible. A perhaps more surprising result
is that even at runtime these languages do not agree on what consistutes a
valid program. Consider \figref{litmus} which shows three small programs
that are well-typed in all four languages. These are synthetic examples
consisting of a class table and a top-level expression. All three will
execute under TypeScript's permissive semantics and will not get stuck.
{\bf L1} fails in all other semantics because \A is not a subtype of \xt I
(Thorn) and \A and \xt I have different sets of methods (Racket and Python).
{\bf L2} fails in Thorn, again because the concrete type checks require
subtyping of \A and \xt I to hold. {\bf L3} fails in Racket as the \xt{F.m}
casts the instance of \C to \code E, in turn requiring \C.\m to only take
and return values of type \D. We then pass \C.\m a value of type \C, which
is then caught by the wrapper. Thorn and Transient do not persist the cast
to \code E, and do not induce an error as a result. Thus given these three
programs and the ability to observe errors, one can divine which semantics a
language implements.
