...
to the top level dynamics. Similarly, \RuleRef{BC2} wraps
\a with the guards to let \a act like an instance of \any.\FZ{rule arguments look bogus}

Wrapper generation is implemented by the \xt{wrap} function.  Its first argument \(\C\), \(\bmt\), \(\bmtp\) are the type of the expression being casted (the \emph{source} type), the type declarations of all its fields and methods, and the type declarations of all the fields and methods of class type the expressions is being casted to (the \emph{target} type).  The final argument, \(\D\), is the class name  of the wrapper class being built; this is always a fresh name in our semantics. \FZ{and that?}  The function \xt{wrap} builds a new wrapper class with name \(\D\), a \xt{that} field pointing to the source type and storing the wrapped object.

\FZ{DESCRIBE INTERNAL CASES}  \FZ{REMARK THAT BEHAVIOURAL CASTS ARE USED INTERNALLY}

The last three cases deal with methods defined in the source type but not in the 
target type. For each of these method, their method call is redirected 
to its corresponding method in the source type.  

Wrapper generation for dyn is simpler, as wrappers only cast arguments and return types for all fields and methods to \any. 
%
...
A wrapper generated for a type \C is a subtype of \C \FZ{if C is the target type, otherwise no!} allowing us to refer to
