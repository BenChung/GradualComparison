...
\noindent The first call is rejected by the static type system because the concretely typed method \n expects a value of type \C, while a value of type \D has been passed.  Formally, the Thorn type system (reported in~\figref{f:thornts}), rejects this because the \(\D\Mapsto\C\) convertibilty test required by the STH-CALL rule fails.    The second call  is instead allowed because arbitrary values can flow into a \dt\C typed variable: convertibility follows from 

.  Since \D and \C are incompatible the static type system emits a warning as guidance for the programmer; in the third call it is a \C value that is passed where a \dt\C is expected: not only this is allowed but in this case no warnings are emitted.  The last expression invokes \n passing an argument of type \dt\C where a \C is expected; this is potentially incorrect but to foster program evolution Thorn static type system allows it, and a dynamic type check is inserted to protect the call.


The type system of Thorn is reported in Fig.~\figref{f:thornts}.  The convertibility relation allows passing a \dt\C when a \D is expected provided that \C <: \D (expression 3), \FZ{ANYCONC should go to be in synch with Fig.2 of the liketype paper}.

, distinguishes method invocation whether the callee is a concrete or optional type, or dynamic.  
...
%sound counterpart, but generates no runtime casts.  

%Over this new type, we
...
%If the callee has a dynamic type  // what if like?  in fig 2 it is slightly different...

Optional types (written \dt\C) are translated to the \any type with
dynamically resolved method invocation, as in TypeScript.
