...
\item{\bf L3:} Typed Racket has an error on this case, as the call
to \code F.\m casts the instance of \C to \code E, in turn requiring \C.\m
to only take and return values of type \D. We then pass \C.\m a value of
type \C, which is then caught by the wrapper. Thorn and Transient do not
persist the cast to \code E, and do not induce an error as a result.
\end{itemize}
