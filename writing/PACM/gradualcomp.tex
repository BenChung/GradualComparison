...
we would pick up a wrapper that ensures that the argument to \m is always a \D
and that it always returns a \D. As a result, when we finally call \m with a
\C, the program would get stuck at a cast to \D. However, in Transient, only
the actual final method's type matters, which, in this example is \C, so
Transient will allow the program.
