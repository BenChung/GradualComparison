...
\usepackage{xspace,listings,url,framed,amssymb,colortbl,
            amsmath,mathpartir,hyperref,doi,
...
More precisely, \kafka can be thought of as a simple object calculus with fields accesses protected by getters and setters, and with statically and dynamically checked method invocation.  \kafka supports two different cast mechanisms.  The former, the \emph{structural cast} is the usual subtype cast that dynamically type-checks its argument.  The latter, the \emph{behavioural cast}, instead of type-checking the argument builds a wrapper around it, and the wrapper will dynamically ensure that all the successive requests are understood. \FZ{rewrite.}

The syntax of \kafka is given below:
%in \figref{syn}.
...
\noindent Meta-variables \C, \D and \E
...
The small operational step operational semantics for \kafka appears below.
%in \figref{fig:semantics}.
...
\FZ{If we keep this, this is a good placeholder to explain why we ignore monotonic.  Otherwise, excise and move to related works.}
