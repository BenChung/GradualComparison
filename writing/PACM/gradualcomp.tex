...
Likewise, in order to ensure that field assignment will not get stuck,
Transient gives all fields type \any, then checks the field value on
dereference, based on the static typing.

BEN TODO: Make an example for this.

In the following example, if we assume that the types \C and \D are incompatible,
we see a program that Transient will reject at runtime, like Typed Racket and 
Thorn would. When \A.\m is passed an instance of \C, no types are violated, but
when passed an instance of \D (through the dynamic intermediary \n), Transient
produces a dynamic error.
...
The Transient guarantee is not identical to that of Typed Racket or Thorn,
however. In Typed Racket, wrappers stack, causing intermediate type assertions
to be retained if the wrapped value is passed into another context, even if
the new context has a different type, illustrated in the following example.
...
alongside potential consumers for \C and \D, \C itself and \code E, alongside a
conversion helper \code F. Here, we use \code F to acquire a reference to \C
as an \code E, which would imply that \C's \m has type \D to \D, despite it
actually having type \C to \C, then call (again, using untyped code) it with
a \C.
...
\section{Trace preservation/ Lay of the land  -find a better title}\label{litm}
...
completion without getting stuck. With Thorn, all litmus programs fail at a structural cast from \code A to \code I.  
