...
The main semantic reality-inspired oddity in Typed Racket is how it deals with
object types. In Typed Racket, an object must not only be well-typed as
observed by a typed caller, but must also respect the invariants of its
callers. This mechanism, which can be disabled by use of direct invocation, is
needed to  ensure soundness in the presence of inheritance. However, this is
not needed for \kafka and therefore has been omitted. Our Typed Racket source
language exihbits some similarities to Vitousek, Kent, and Siek's Guarded
Reticulated Python~\cite{siek14}, though we differ in not supporting
consistency.
...
mixins in \kafka only requires adding additional wrappers, rather than provides otentially having to 
