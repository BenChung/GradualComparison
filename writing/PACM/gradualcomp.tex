...
the absence of errors at run-time, rather it means that some errors will
...
elegant idea have proved elusive. One reason for this may be that the
original paper did not consider state. The combination of mutable state,
aliasing and subtyping complicates enforcement strategies as one must
consider situations where an object is being accessed, and mutated, at
different types. While several solutions have been proposed, the performance
implications of their implementation strategies appear
daunting.\footnote{Languages with \emph{sound} gradual type systems
  experience orders of magnitude pathologies, e.g.~5x for
...
degrading performance.\footnote{Dart, Typescript and Hack are good examples,
  they all use unchecked modes for production; in that mode no runtime
  checks are performed, and thus type errors are not caught.}
...
workable compromises that have not been fully explored yet.  To capture the
essence of gradual typing and to highlight the choices at hand, we present
translations of representative subsets of four gradually typed
object-oriented languages into a common target language. This target
language, which we name \kafka, is modeled on the intermediate language of
the Java Virtual Machine and the Common Language Runtime -- both of which
have been used as targets for implementations of gradual typing systems.
The features that we exploit for gradual typing are the presence of static
type system equipped with dynamic casts that test the run-time type of
values, the ability to dynamic resolve method targets, and support for
generating new classes at run-time.\footnote{Both the JVM and the CLR have
  runtime type tests, reflective invocation based on method names and
  runtime code generation. Dynamic resolution was added to C\# in version
...
references by \a is a subtype of type \t.  To support some of the more
complex type systems, \kafka is extended by \emph{generative} casts which
create new wrapper classes.  The behavioral cast \BehCast\t\a generates a
...
to objects denoted \hspace{-1mm}\obj\C{\a[1]..}.
...
\medskip\noindent{\bf Lemma 1: Wrapper preserves no duplication.} ~
...
\medskip\noindent{\bf Lemma 2: Wrapper preserves subtype over target type.} ~
...
\TR{\Class \C{\fd[1]..}{\md[1] .. } } = \src{ \Class \C{ \fdp[1]..}{\mdp[1]..}}\HS \WHERE\HS
  ...
  \>\md[1] = \src{\Mdef\m\x\any\any\ep},\HS\HS \>\>\ep = \TR{\e}
\end{tabbing}
\begin{tabbing}
...
\IRule{STH-NEW}{
  \Class \C {\Fdef{\f[1]}{\t[1]}..}{\md[1]..} \in \K\\
  \EnvTypeW \Env\K{\e[1]}{\tp[1]}..\\
 \ConvertE\K{th}{\tp[1]}{\t[1]}..
}{
  \EnvTypeW \Env\K{\New\C{\e[1]..}}\C
}
\end{mathpar}
\hrulefill
...
\noindent The first call is rejected by the static semantics because the concretely typed method \n expects a value of type \C, while a value of type \D has been passed.  Formally, the Thorn type system (reported in~\figref{f:thornts}), rejects this because the \(\D\Mapsto\C\) convertibilty test required by the static method invocation (\RuleRef{STH-CALL}) rule fails.  Similarly, the second call is rejected because the \(\D\Mapsto\dt\C\) convertibilty check fails as well.  The \dt\C annotation for the argument of \m suggests that the intention of the programmer is to invoke this  method only with instances of \C; even if this is not strictly enforced, expressions as this one, that are likely to fail at run-time, are identified as errors.
In the third call, a \C value that is passed while a \dt\C is expected: this is allowed as subtyping implies convertibility.  The last expression invokes \n and passes an argument of type \dt\C where a \C is expected; although potentially unsound, this is allowed by Thorn to foster program evolution.  Convertibility thus includes a \(\dt\C\Mapsto\C\) rule; at run-time Thorn protects the call and ensures soundness by inserting a dynamic type cast to \C around the argument.

If the receiver object has an optional type,  method invocation is statically checked as in the concrete case above:
 optional types behave as contracts between variables and contexts, and whenever an object has type \dt\C a well-typed context is expected to use it only as a variable pointing to an instance of the class \C.  Since the run-time does not guarantee that  \dt\C variables points to instances of the class \C, the conformance of the value actually accessed is checked individually at each method invocation, as if the callee had type \any.  
...
language, supporting Racket's key functionality %, including first class
classes,  through a system of contracts, dynamically enforcing type
guarantees.  Our translation from Typed Racket to \kafka encapsulates the core
of Typed Racket's functionality, highlighting the key issues of type identity
that Typed Racket raises.  For this we leave out some features that are
important for a real programming language, but are not relevant to  the core
of the language.   For instance we omit the ``macro'' model of gradual typing
of the source language: in Typed Racket types must be added one module at a
time, whereas in our system individual methods can be typed inside untyped
classes.  This does not affect the fundamental semantics of the typing system.
We also omit first-class classes: they are not easily mapped to \kafka and
supporting them on a common compilation target as the JVM or .NET CLR requires
substantial complexity.  More interestingly, by default Typed Racket ensures
that the receiver of a method call abides   by the externally asserted type of
that receiver. This mechanism, which can be disabled, is needed to    ensure
soundness in the presence of inheritance, but is not needed for    \kafka and
we omit it.  The source language can also be compared to Vitousek, Kent, and
Siek's Guarded Reticulated Python~\cite{siek14}, though we do not support
consistent subtyping, as Vitousek et al do.

The type system of Typed Racket is analogous to that of TypeScript, already
reported in~\figref{f:typescriptts}.  However, contrarily to TypeScript, Typed
Racked  ensures that values passing a typed/untyped boundary are consistent
with their declared types.  This enforcement mechanism constitutes the core of
any gradual typing system:  Thorn handled this problem through the use of
\emph{concrete} typing, checking that the type tag on the value is statically
known to be a subtype of the required type, and Transient tasks the user with
ensuring that the operations that they require are safe. In contrast, Typed
Racket ensures type correctness of  untyped code through the insertion of
wrapper contracts, interposed between typed and untyped code.
...
In this example, we invoke \code A's method \m with a \code C, which then
passes its argument to \code B's \m, which is expecting another instance of
\code B. In order to protect \code B from \code C's potentially not behaving
as a \code B, our translation inserts a behavioral cast at the boundary
between \code A and \code B, wrapping over \C to make sure that it acts like a
\B, producing the following translated result.
...
\(
  ...
  \KCall{\New{\xt A}{}}\m{\New{\xt B}{}}\any\any
  \end{array}
\)

\noindent By enforcing types on the values themselves, rather than leaving it
up to the consumer to check that the types are correct like the Transient
semantics do, Typed Racket is able to translate the typed code in \code B as
if the language had no untyped code at all, as the values it is interacting
with will catch themselves if they misbehave.

These two key features, having wrappers applied at type boundaries and having
unchanged type code, informs our translation from Typed Racket to \kafka,
shown in figure~\ref{f:typedrackettothorn}. Under this translation, classes
retain their type signatures, with method types staying the same, and
expressions likewise retain their types. Boundaries between typed and untyped
code are statically checked with the auxiliary translation function, inserting
a cast if required.
...
\TRG{\FRead\f}\Env  = \src{\FRead\f}
\\[1mm]
\TRG{\FWrite\f\e}\Env =  \src{\FWrite\f\ep} \hspace{1cm}
\=\WHERE
...
\hrulefill
\caption{Typed Racket}\label{f:typedrackettothorn}
\end{figure}

Notably, Typed Racket has more features in comparison to several of the other
approaches that we describe here, and has many practical language features as
a result. Our translation does not worry about many of these, as we focus on
the core object system and how it interacts with gradual typing, but two of
the features in Typed Racket have special importance for gradual typing.

First, Typed Racket supports inheritance, like many practical object oriented
languages. This feature creates special difficulties for a gradual typing
system, as if an untyped class inherits from a typed one, then the self-
reference may be accessed from both typed and untyped contexts without
nesescarily passing through a typed/untyped boundary. As a result, inheritance
in a gradually typed setting requires the protection of all self-references.
Since we do not model inheritance, we do not need this feature, however.

Secondly, Typed Racket provides support for first class classes, which allow
users to have typed mixins. While we do not have mixins in our system, the 
behavioral semantics makes adding them comparatively easy, as checking for them
now reduces to adding additional wrappers, rather than potentially having to 
rewrite code, as in other approaches~\cite{Takikawa:2012}.
...
\opdef{\WF{}\s\K {\Class\C{\fd[1]..}{\md[1]..}}}{Well-formed class}
\vspace{-2.5mm}
\begin{mathpar}
\IRule{WC}{
 \cload{\md[1]..~\fd[1]..} \\
...
\begin{mathpar}
\IRule{WH}{
\Bind\ap{\obj\C{\a[1] ..}}~\in~\s \implies \Class\C{\fd[1]..}{\md[1]..}\in\K ~~~\wedge~~~  
...
\opdef{\WFW{}\s\K {\Class\C{\fd[1]..}{\md[1]..}}}{Well-formed classes}
\vspace{-1mm}
\begin{mathpar}
...
\opdef{\WFx{}\s\K {\Class\C{\fd[1]..}{\md[1]..}}}{Well-formed classes}
\begin{mathpar}
\vspace{-3mm}
...
If $\WFp \K$ (TODO), if $\C \in \K$, $\Cp \in \K$, $\D$ free, $\md[1].. \In\App\K\C$, $\mt[1].. = \classoff\C\K$, $\mtp[1].. = \classoff\Cp\K$, and $\k = \wrap\C{\mt[1].. }{\mtp[1]..}\D\that$, then $\StrSub\cdot{\K~\k}\D\Cp$ and $\WFp \k$.
...
\subsection{Wrap function creates well-formed classes}
  ...
  \begin{conds}
   ...
   \cond{\mt[1].. = \classoff\C\K}\lbl{cond3a}
   \cond{\mtp[1].. = \classoff\Cp\K}\lbl{cond3b}
