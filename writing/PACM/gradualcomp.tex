...
\FZ{TODO: grey ?C out}
\hrulefill
\caption{Syntax of source languages}\label{f:sourcesyntax}
\end{figure}

\noindent
We are now ready to give semantics to the four gradual type systems of interest.  For this we use the idealised syntax reported in \figref{f:sourcesyntax}, and we define the semantics of each gradual type system by
translation to \kafka.  The source syntax allows typed and untyped terms to interact freely, as in the gradually typed systems of interest, constrained only by the source type system rules.  The compilation into \kafka makes explicit which type casts (aka type-checks and wrapper introduction) are implicitly inserted by the run-time of each language.  To avoid unnecessary clutter, the source syntax we use is just \kafka one without casts.
