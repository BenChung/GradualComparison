...
\noindent Assume that \D is not a subtype of \C.  The following expressions illustrate the interesting cases of the interaction between the types of the arguments and the declared type of the method parameter:
...
dynamically resolved method invocation, as in TypeScript.  Consider the
...
\begin{lemma}{Type correctness of TypeScript/Typed Racket translation}{THTS-TRANS}
  ...
  \begin{proof} \indmsg{\EnvTypeE\Env\K{s}\e\t}
    ...
    \begin{case}{STG-VAR}\lbl{A}
      \step{$\TRG{\x}\Env = \x$}{\byjdg{TTS4}{\trf{cond4}}}\lbl{A1}
      \step{$\EnvType\Envp\cdot\Kp\x{\kty\t}$}{\trf{cond4}, \trf{cond5}, \trf{cond6}, \RuleRef{KT-VAR}}\lbl{A2}
      \done{\trf{A2}}      
      
    \end{case}

    \begin{case}{STG-GET}\lbl{B}
      \step{$\TRG{\FRead\f}\Env = \FRead\f$}{\byjdg{TTS1}{\trf{cond4}}}\lbl{B1}
      \step{$\EnvType\Envp\cdot\Kp{\FRead\f}{\kty\t}$}{\trf{cond4}, \trf{cond5}, \trf{cond6}, \RuleRef{KT-READ}}\lbl{B2}
      \done{\trf{B2}}
    \end{case}

    \begin{case}{STG-SET}\lbl{C}
      \step{$\EnvTypeW\Env\K{\FWrite\f\e}\t$}{case}\lbl{C1}
      \step{$\Convertible\K\tp\t$}{case}\lbl{C2}
      \step{$\EnvTypeW\Env\K\e\tp$}{inversion on \trf{C1}}\lbl{C3}
      \slet{\ep}{\TRG\e\Env}\lbl{C4}
      \step{$\EnvType\Envp\cdot\Kp\ep{\kty\tp}$}{IH on \trf{C3}}\lbl{C5}
      \slet{\epp}{{\TAG\e\Env{\t}}}\lbl{C6}
      \step{$\EnvType\Envp\cdot\Kp\epp{\kty\t}$}{lemma \ref{lem:THT-COMP} on \trf{C5}}\lbl{C7}
      \step{$\TRG{\FWrite\f\e}\Env = \FWrite\f\epp$}{\byjdg{TTS2}{\trf{C6}, \trf{C1}, \trf{cond5}}}\lbl{C8}
      \step{$\EnvType\Envp\cdot\Kp{\FWrite\f\epp}{\kty\t}$}{\trf{C8}, \trf{C1}, \trf{cond5}, \RuleRef{KT-READ}}\lbl{C9}    
    \end{case}
...
\begin{lemma}{Type correctness of Typed Racket translation}{THTR-TRANS}
  ...
  \begin{proof} \indmsg{\EnvTypeW\Env\K\e\t}
    ...
    \begin{case}{STG-VAR}    
      \step{$\e = \x$}{case}\lbl{V1}
      \step{${\TRG{\e}\Env} = \x$}{TTK1}\lbl{V2}
      \step{$\EnvType\Envp\cdot\Kp\x\t$}{\RuleRef{KT-VAR}, \trf{cond6}}\lbl{V3}
      \done{\trf{V3}}
    \end{case}

    \begin{case}{STG-GET} 
      \step{$\e = \FRead\f$}{case}\lbl{V1}
      \step{${\TRG{\e}\Env} = \FRead\f$}{TTK2}\lbl{V2}
      \step{$\EnvType\Envp\cdot\Kp\this\C$}{\trf{cond6}}\lbl{V3}
      \step{$\Fdef\f\t \in \App\Kp\C$}{\trf{cond5}}\lbl{V4}
      \step{$\EnvType\Envp\cdot\Kp{\FRead\f}\t$}{\RuleRef{KT-READ} on \trf{V3}, \trf{V4}}\lbl{V5}
      \done{\trf{V5}}
    \end{case}

    \begin{case}{STG-SET} 
      \step{$\e = \FWrite\f{\e_1}$}{case}\lbl{V1}
      \slet{$\ep_1$}{$\TRG{\e_1}\Env$}\lbl{V2}
      \step{$\EnvType\Envp\cdot\Kp{\ep_1}\tp$}{IH on case}\lbl{V3}
      \slet{$\epp_1$}{$\TAG{\e_1}\Env\t$}\lbl{V4}
      \slet{$\e$}{$\FWrite\f{\epp_1}$}
      \step{$\EnvType\Envp\cdot\Kp{\epp_1}\t$}{lemma \ref{lem:THTS-COMP} on case, \trf{V4}}\lbl{V5}
      \step{$\EnvType\Envp\cdot\Kp\this\C$}{\trf{cond6}}\lbl{V6}
      \step{$\Fdef\f\t \in \App\Kp\C$}{\trf{cond5}}\lbl{V7}
      \step{$\EnvType\Envp\cdot\Kp{\e}\t$}{\byjdg{\RuleRef{KT-WRITE}}{\trf{V5}, \trf{V6}, \trf{V7}}}\lbl{V8}
      \done{\trf{V8}}
    \end{case}

    \begin{case}{STG-CALL} 
      \step{$\e = \Call\ep\m\epp$}{case}\lbl{V1}
      \slet{$\ep_1$}{$\TRG{\ep}\Env$}
      \slet{$\epp_1$}{$\TRG{\epp}\Env$}
      \slet{$\epp_2$}{$\TAG{\epp}\Env\t$}
      \step{$\EnvType\Envp\cdot\Kp{\ep_1}\C$}{IH on case}\lbl{V2}
      \step{$\EnvType\Envp\cdot\Kp{\epp_1}\tpp$}{IH on case}\lbl{V3}
      \step{$\EnvType\Envp\cdot\Kp{\epp_2}\t$}{lemma \ref{lem:THTS-COMP} on case, \trf{V3}}\lbl{V4}
      \step{$\Mtype\m\t\tp \in \App\Kp\C$}{\trf{cond5}}\lbl{V5}
      \step{$\EnvType\Envp\cdot\Kp{\Call{\ep_1}\m{\epp_2}}\tp$}{\byjdg{\RuleRef{KT-CALL}}{\trf{V2}, \trf{V4}, \trf{V5}}}\lbl{V6}
      \done{\trf{V6}}
    \end{case}
    ...
    \begin{case}{STG-DCALL} 
      ...
      \slet{$\epp_1$}{$\TRG{\epp}\Env$}
      \slet{$\epp_2$}{$\TAG{\epp}\Env\t$}
      \step{$\EnvType\Envp\cdot\Kp{\ep_1}\any$}{IH on case}\lbl{V2}
      \step{$\EnvType\Envp\cdot\Kp{\epp_1}\tpp$}{IH on case}\lbl{V3}
      \step{$\EnvType\Envp\cdot\Kp{\epp_2}\any$}{lemma \ref{lem:THTS-COMP} on case, \trf{V3}}\lbl{V4}
      \step{$\EnvType\Envp\cdot\Kp{\Call{\ep_1}\m{\epp_2}}\tp$}{\byjdg{\RuleRef{KT-DYNCALL}}{\trf{V2}, \trf{V4}}}\lbl{V5}
      \done{\trf{V5}}
    \end{case}

    \begin{case}{STG-NEW} 
      \step{$\e = \New\C{\e_i ~ \ldots}$}{case}\lbl{V1}
      \slet{$\ep_i$}{$\TRG{\epp}\Env$}
      \slet{$\epp_i$}{$\TAG{\epp}\Env{\t_i}$}
      \step{$\EnvType\Envp\cdot\Kp{\ep_i}{\tp_i}$}{IH on case}\lbl{V2}
      \step{$\EnvType\Envp\cdot\Kp{\epp_i}{\t_i}$}{lemma \ref{lem:THTS-COMP} on case, \trf{V2}}\lbl{V3}
      \step{$\Fdef{\f_i}{\t_i} \in \App\Kp\C$}{\trf{cond5}}\lbl{V4}
      \step{$\EnvType\Envp\cdot\Kp{\New\C{\e_i ~ \ldots}}\C$}{\byjdg{\RuleRef{KT-NEW}}{\trf{V3}, \trf{V4}}}\lbl{V5}
      \done{\trf{V5}}
    \end{case}
  \end{proof}
\end{lemma}
