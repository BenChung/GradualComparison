...
\begin{tabbing}
  \wrap{\C}{\mts 1..}{\mtps 1..}{\D}{\that} = \src{\Class \D{ \Fdef\that\C}{ \mds 1..}}\\
\HS\HS\WHERE\HS\=\mds 1 = \src{ \Mdef\m\x{\tp_1}{\tp_2}{~\BehCast{\tp_2}{ \KCall{\FRead\that} \m {\bscast{\t_1}\x}{\t_1}{\t_2}} } }   ..
    \HS \= \textbf{if} \HS \Mtype\m{\t_1}{\t_2}\In\mts 1.. \HS\HS \Mtype\m{\tp_1}{\tp_2}\In\mtps 1.. \\
\\[-3mm]
\> \EM{\mds 1} =  \src{\Mdef\m\x{\t}{\tp}{~\KCall{\FRead\that}\m{\x}\t\tp~}} ..
   \> \textbf{if} \HS\ \Mtype\m\t\tp\In\mts 1.. \HS\HS \Mtype\m{\tp_1}{\tp_2}$\not\In$ \mtps 1..
\\[3mm]
  \wrapAny{\C}{\mts 1..}{\D}{\that} = \src{\Class \D{ \Fdef\that\C}{ \mds 1..}}\\
\HS\HS\WHERE\HS\=\mds 1 = \src{ \Mdef\m\x{\any}{\any}{~\BehCast\any{ \KCall{\FRead\that} \m {\bscast{\t}\x}{\t}{\tp}} } }   ..
    \HS\HS\HS\HS \= \textbf{if} \HS \Mtype\m{\t}{\tp}\In\mts 1.. \\
\end{tabbing}
...
% \SP \class~\D~\{\\
% \SPP \Fdef \that \C\\ 
% \SPP   \Mdef\m\x\any\any {~\bscast\any{\KCall{\FRead\that()_{\C}}\m{\bscast{\t}\x}\t\tp }~}
...
\begin{equation*}
  \behcastE\a\Cp\s\K {\K\,\k}\ap\sp ~~\mathit{if}~~ \begin{cases}
  \App\s\a = \obj\C{\b{\a_1}} \\
  \freshb\D \qquad \freshb\ap \qquad \freshb\that \\
  \bmt = \classoff\C\K \qquad \bmtp = \classoff\Cp\K \\
  \cload{\bmt} \qquad
  \names{\bmtp} \subseteq \names{\bmt} \\  
  \k = \wrap\C{\bmt}{\bmtp}\D\that \qquad
  \sp = \Map \s{\Bind\ap{\obj\D{\a}}} \\ 
  \end{cases}
\end{equation*}
...
\begin{equation*}
  \behcastE\a\any\s\K {\K\,\k}\ap\sp  ~~\mathit{if}~~ \begin{cases}
  \App\s\a = \obj\C{\b{\a_1}} \qquad \bmt = \classoff\C\K\\
  \freshb\D \qquad \freshb\ap \qquad \freshb\that \\
  \k = \wrapAny\C{\bmt}\D\that \qquad
  \sp = \Map \s{\Bind\ap{\obj\D{\a}}} \\ 
  \end{cases}
\end{equation*}
