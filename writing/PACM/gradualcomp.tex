...
formalization have shown that they differ in which programs are viewed as
statically correct. Thorn distinguishes between concrete and like types,
whereas Transient uses the notion of consistency to determine if values may
be compatible. A perhaps more surprising result is that even at runtime
these languages do not agree on what constitutes a valid program. Consider
...
permissive semantics.  {\bf L1} fails in all other semantics because \A is
not a subtype of \xt I (Thorn) and \A and \xt I have different sets of
methods (Racket and Python).  {\bf L2} fails in Thorn because the concrete
type checks require subtyping of \A and \xt I to hold. {\bf L3} fails in
Racket as the \xt{F.m} casts an instance of \C to \xt E, in turn requiring
\C.\m to only take and return values of type \D yet it receives a \C which
is then caught by the wrapper. Thorn and Transient do not persist the cast
to \xt E, and accept the program as a result. Thus given these three
