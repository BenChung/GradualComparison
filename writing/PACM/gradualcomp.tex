...
\SP \}
\\[3mm]
\wrap{\C}{\bmt}{\bmtp}{\D}{\that}_{collapsed} = \\
\SP \class ~\D ~ \{\\
\SPP \Fdef\that\C \\
...
%
\noindent
The TypeScript type system is reported in \figref{f:typescriptts}.  Well-typed code is translated to plain JavaScript with all types erased and methods resolved dynamically. 
In \kafka, one would translate the class
...
\begin{lemma}{Soundness of compatibility}{THT-COMP}
  ...
  \begin{case}{STHC-SUB}
    \step{$\StrSub\cdot\Kp{\kty\t}{\kty\tp}$}{correctness of Thorn typing over $\kty{}$ on case}\lbl{CS1}
    \step{$\TAG\e\Env\tp = \ep$}{TTR7 on \trf{cond6}, \trf{CS1}}\lbl{CS2}
    \step{$\EnvType\Envp\cdot\Kp{\TAG\e\Env{\tp}}{\kty\tp}$}{\RuleRef{KT-SUB} on \trf{CS1}, \trf{CS2}, \trf{cond6}}
  \end{case}
