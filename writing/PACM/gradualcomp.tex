...
The Transient variant of Reticulated Python aims for soundness with a
predictable cost model, by defensively checking the declared types of the
...
The expression typing rules for our Transient Racket source is identical to
the typing rules of TypeScript presented in~\figref{f:typescriptts}, except
that the convertibility rules, defined in~\figref{f:transientts}, now builds
on an auxiliary \emph{consistency} relation, defined
in~\figref{f:consubtype} to relate types \t and \tp.
...
\hrulefill
\caption{Transient convertibility}\label{f:transientts}
...
\FZ{TODO: add translated code for the example above} Please check the code below.

\medskip
\(
 \Class \A {
    \Mdef \m\x\any\any  {\SubCast\C\x;~\SubCast\D{\SubCast\any{\KCall\x\n{\New{\C}{}}\any\any}}}  ; \ 
    \Mdef \n\x\any\any  {\SubCast\any\x;~\SubCast\any\x}
    }
\)

\medskip
\(\SubCast\D{(\KCall {\New \A{}} \m {\SubCast \any {( \KCall {\New \A{}} \n {\SubCast \any {\New \C{}}} \any \any)}} \any \any)} \)
