...
typing, and that the different choices are observationally distinct.
...
\begin{itemize}
\item {\bf L1:} Fails every non-TypeScript language because \code A is not a subtype of \code I,
  caused by \code A and \code I having different sets of methods.
  
\item{\bf L2:} Succeeds in Transient and Typed Racket as, now, \code A is behaviorally
  equivalent to \code I. However, Thorn rejects this case because, in the source language,
  \code A is not a subtype of \code I.
 
\item{\bf L3:} The Typed Racket semantics reject this case, because the call
  to \code F.\code m   causes the instance of \C to acquire a wrapper, forcing
  it to only take and return instances of   \D. When we pass it another \C -
  from untyped code - this wrapper rejects the \C. In both Thorn and Transient,
  the program is accepted as neither retains the cast inside of \code F.\code m,
  as the Typed Racket semantics does. Only the receiver's type, in this case \C,
  matters to the final invocation. 
\end{itemize}
