...
Python~\cite{siek14}. TypeScript is representative of unsound systems that
work by erasure such as Dart and Hack. Thorn's approach for combining
dynamic and static types is similar to that of C\# and was also adopted in
StrongScript. Typed Racket provides a sound type system close in spirit to
Siek and Taha's idea. Lastly, Transient was design to decrease the overhead
of soundness while retaining some runtime checking.  We focus on core
features of each type system, namely their treatment of objects, and explain
which errors are caught statically and which errors are caught
dynamically. One, perhaps surprising, observation is that there is no
agreement on what constitutes a ``correct`` program.  We illustrates with
litmus tests that allow us to differentiate between the type systems based
on which runtime errors will be generated (Section~\ref{litm}).
...
\noindent Here, \A's method \m is invoked with an object of type \C, 
which then invokes class {\xt B}'s method \m with the same object. However, class {\xt B}'s method \m 
is expecting another instance of {\xt B}. In order to protect class {\xt B} from the object \C 
potentially not behaving as an object {\xt B}. Our translation for Typed Racket inserts a behavioral 
cast at the boundary between the invocation of class {\xt B}'s method \m in class \A. 
The object with type \C is wrapped to make sure that it acts as an object of type {\xt B}, 
