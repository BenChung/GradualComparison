...
and a top-level expression in the small boxes below. 
...
{\bf L3} also fails in Typed Racket as the method invocation of \xt{.m(new C())} in the top-level expression 
is calling method \texttt{m} of class \xt E. This requires casting an instance of \C to \D,
which in turn requires method \m of class \C to only take and return values of type \D, however,
...
forall (k:ct) (s : heap) (e:expr) (t : type),
  ...
  (* case 2 *) 
  (exists (e' : expr) (s' : heap)(k':ct), 
    Steps k e s k' e' s' /\  WellFormedState k' e' s' /\
    HasType nil s' k' e' t /\ retains_references s s' /\ t_ext k k') \/
  (exists E : EvalCtx,
    (* case 3 *)
...
\begin{enumerate}
  \item The program is already fully evaluated (case 1). Since \kafka has only one value,
  ...
  \item The program can take a step (case 2). In this case, the program will
...
\verb|retains_references|) and class table \verb|k| (expressed using \verb|ct_ext|).
  ...
  \item The program could be stuck at a dynamic call expression (case 3). T
  ...
  \item The program could have gotten stuck on a subtype cast (case 4). 
  ...
  \item The program may get stuck on a behavioral cast (case 5), of the form
...
We therefore prove that a \kafka program will not get stuck, except if it
...
to force an object to act like one it cannot implement.
