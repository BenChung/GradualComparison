...
(define-metafunction L
  ...
  [(do-cast a (beh mt ...) σ c ...) (a σ (class ) c ...)]
...
(define red
  (reduction-relation L
                      #:domain p
                      (--> (c_1 ... (name c (class C fd ..._1 md ...)) c_2 ... (in-hole P (new C to v ..._1)) σ_1)
...
(test-equal (apply-reduction-relation* red (term ((class C (f anyt)) (class D) (new C opt (new C opt (new D opt))) ·)))
            '(((class C (f anyt)) (class D) 2 (2 ↦ (1 C) (1 ↦ (0 C) (0 ↦ (D) ·))))))
(test-equal (apply-reduction-relation* red (term ((class C (m (x anyt) anyt (new C opt))) (send (new C opt) m (new C opt)) ·)))
            '(((class C (m (x anyt) anyt (new C opt))) 2 (2 ↦ (C) (1 ↦ (C) (0 ↦ (C) ·))))))
(test-equal (apply-reduction-relation* red (term ((class C (f anyt)) (class D) (get (new C opt (new D opt)) f) ·)))
            '(((class C (f anyt)) (class D) 0 (1 ↦ (0 C) (0 ↦ (D) ·)))))
(test-equal (apply-reduction-relation* red (term ((class C (f anyt)) (class D) (set (new C opt (new D opt)) f (new D opt)) ·)))
...
(define-judgment-form L
  ...
  [(anatype (c_1 ... c c_2 ...) Γ e_1 t e_2) ...
   (where t_2 (class-to-type to c))
   ----- "A5"
   (syntype (c_1 ... (name c (class C (f t) ..._1 md ...)) c_2 ...) Γ (new C to e_1 ..._1) (new C to e_2 ...) t_2)]
  )
