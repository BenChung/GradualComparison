...
\newcommand{\class}[4]{\ensuremath\texttt{class } \texttt{#1} \texttt{ implements } \texttt{#2} \;\{#3; #4\}}
\newcommand{\field}[2]{\ensuremath \texttt{#1} : \texttt{#2}}
\newcommand{\method}[4]{\ensuremath {\texttt{#1}}({#2}):\texttt{#3} = \{\texttt{#4}\}}

\newcommand{\ifabs}[3]{\texttt{if } #1 \texttt{ then } #2 \texttt{ else } #3}
\newcommand{\opplus}[2]{#1 + #2}
\newcommand{\newclass}[2]{\texttt{new #1 }(#2)}
...
\begin{figure}
\begin{align*}
\texttt{d}&::= \class{C}{D}{\field{f}{T}}{\method{m}{\field{l}{T}}{T}{e}}\\
\texttt{T} &::= \intt \vbar \strt \vbar \anyt \vbar C \vbar !C\\
\texttt{e}&::= \texttt{f} \vbar \texttt{f} = \texttt{e} \vbar \texttt{e.m}(\texttt{e} \ldots) \vbar \texttt{const} \vbar 
\ifabs{\texttt{e}}{\texttt{e}}{\texttt{e}} \vbar \opplus{\texttt{e}}{\texttt{e}} \vbar \newclass{\texttt{C}}{\texttt{e}\ldots}
\end{align*}
